...
VDJSONReader::VDJSONReader() : mNameBuffer(NULL), mNameBufferIndex(0), mNameBufferLength(0) {}
  ...
  if (mpSrc != mpSrcEnd)
  {
    ...
    if (mbUTF32Mode)
    {
      if (sizeof(wchar_t) >= 4)
      {
        ...
        if (mbBigEndian)
        {
          ...
          {
            mInputBuffer[i + len] =
              (wchar_t)(((uint32)mpSrc[0] << 24) + ((uint32)mpSrc[1] << 16) + ((uint32)mpSrc[2] << 8) + mpSrc[3]);
            mpSrc += 4;
          }
        }
        else
    ...
    else
    {
      while (len < kInputBufferSize && mpSrc != mpSrcEnd)
      {
        ...
        { // invalid: follower without leader or too high of a code point
          encodingError = true;
          break;
        }
        ...
        else if (c >= 0xF0 && c <= 0xF7)
        {
          ...
          uint32 e =
            ((uint32)(c - 0xF0) << 18) + (((uint32)x0 - 0x80) << 12) + (((uint32)x1 - 0x80) << 6) + (uint32)(x2 - 0x80);
          if (e < 0x10000)
          {
            encodingError = true;
            break;
