...
VDRealFFT::VDRealFFT() : mpPermuteTable(NULL), mpWeightTable(NULL) {}

VDRealFFT::VDRealFFT(unsigned bits) : mpPermuteTable(NULL), mpWeightTable(NULL)
{
  Init(bits);
}
...
VDRollingRealFFT::VDRollingRealFFT()
  : mPoints(0), mBufferLevel(0), mpWindow(NULL), mpSampleBuffer(NULL), mpTempArea(NULL)
{}
