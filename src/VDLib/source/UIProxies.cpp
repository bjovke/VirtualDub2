...
VDUIProxyControl::VDUIProxyControl() : mhwnd(NULL), mRedrawInhibitCount(0) {}
...
VDUIProxyListView::VDUIProxyListView() : mChangeNotificationLocks(0), mNextTextIndex(0) {}
...
{
  return ((IVDUIListViewVirtualComparer *)cookie)
    ->Compare((IVDUIListViewVirtualItem *)x, (IVDUIListViewVirtualItem *)y);
}
  ...
  switch (hdr->code)
  {
    ...
    case LVN_ENDLABELEDITA: {
      ...
      if (di->item.pszText)
      {
        ...
        LabelChangedEvent event = {true, di->item.iItem, label.c_str()};
    ...
    case LVN_ENDLABELEDITW: {
      ...
      if (di->item.pszText)
      {
        LabelChangedEvent event = {true, di->item.iItem, di->item.pszText};
...
VDUIProxyTreeViewControl::VDUIProxyTreeViewControl() : mNextTextIndex(0) {}
...
VDUIProxyTreeViewControl::NodeRef VDUIProxyTreeViewControl::AddItem(
  NodeRef        parent,
  NodeRef        insertAfter,
  const wchar_t *label)
{
...
VDUIProxyTreeViewControl::NodeRef VDUIProxyTreeViewControl::AddVirtualItem(
  NodeRef                   parent,
  NodeRef                   insertAfter,
  IVDUITreeViewVirtualItem *item)
{
  if (!mhwnd)
    return NULL;
