...
VDUIDropFileListW32::VDUIDropFileListW32(VDZHDROP hdrop)
  : mhdrop(hdrop), mFileCount(DragQueryFile(mhdrop, 0xFFFFFFFF, NULL, 0))
{}
...
VDDialogFrameW32::VDDialogFrameW32(uint32 dlgid)
  : mpDialogResourceName(MAKEINTRESOURCEA(dlgid)), mbIsModal(false), mhdlg(NULL), mMinWidth(0), mMinHeight(0),
...
bool VDDialogFrameW32::Create(VDGUIHandle parent)
{
  if (!mhdlg)
  {
    ...
    CreateDialogParamW(
      g_hInst,
      IS_INTRESOURCE(mpDialogResourceName) ? (LPCWSTR)mpDialogResourceName : VDTextAToW(mpDialogResourceName).c_str(),
      (HWND)parent,
      StaticDlgProc,
      (LPARAM)this);
  }
...
vdrect32 VDDialogFrameW32::GetControlPos(uint32 id)
{
  if (mhdlg)
  {
    ...
    {
      RECT r;
      if (GetWindowRect(hwnd, &r) && MapWindowPoints(NULL, mhdlg, (LPPOINT)&r, 2))
      {
        return vdrect32(r.left, r.top, r.right, r.bottom);
      }
    }
...
void VDDialogFrameW32::ExchangeControlValueDouble(
  bool           write,
  uint32         id,
...
{
  return ::MessageBoxW(mhdlg, message, caption, MB_OKCANCEL | MB_ICONEXCLAMATION) == IDOK;
}
...
int VDDialogFrameW32::ActivateMenuButton(uint32 id, const wchar_t *const *items)
{
  ...
  params.rcExclude = r;
  UINT selectedId  = (UINT)TrackPopupMenuEx(
    hmenu,
    TPM_LEFTALIGN | TPM_TOPALIGN | TPM_HORIZONTAL | TPM_NONOTIFY | TPM_RETURNCMD,
    r.left,
    r.bottom,
    mhdlg,
    &params);
...
int VDDialogFrameW32::ActivatePopupMenu(int x, int y, const wchar_t *const *items)
{
  ...
  UINT selectedId = (UINT)TrackPopupMenuEx(
    hmenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_HORIZONTAL | TPM_NONOTIFY | TPM_RETURNCMD, x, y, mhdlg, NULL);
