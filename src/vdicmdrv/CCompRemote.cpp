...
{
  return (void *)((char *)ptr + x * (bih->biBitCount / 8) + y * 4 * ((bih->biBitCount * bih->biWidth + 31) / 32));
}
...
LRESULT CCompRemote::DecompressBegin(BITMAPINFO *lpbiInput, BITMAPINFO *lpbiOutput)
{
  ...
  if (
    (lres = ivdac->readFormat(&bmih, FALSE)) < 0 || bmih.biWidth != lpbiOutput->bmiHeader.biWidth ||
    bmih.biHeight != lpbiOutput->bmiHeader.biHeight)
  {
    if (lres)
      _RPT1(0, "Failed to read format (%d)\n", lres);
    else
      _RPT4(
        0,
        "Formats not same: %dx%d vs. %dx%d\n",
        bmih.biWidth,
        bmih.biHeight,
...
LRESULT CCompRemote::DecompressExQuery(ICDECOMPRESSEX *icdex, DWORD cbSize)
{
  ...
  if (bmihInput->biCompression != 'TSDV')
    return ICERR_BADFORMAT;
  ...
  if (icdex->lpbiDst)
  {
    ...
    if (bmihOutput->biPlanes != 1)
      return ICERR_BADFORMAT;
    if (bmihOutput->biCompression != BI_RGB)
      return ICERR_BADFORMAT;
    if (bmihOutput->biBitCount != 24)
      return ICERR_BADFORMAT;
    if (bmihOutput->biWidth != bmihInput->biWidth)
      return ICERR_BADFORMAT;
    if (bmihOutput->biHeight != bmihInput->biHeight)
      return ICERR_BADFORMAT;
  }
