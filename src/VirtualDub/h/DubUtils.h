...
{
public:
  VDDubAutoThreadLocation(const char *volatile &locationVar, const char *location) : mLocationVar(locationVar)
  {
...
public:
  ...
  void EnableInterleaving(bool bEnableInterleave)
  {
    mbInterleavingEnabled = bEnableInterleave;
  }
  void InitStream(
    int    stream,
    sint32 nPreload,
  ...
  bool   GetUseTimestamp(int streamID)
  {
    return mStreams[streamID].mbUseTimestamp;
  }
...
public:
  ...
  VDSignal &getReadSignal()
  {
    return msigRead;
  }
  VDSignal &getWriteSignal()
  {
    return msigWrite;
  }
