...
public:
  ...
  int GetVideoFilterAPIVersion() const
  {
    return mVFHighVersion;
  }

  const VDStringW &GetFilename() const
  {
    return mFilename;
  }
  VDXFilterModule &GetFilterModuleInfo()
  {
    return mModuleInfo;
  }
...
public:
  ...
  VDPluginDescription *operator->() const
  {
    return mpDesc;
  }
  VDPluginDescription &operator*() const
...
public:
  ...
  const uint32 &GetU32() const
  {
    return mData.vu32;
  }
  const sint32 &GetS32() const
  {
    return mData.vs32;
  }
  const uint64 &GetU64() const
  {
    return mData.vu64;
  }
  const sint64 &GetS64() const
  {
    return mData.vs64;
  }
  ...
  const void *GetBlockPtr() const
  {
    return mData.vb.s;
  }
  const unsigned GetBlockLen() const
  {
    return mData.vb.len;
  }
