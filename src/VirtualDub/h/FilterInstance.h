...
public:
  ...
  VDFilterFrameBuffer *GetBuffer() const
  {
    return mpBuffer;
  }
...
class VDFilterActivationImpl
{ // clone of VDXFilterActivation
...
public:
  ...
  virtual bool CreateRequest(
    ...
    bool                          writable,
    uint32                        batchNumber,
    IVDFilterFrameClientRequest **req)                                                                      = 0;
...
public:
  ...
  bool IsForceSingleFBEnabled() const
  {
    return mbForceSingleFB;
  }
  void SetForceSingleFBEnabled(bool en)
  {
    mbForceSingleFB = en;
  }
  ...
  void     GetRangeFrames(VDPosition &start, VDPosition &end) const
  {
    start = mRangeStart;
    end   = mRangeEnd;
  }

  VDParameterCurve *GetAlphaParameterCurve() const
  {
    return mpAlphaCurve;
  }
  void SetAlphaParameterCurve(VDParameterCurve *p)
  {
    mpAlphaCurve = p;
  }
...
class FilterInstance : protected VDFilterActivationImpl,
                       public VDFilterConfiguration,
                       public vdrefcounted<IVDFilterFrameSource>
{
...
public:
  ...
  uint32 GetFrameDelay() const
  {
    return mFlags >> 16;
  }

  bool GetInvalidFormatState() const
  {
    return mbInvalidFormat;
  }
  bool GetInvalidFormatHandlingState() const
  {
    return mbInvalidFormatHandling;
  }
  bool GetExcessiveFrameSizeState() const
  {
    return mbExcessiveFrameSize;
  }
  ...
  IVDFilterFrameSource *GetSource(uint32 index) const
  {
    return index < mSources.size() ? mSources[index] : (IVDFilterFrameSource *)NULL;
  }

  VDFilterStreamDesc GetSourceDesc() const
  {
    return mRealSrc.GetStreamDesc();
  }

  VDFilterStreamDesc GetOutputDesc() const
  {
    return mRealDst.GetStreamDesc();
  }
  uint32 GetOutputFrameWidth() const
  {
    return mRealDst.w;
  }
  uint32 GetOutputFrameHeight() const
  {
    return mRealDst.h;
  }
  VDFraction GetOutputFrameRate() const
  {
    return VDFraction(mRealDst.mFrameRateHi, mRealDst.mFrameRateLo);
  }
  sint64 GetOutputFrameCount() const
  {
    return mRealDst.mFrameCount;
  }
  const VDPixmapLayout &GetOutputLayout()
  {
    return mRealDst.mPixmapLayout;
  }

  sint64 GetLastSourceFrame() const
  {
    return mfsi.lCurrentSourceFrame;
  }
  sint64 GetLastOutputFrame() const
  {
    return mfsi.lCurrentFrame;
  }
  uint32 GetAlignReq() const
  {
    return mAlignReq;
  }
...
protected:
  ...
  const VDXFilterActivation *AsVDXFilterActivation() const
  {
    return (const VDXFilterActivation *)static_cast<const VDFilterActivationImpl *>(this);
  }
  VDXFilterActivation *AsVDXFilterActivation()
  {
    return (VDXFilterActivation *)static_cast<VDFilterActivationImpl *>(this);
  }
...
public:
  ...
  bool IsAltFormatCheckRequired() const
  {
    return mAPIVersion < 16;
  }

  VFBitmapInternal &GetOutputStream()
  {
    return mRealDst;
  }
  bool       CacheLookup(VDPosition frame, VDFilterFrameBuffer **buf);
  VDPosition LastResultFrame()
  {
    return mLastResultFrame;
  }
