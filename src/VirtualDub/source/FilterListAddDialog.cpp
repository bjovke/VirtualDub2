...
VDDialogFilterListW32::VDDialogFilterListW32() : VDDialogFrameW32(IDD_FILTER_LIST)
{
...
bool VDDialogFilterListW32::OnCommand(uint32 id, uint32 extcode)
{
  if (id == IDC_LOAD)
  {
    ...
    const VDStringW filename(VDGetLoadFileName(
      kFileDialog_LoadPlugin,
      (VDGUIHandle)mhdlg,
      title.c_str(),
      L"VirtualDub filter (*.vdf)\0*.vdf\0Windows Dynamic-Link Library (*.dll)\0*.dll\0All files (*.*)\0*.*\0",
      NULL,
      NULL,
      NULL));
...
void VDDialogFilterListW32::RebuildList()
{
  ...
  uintptr idx     = 0;
  uintptr sel_idx = 0;
  for (vdfastvector<const FilterBlurb *>::const_iterator it(mSortedFilters.begin()), itEnd(mSortedFilters.end());
       it != itEnd;
       ++it, ++idx)
  {
