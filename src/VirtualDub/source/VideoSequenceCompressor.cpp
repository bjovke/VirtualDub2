...
void VideoSequenceCompressor::SetDriver(
  EncoderHIC *driver,
  uint32      kilobytesPerSecond,
  long        quality,
  long        keyrate,
  bool        ownHandle)
{
...
void VideoSequenceCompressor::GetOutputFormat(
  const VDPixmapLayout *           inputFormat,
  vdstructex<tagBITMAPINFOHEADER> &outputFormat)
{
  vdprotected("querying video compressor for output format")
  {
    DWORD icErr;
...
void VideoSequenceCompressor::internalStart(
  const void *      outputFormat,
  uint32            outputFormatSize,
  const VDFraction &frameRate,
  VDPosition        frameCount)
{
...

  {
    VDExternalCodeBracket bracket(mDriverName.c_str(), __FILE__, __LINE__);
    lMaxPackedSize =
      driver->compressGetSize((LPBITMAPINFO) & *mInputFormat, (LPBITMAPINFO) & *mOutputFormat, &mInputLayout);
  }
...
void VideoSequenceCompressor::Truncate()
{
...

  {
    VDExternalCodeBracket bracket(mDriverName.c_str(), __FILE__, __LINE__);
    if (driver->vdproc)
      driver->vdproc(driver->obj, 0, VDICM_COMPRESS_TRUNCATE, 0, 0);
  }
}
...
bool VideoSequenceCompressor::packFrame(void *dst, const void *src, uint32 &size, VDPacketInfo &packetInfo)
{
  ...
  crunch_complete:;
    ...
    //		VDDEBUG("VideoSequenceCompressor: Packed frame %5d to %6u bytes; target=%d bytes / %d bytes, iterations = %d,
    //range = [%5d, %5d, %5d]\n", lFrameNum, bytes, lAllowableFrameSize, lMaxFrameSize, packs, mQualityLo, mQualityLast,
    //mQualityHi);
  }
  else
  { // No crunching or crunch directly supported
    ...
    //bytes, lAllowableFrameSize, lMaxFrameSize);
  }
  ...
  if (
    mOutputFormat->biCompression == '05xd' || mOutputFormat->biCompression == '05XD' ||
    mOutputFormat->biCompression == 'divx' || mOutputFormat->biCompression == 'DIVX')
  {
  ...
  if (pPrevBuffer && (!lKeyRate || lKeyRateCounter > 1))
  {
...

    {
      VDExternalCodeBracket bracket(mDriverName.c_str(), __FILE__, __LINE__);
      vdprotected4(
        "decompressing frame %u from %08x to %08x using codec \"%ls\"",
        unsigned,
        lFrameDone,
        unsigned,
        (unsigned)dst,
        unsigned,
        (unsigned)pPrevBuffer,
        const wchar_t *,
        mCodecName.c_str())
      {
        res = driver->decompress(
          dwFlagsOut & AVIIF_KEYFRAME ? 0 : ICDECOMPRESS_NOTKEYFRAME,
          &*mOutputFormat,
          dst,
          &*mInputFormat,
          pPrevBuffer);
...
void VideoSequenceCompressor::PackFrameInternal(
  void *        dst,
  DWORD         frameSize,
  DWORD         q,
  const void *  src,
  ...
  sint32 &      bytes)
{
  ...
  VDExternalCodeBracket bracket(mDriverName.c_str(), __FILE__, __LINE__);
  vdprotected4(
    "compressing frame %u from %08x to %08x using codec \"%ls\"",
    unsigned,
    lFrameSent,
    unsigned,
    (unsigned)src,
