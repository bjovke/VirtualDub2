...
VFBitmapInternal::VFBitmapInternal()
  : VBitmap(), dwFlags(0), hdc(NULL), mFrameRateHi(1), mFrameRateLo(0), mFrameCount(0),
    mpPixmap(reinterpret_cast<VDXPixmap *>(&mPixmap)),
    mpPixmapLayout(reinterpret_cast<VDXPixmapLayout *>(&mPixmapLayout)), mAspectRatioHi(0), mAspectRatioLo(0),
    mFrameNumber(0), mFrameTimestampStart(0), mFrameTimestampEnd(0), mCookie(0), mpBuffer(NULL), mVDXAHandle(0),
...
VFBitmapInternal::VFBitmapInternal(const VFBitmapInternal &src)
  : VBitmap(static_cast<const VBitmap &>(src)), dwFlags(src.dwFlags), hdc(src.hdc), mFrameRateHi(src.mFrameRateHi),
    mFrameRateLo(src.mFrameRateLo), mFrameCount(src.mFrameCount), mpPixmap(reinterpret_cast<VDXPixmap *>(&mPixmap)),
    mpPixmapLayout(reinterpret_cast<VDXPixmapLayout *>(&mPixmapLayout)), mPixmap(src.mPixmap),
    mPixmapLayout(src.mPixmapLayout), mAspectRatioHi(src.mAspectRatioHi), mAspectRatioLo(src.mAspectRatioLo),
    mFrameNumber(src.mFrameNumber), mFrameTimestampStart(src.mFrameTimestampStart),
    mFrameTimestampEnd(src.mFrameTimestampEnd), mCookie(src.mCookie), mpBuffer(src.mpBuffer),
    mVDXAHandle(src.mVDXAHandle), mBorderWidth(src.mBorderWidth), mBorderHeight(src.mBorderHeight)
{
  if (mpBuffer)
    mpBuffer->AddRef();
}
...
VDFilterActivationImpl::VDFilterActivationImpl()
  : filter(NULL), filter_data(NULL), dst((VDXFBitmap &)mRealDst), src((VDXFBitmap &)mRealSrc), _reserved0(NULL),
    last((VDXFBitmap *)&mRealLast), x1(0), y1(0), x2(0), y2(0), pfsi(NULL), ifp(NULL), ifp2(NULL), mSourceFrameCount(0),
    mpSourceFrames(NULL), mpOutputFrames(mOutputFrameArray), mpVDXA(NULL), mRealSrc(), mRealDst(), mRealLast()
{
...
VDFilterActivationImpl::VDFilterActivationImpl(const VDFilterActivationImpl &src)
  : filter(NULL), filter_data(NULL), dst((VDXFBitmap &)mRealDst), src((VDXFBitmap &)mRealSrc), _reserved0(NULL),
    last((VDXFBitmap *)&mRealLast), x1(0), y1(0), x2(0), y2(0), pfsi(NULL), ifp(NULL), ifp2(NULL), mSourceFrameCount(0),
    mpSourceFrames(NULL), mpOutputFrames(mOutputFrameArray), mpVDXA(NULL), mRealSrc(src.mRealSrc),
    mRealDst(src.mRealDst), mRealLast(src.mRealLast)
{
  VDASSERT(((char *)&mSizeCheckSentinel - (char *)this) == sizeof(VDXFilterActivation));
...
FilterInstance::VideoPrefetcher::VideoPrefetcher(uint32 sourceCount)
  : mDirectFrame(-1), mDirectFrameSrcIndex(0), mSymbolicFrame(-1), mSymbolicFrameSrcIndex(0), mSourceCount(sourceCount),
    mpError(NULL)
{}
...
VDFilterConfiguration::VDFilterConfiguration()
  : mbPreciseCrop(true), mCropX1(0), mCropY1(0), mCropX2(0), mCropY2(0), mBlendX1(0), mBlendY1(0), mBlendX2(0),
    mBlendY2(0), mRangeStart(0), mRangeEnd(-1), mbEnabled(true), mbForceSingleFB(false)
{}
...
VDFilterScriptWrapper::VDFilterScriptWrapper(const VDFilterScriptWrapper &src) : mFuncList(src.mFuncList)
{
  mpName      = NULL;
  obj_list    = NULL;
  Lookup      = NULL;
...
void VDFilterScriptWrapper::Init(
  const VDXScriptObject *src,
  VDScriptFunction       voidfunc,
  VDScriptFunction       intfunc,
  VDScriptFunction       varfunc)
{
  if (!src)
    return;
...
FilterInstance::FilterInstance(const FilterInstance &fi)
  : VDFilterConfiguration(fi), mbInvalidFormat(fi.mbInvalidFormat), mbInvalidFormatHandling(fi.mbInvalidFormatHandling),
    mbExcessiveFrameSize(fi.mbExcessiveFrameSize), mbAccelerated(fi.mbAccelerated), mFlags(fi.mFlags),
    mbStarted(fi.mbStarted), mbFirstFrame(fi.mbFirstFrame), mFilterName(fi.mFilterName), mpAutoDeinit(fi.mpAutoDeinit),
    mpLogicError(fi.mpLogicError), mScriptWrapper(fi.mScriptWrapper), mpFDInst(fi.mpFDInst), mpRequestInProgress(NULL),
    mbRequestFramePending(false), mbRequestFrameCompleted(false), mbRequestFrameBeingProcessed(false),
    mpAccelEngine(NULL), mpAccelContext(NULL), mPrepareInfo(), mPrepareInfo2(), fmProject(fi.fmProject)
{
  if (mpAutoDeinit)
    mpAutoDeinit->AddRef();
  else
...
FilterInstance::FilterInstance(FilterDefinitionInstance *fdi)
  : mbInvalidFormat(true), mbInvalidFormatHandling(false), mbExcessiveFrameSize(false), mbAccelerated(false), mFlags(0),
    mbStarted(false), mbFirstFrame(false), mpFDInst(fdi), mpAutoDeinit(NULL), mpLogicError(NULL),
    mpRequestInProgress(NULL), mbRequestFramePending(false), mbRequestFrameCompleted(false),
    mbRequestFrameBeingProcessed(false), mpAccelEngine(NULL), mpAccelContext(NULL), mPrepareInfo(), mPrepareInfo2()
{
...
FilterInstance *FilterInstance::Clone()
{
  ...
  if (fi->filter_data)
  {
    ...
    if (def->copyProc2)
      def->copyProc2(
        AsVDXFilterActivation(),
        &g_VDFilterCallbacks,
        fi->filter_data,
        fi->AsVDXFilterActivation(),
        &g_VDFilterCallbacks);
...
const VDXFilterDefinition *FilterInstance::GetDefinition() const
{
  return &mpFDInst->GetDef();
}
...
uint32 FilterInstance::Prepare(
  const VFBitmapInternal *inputs,
  uint32                  numInputs,
  VDFilterPrepareInfo &   prepareInfo,
  uint32                  exitAlign)
{
  ...
  for (uint32 streamIndex = 0; streamIndex < numInputs; ++streamIndex)
  {
    ...
    else
    {
      ...
      if (!streamIndex)
      {
        const VDPixmapFormatInfo &formatInfo = VDPixmapGetInfo(
          VDIsVDXAFormat(preAlignLayout.format) ? nsVDPixmap::kPixFormat_XRGB8888 : preAlignLayout.format);
  ...
  for (;;)
  {
    for (uint32 streamIndex = 0; streamIndex < numInputs; ++streamIndex)
    {
      ...
      if (streamInfo.mAlignOnEntry)
      {
        VDPixmapCreateLinearLayout(
          src.mPixmapLayout,
          src.mPixmapLayout.formatEx,
          src.mPixmapLayout.w,
          src.mPixmapLayout.h,
          streamInfo.mAlignOnEntry);
    ...
    if (aflags == FILTERPARAM_ALIGN_SCANLINES_16)
      mAlignReq = 16;
    if (aflags == FILTERPARAM_ALIGN_SCANLINES_32)
      mAlignReq = 32;
    if (aflags == FILTERPARAM_ALIGN_SCANLINES_64)
      mAlignReq = 64;
  ...
  if (mRealDst.depth)
  {
    ...
    if (
      (mRealDst.w | mRealDst.h) < 0 || mRealDst.w > kFrameAxisLimit || mRealDst.h > kFrameAxisLimit ||
      (uint64)mRealDst.w * mRealDst.h > kFrameSizeLimit)
    {
      mRealDst.offset      = 0;
      mRealDst.w           = 128;
      mRealDst.h           = 128;
  ...
  else
  {
    // check if the frame size is excessive and clamp if so
    if (
      (mRealDst.mPixmapLayout.w | mRealDst.mPixmapLayout.h) < 0 || mRealDst.mPixmapLayout.w > kFrameAxisLimit ||
      mRealDst.mPixmapLayout.h > kFrameAxisLimit ||
      (uint64)mRealDst.mPixmapLayout.w * mRealDst.mPixmapLayout.h > kFrameSizeLimit)
    {
    ...
    if (!mRealDst.mPixmapLayout.pitch)
    {
      ...
      {
        VDPixmapCreateLinearLayout(
          mRealDst.mPixmapLayout,
          mRealDst.mPixmapLayout.formatEx,
          mRealDst.mPixmapLayout.w,
          mRealDst.mPixmapLayout.h,
          exitAlign);
...
void FilterInstance::Start(
  uint32                       flags,
  IVDFilterFrameSource *const *pSources,
  IVDFilterFrameEngine *       engine,
  VDFilterAccelEngine *        accelEngine)
{
  if (mbStarted)
    return;
  ...
  if (GetInvalidFormatHandlingState())
    throw MyError("Cannot start filters: Filter \"%s\" is not handling image formats correctly.", GetName());

  if (GetInvalidFormatState())
    throw MyError(
      "Cannot start filters: An instance of filter \"%s\" cannot process its input of %ux%d (%s).",
      GetName(),
...
bool FilterInstance::CreateRequest(
  sint64                        outputFrame,
  bool                          writable,
  uint32                        batchNumber,
  IVDFilterFrameClientRequest **req)
{
  VDASSERT(mbStarted);
    ...
    else
    {
      ...
      if (!mSources.empty())
      {
        ...
        else
        {
          ...
          uint32 idx = 0;
          for (VideoPrefetcher::SourceFrames::const_iterator it(vp.mSourceFrames.begin()),
               itEnd(vp.mSourceFrames.end());
               it != itEnd;
               ++it)
          {
...
bool FilterInstance::CreateSamplingRequest(
  sint64                         outputFrame,
  VDXFilterPreviewSampleCallback sampleCB,
  void *                         sampleCBData,
  IFilterModPreviewSample *      sampleHandler,
  uint32                         batchNumber,
  IVDFilterFrameClientRequest ** req)
{
  VDASSERT(mbStarted);
  ...
  if (!mSources.empty())
  {
    ...
    uint32 idx = 0;
    for (VideoPrefetcher::SourceFrames::const_iterator it(vp.mSourceFrames.begin()), itEnd(vp.mSourceFrames.end());
         it != itEnd;
         ++it)
    {
      const VideoPrefetcher::PrefetchInfo &prefetchInfo = *it;
...
bool FilterInstance::BeginFrame(
  VDFilterFrameRequest &            request,
  uint32                            sourceOffset,
  uint32                            sourceCountLimit,
  const VDFilterFrameRequestTiming *overrideTiming)
{
  VDASSERT(!mbRequestFramePending);
  ...
  mfsi.lSourceFrameMS      = VDClampToSint32(
    VDRoundToInt64((double)di.mSourceFrame * (double)mRealSrc.mFrameRateLo / (double)mRealSrc.mFrameRateHi * 1000.0));
  mfsi.lDestFrameMS = VDClampToSint32(
    VDRoundToInt64((double)di.mOutputFrame * (double)mRealDst.mFrameRateLo / (double)mRealDst.mFrameRateHi * 1000.0));
...
void FilterInstance::EndFrame()
{
  ...
  if (mRequestError.gets())
  {
    ...
    if (err)
      err->mError.sprintf(
        "Error processing frame %lld with filter '%s': %s", mRequestOutputFrame, filter->name, mRequestError.gets());
...
void FilterInstance::RunFilterInner()
{
  ...
  {
    if (
      mRealSrc.w == mRealDst.w && mRealSrc.h == mRealDst.h &&
      mRealSrc.mPixmapLayout.format == mRealDst.mPixmapLayout.format)
    {
      if (outputFrame < mRangeStart)
        alpha = 0;
      if (outputFrame >= mRangeEnd)
        alpha = 0;
    }
  }
  ...
  if (!skipFilter)
  {
    ...
    else
    {
      vdprotected1("running filter \"%s\"", const char *, filter->name)
      {
        ...
        {
          sampleCB(
            &AsVDXFilterActivation()->src, mfsi.mOutputFrame, VDClampToSint32(mRealDst.mFrameCount), sampleCBData);
        }
        else
        {
          ...
          if (view)
            view->SetImage(mRealDst.mPixmap);
          view = 0;
        }
      }
    }
...
bool FilterInstance::ConnectAccelBuffer(VFBitmapInternal *buf, bool bindAsRenderTarget)
{
  ...
  if (bindAsRenderTarget)
  {
    ...
    buf->mVDXAHandle =
      mpAccelContext->RegisterRenderTarget(surf, buf->w, buf->h, fbuf->GetBorderWidth(), fbuf->GetBorderHeight());
...
bool FilterInstance::GetDirectMapping(sint64 outputFrame, sint64 &sourceFrame, int &sourceIndex)
{
  ...
  if (prefetcher.mDirectFrame >= 0)
    return mSources[prefetcher.mDirectFrameSrcIndex]->GetDirectMapping(
      prefetcher.mDirectFrame, sourceFrame, sourceIndex);
  ...
  return mSources[prefetcher.mSourceFrames[0].mSrcIndex]->GetDirectMapping(
    prefetcher.mSourceFrames[0].mFrame, sourceFrame, sourceIndex);
}
...
void FilterInstance::GetPrefetchInfo(sint64 outputFrame, VideoPrefetcher &prefetcher, bool requireOutput) const
{
  ...
  double factor = ((double)mRealSrc.mFrameRateHi * (double)mRealDst.mFrameRateLo) /
                  ((double)mRealSrc.mFrameRateLo * (double)mRealDst.mFrameRateHi);
...
void FilterInstance::CheckValidConfiguration()
{
  ...
  if (IsOpacityEnabled())
  {
    ...
    {
      throw MyError(
        "Cannot start filter chain: Filter \"%s\" is using blending and has differing input and output sizes (%dx%d -> "
        "%dx%d). Input and output sizes must match.",
        GetName(),
        mRealSrc.w,
        mRealSrc.h,
        mRealDst.w,
    ...
    {
      throw MyError(
        "Cannot start filter chain: Filter \"%s\" is using blending and has differing input and output formats. Input "
        "and output formats must match.",
        GetName());
    }
  }
}
...
void FilterInstance::InitSharedBuffers(VDFixedLinearAllocator &lastFrameAlloc)
{
  ...
  {
    uint32 mapSize = std::max<uint32>(
      VDPixmapLayoutGetMinSize(mRealSrc.mPixmapLayout), VDPixmapLayoutGetMinSize(mRealDst.mPixmapLayout));
