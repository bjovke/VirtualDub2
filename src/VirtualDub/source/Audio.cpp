...
AudioFormatConverter AudioPickConverter(const VDWaveFormat *src, bool to_16bit, bool to_stereo)
{
  return acv[(src->mChannels > 1 ? 8 : 0) + (src->mSampleBits > 8 ? 4 : 0) + (to_stereo ? 2 : 0) + (to_16bit ? 1 : 0)];
}

AudioFormatConverter AudioPickConverterSingleChannel(const VDWaveFormat *src, bool to_16bit)
{
  return acv2[(src->mChannels > 1 ? 4 : 0) + (src->mSampleBits > 8 ? 2 : 0) + (to_16bit ? 1 : 0)];
}

///////////////////////////////////

AudioStream::AudioStream()
  : format(NULL), format_len(0), source(NULL), samples_read(0), stream_len(0), stream_limit(0x7FFFFFFFFFFFFFFF)
{}
  ...
  : AudioStream()
{
  ...
  bool isPCM = false;
  if (is_audio_pcm(iFormat) || is_audio_float(iFormat))
    isPCM = true;


  if (!isPCM && allow_decompression)
  {
    mpCodec = VDLocateAudioDecompressor(
      (const VDWaveFormat *)iFormat, NULL, VDPreferencesIsPreferInternalAudioDecodersEnabled());
...
long AudioStreamSource::_Read(void *buffer, long max_samples, long *lplBytes)
{
  ...
  if (mPrefill > 0)
  {
    ...
    else
    {
      ...
      if (err == DubSource::kFileReadError)
        throw MyError(
          "Audio sample %lu could not be read in the source.  The file may be corrupted.", (unsigned long)startPos);
      else if (err)
      ...
      //			if (!fZeroRead)
      {
        ...
        if (bytes >= mBlockSize)
        {
          ...
          do
          {
            ...
            vdprotected3(
              "reading %u compressed audio samples starting at %I64d (stream length=%I64d)",
              unsigned,
              to_read,
              sint64,
              cur_samp,
            ...
            if (err != DubSource::kOK && err != DubSource::kBufferTooSmall)
            {
              if (err == DubSource::kFileReadError)
                throw MyError(
                  "Audio samples %lu-%lu could not be read in the source.  The file may be corrupted.",
                  (unsigned long)cur_samp,
                  (unsigned long)(cur_samp + to_read - 1));
              else
                throw MyAVIError("AudioStreamSource", err);
            }
  ...
  else
  {
    ...
    if (max_samples > 0)
    {
      ...
      vdprotected3(
        "reading %u raw audio samples starting at %I64d (stream length=%I64d)",
        unsigned,
        max_samples,
        sint64,
        cur_samp,
      ...
      if (DubSource::kOK != err)
      {
        if (err == DubSource::kFileReadError)
          throw MyError(
            "Audio samples %lu-%lu could not be read in the source.  The file may be corrupted.",
            (unsigned long)cur_samp,
            (unsigned long)(cur_samp + max_samples - 1));
        else
          throw MyAVIError("AudioStreamSource", err);
      }
...
AudioStreamConverter::AudioStreamConverter(AudioStream *src, bool to_16bit, bool to_stereo_or_right, bool single_only)
{
  ...
  bytesPerInputSample = (iFormat->mChannels > 1 ? 2 : 1) * (iFormat->mSampleBits > 8 ? 2 : 1);

  bytesPerOutputSample = (to_stereo ? 2 : 1) * (to_16bit ? 2 : 1);
...
long AudioStreamConverter::_Read(void *buffer, long samples, long *lplBytes)
{
  ...
  while (samples > 0)
  {
    ...
    if (srcSamples > BUFFER_SIZE)
      srcSamples = BUFFER_SIZE;
...
static long audio_downsample_mono8(
  void *dst,
  void *src,
  long *filter_bank,
  int   filter_width,
...
static long audio_downsample_mono16(
  void *dst,
  void *src,
  long *filter_bank,
  int   filter_width,
...
static long audio_downsample_stereo8(
  void *dst,
  void *src,
  long *filter_bank,
  int   filter_width,
...
static long audio_downsample_stereo16(
  void *dst,
  void *src,
  long *filter_bank,
  int   filter_width,
...
static void make_downsample_filter(long *filter_bank, int filter_width, long samp_frac)
{
  ...
  for (i = 0; i < 128 * filter_width; i++)
  {
    ...
    filter_bank[permute_index(128 * filter_width + i, filter_width)] =
      filter_bank[permute_index(128 * filter_width - i, filter_width)] = y;
  }
...
AudioStreamResampler::AudioStreamResampler(AudioStream *src, long new_rate, bool hi_quality) : AudioStream()
{
  ...
  bytesPerSample = (iFormat->mChannels > 1 ? 2 : 1) * (iFormat->mSampleBits > 8 ? 2 : 1);

  uint64 sampFrac64 = (((uint64)iFormat->mSamplingRate << 20) / new_rate + 1) >> 1;

  if (sampFrac64 > 0x7FFFFFFF)
    throw MyError(
      "Cannot resample audio from %uHz to %uHz: the conversion ratio is too low.", iFormat->mSamplingRate, new_rate);

  samp_frac = (long)sampFrac64;

  if (samp_frac == 0)
    throw MyError(
      "Cannot resample audio from %uHz to %uHz: the conversion ratio is too high.", iFormat->mSamplingRate, new_rate);
  ...
  if (!newSamplingRate || newSamplingRate > 0x7FFFFFFF)
    throw MyError(
      "Cannot resample audio from %uHz to %uHz: the conversion ratio is too %s.",
      iFormat->mSamplingRate,
      new_rate,
      newSamplingRate ? "high" : "low");
...
sint64 AudioTranslateVideoSubset(
  FrameSubset &       dst,
  const FrameSubset & src,
  const VDFraction &  videoFrameRate,
  const VDWaveFormat *pwfex,
  sint64              tail,
  IVDStreamSource *   pVBRAudio)
{
...
void VDTranslateVideoSubsetToAudioSubset(
  FrameSubset &                      dst,
  const vdfastvector<AudioStream *> &sources,
  const FrameSubset &                videoSubset,
  const VDFraction &                 videoFrameRate,
  bool                               appendTail)
{
  sint64 total = 0;
...
AudioSubset::AudioSubset(
  ...
  sint64                             preskew,
  bool                               appendTail)
  : mCurrentSource(-1), mpCurrentSource(NULL), mbLimited(!appendTail), mbEnded(false)
{
...
AudioStreamAmplifier::AudioStreamAmplifier(AudioStream *src, float factor) : mFactor(VDRoundToInt(256.0f * factor))
{
  VDWaveFormat *iFormat = src->GetFormat();
  VDWaveFormat *oFormat;
