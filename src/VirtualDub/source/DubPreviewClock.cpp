...
VDDubPreviewClock::VDDubPreviewClock() : mpTimer(NULL), mpBlitter(NULL), mTicksToFrames(1.0) {}

VDDubPreviewClock::~VDDubPreviewClock() {}

void VDDubPreviewClock::Init(
  IVDDubPreviewTimer *timer,
  IVDAsyncBlitter *   blitter,
  double              frameRate,
  double              frameMultiplicationFactor)
{
  mpTimer        = timer;
  mpBlitter      = blitter;
