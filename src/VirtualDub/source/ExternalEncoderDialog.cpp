...
{
public:
  JSONStreamWriter(IVDStream *output) : mOutput(output) {}
...
VDUIDialogExtEncMain::VDUIDialogExtEncMain(VDExtEncProfile &profile)
  : VDDialogFrameW32(IDD_EXTENC_EDIT_MAIN), mProfile(profile), mhFontMarlett(NULL)
{
...
bool VDUIDialogExtEncMain::OnCommand(uint32 id, uint32 extcode)
{
  if (id == IDC_BROWSE)
  {
    const VDStringW fn(VDGetLoadFileName(
      'encp', (VDGUIHandle)mhdlg, L"Select External Encoder", L"Executable Program (*.exe)\0*.exe\0", L"exe"));
...
VDUIDialogExtEncAudio::VDUIDialogExtEncAudio(VDExtEncProfile &profile)
  : VDDialogFrameW32(IDD_EXTENC_EDIT_AUDIO), mProfile(profile)
{}
...
VDUIDialogExtEncVideo::VDUIDialogExtEncVideo(VDExtEncProfile &profile)
  : VDDialogFrameW32(IDD_EXTENC_EDIT_VIDEO), mProfile(profile)
{
  LOGFONT f = {
    14,
    0,
...
VDUIDialogExtEncProfile::VDUIDialogExtEncProfile(VDExtEncProfile &profile)
  : VDDialogFrameW32(IDD_EXTENC_EDIT), mMainPane(profile), mAudioPane(profile), mVideoPane(profile), mProfile(profile)
{
...
VDUIDialogEditExternalEncoderSet::VDUIDialogEditExternalEncoderSet(VDExtEncSet &eset)
  : VDDialogFrameW32(IDD_EXTENC_EDIT_SET), mSet(eset)
{}
...
void VDUIDialogEditExternalEncoderSet::OnDataExchange(bool write)
{
  ...
  else
  {
    Profiles::const_iterator it(
      std::find_if(mVideoProfiles.begin(), mVideoProfiles.end(), FindEncByIndexPred(mSet.mVideoEncoder.c_str())));
...
protected:
  ...
  public:
    ...
    {
      return vdwcsicmp(
        static_cast<ListSetItem *>(x)->mpSet->mName.c_str(), static_cast<ListSetItem *>(y)->mpSet->mName.c_str());
    }
  };
  ...
  public:
    ...
    {
      return vdwcsicmp(
        static_cast<ListProfileItem *>(x)->mpProfile->mName.c_str(),
        static_cast<ListProfileItem *>(y)->mpProfile->mName.c_str());
    }
  };
...
VDUIDialogConfigureExternalEncoders::VDUIDialogConfigureExternalEncoders()
  : VDDialogFrameW32(IDD_EXTENC_SETS), mbSetMode(true)
{
  mTabControl.OnSelectionChanged() +=
    mDelegateTabChanged.Bind(this, &VDUIDialogConfigureExternalEncoders::OnTabChanged);
  mListView.OnItemSelectionChanged() +=
    mDelegateSelectionChanged.Bind(this, &VDUIDialogConfigureExternalEncoders::OnSelChanged);
  mListView.OnItemDoubleClicked() +=
    mDelegateDoubleClick.Bind(this, &VDUIDialogConfigureExternalEncoders::OnDoubleClick);
  mListView.OnItemLabelChanged() +=
    mDelegateLabelChanged.Bind(this, &VDUIDialogConfigureExternalEncoders::OnLabelChanged);
}
  ...
  try
  {
    ...
    else if (id == IDC_IMPORT)
    {
      const VDStringW fn(VDGetLoadFileName(
        'impo',
        (VDGUIHandle)mhdlg,
        L"Import external encoder profiles",
        L"External encoder profiles (*.vdprof)\0*.vdprof\0",
        L"vdprof"));
      ...
      if (sets.IsValid())
      {
        for (VDJSONMemberEnum setEnum(sets.AsObject()); setEnum.IsValid(); ++setEnum)
        {
          ...
          if (!overwriteConfirmed && VDGetExternalEncoderSetByName(eset->mName.c_str(), NULL))
          {
            ...
            message.sprintf(
              L"The file contains an external encoder set called \"%ls\" that already exists. Do you want to load the "
              L"file anyway, replacing existing profiles with the same names?",
              eset->mName.c_str());
      ...
      if (profs.IsValid())
      {
        for (VDJSONMemberEnum profEnum(profs.AsObject()); profEnum.IsValid(); ++profEnum)
        {
          ...
          if (!overwriteConfirmed && VDGetExternalEncoderProfileByName(eprof->mName.c_str(), NULL))
          {
            ...
            message.sprintf(
              L"The file contains an external encoder profile called \"%ls\" that already exists. Do you want to load "
              L"the file anyway, replacing existing profiles with the same names?",
              eprof->mName.c_str());
    ...
    {
      const VDStringW fn(VDGetSaveFileName(
        'expo',
        (VDGUIHandle)mhdlg,
        L"Export external encoder profiles",
        L"External encoder profiles (*.vdprof)\0*.vdprof\0",
        L"vdprof"));
...
void VDUIDialogConfigureExternalEncoders::OnLabelChanged(
  VDUIProxyListView *                   sender,
  VDUIProxyListView::LabelChangedEvent *eventData)
{
  if (mbSetMode)
  {
