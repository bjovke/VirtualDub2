...
VDCrashResponse VDDisplayFriendlyCrashDialog(
  HWND                      hwndParent,
  HANDLE                    hThread,
  const EXCEPTION_POINTERS *pExc,
  const char *              pszScopeInfo,
  bool                      allowForcedExit);
bool VDDisplayAdvancedCrashDialog(
  HWND                      hwndParent,
  HANDLE                    hThread,
  const EXCEPTION_POINTERS *pExc,
  const char *              pszScopeInfo,
...
static ModuleInfo *CrashGetModules(void *&ptr)
{
  ...
  if (hmodPSAPI)
  {
    ...
    PGETMODULEINFORMATION pGetModuleInformation =
      (PGETMODULEINFORMATION)GetProcAddress(hmodPSAPI, "GetModuleInformation");
    HMODULE *pModules, *pModules0 = (HMODULE *)((char *)pMem + 0xF000);
    DWORD    cbNeeded;

    if (
      pEnumProcessModules && pGetModuleBaseName && pGetModuleInformation &&
      pEnumProcessModules(GetCurrentProcess(), pModules0, 0x1000, &cbNeeded))
    {
      ...
      do
      {
        ...
        if (
          pGetModuleBaseName(GetCurrentProcess(), hCurMod, pszHeap, pszHeapLimit - pszHeap) &&
  ...
  else
  {
    ...
    PCREATETOOLHELP32SNAPSHOT pCreateToolhelp32Snapshot =
      (PCREATETOOLHELP32SNAPSHOT)GetProcAddress(hmodKERNEL32, "CreateToolhelp32Snapshot");
...
static bool LookupModuleByAddress(
  ModuleInfo &              mi,
  char *                    szTemp,
  const ModuleInfo *        pMods,
  uintptr                   addr,
...
void VDThreadInitHandler(bool bInitThread, const char *debugName)
{
  if (bInitThread)
  {
    ...
    if (DuplicateHandle(
          ...
          GetCurrentThread(),
          GetCurrentProcess(),
          (HANDLE *)&g_PerThreadState.hThread,
          NULL,
          FALSE,
...
static void VDDebugDumpCrashContext(EXCEPTION_POINTERS *pExc, IVDProtectedScopeOutput &out)
{
  ...
  switch (exr.ExceptionCode)
  {
    ...
    case EXCEPTION_ILLEGAL_INSTRUCTION:
      ...
      {
        case kX86Inst3DNow:
          pszExceptionName = "A 3DNow! (K6/Athlon) instruction not supported by the CPU was executed";
          break;
        case kX86InstMMX:
          pszExceptionName = "An MMX instruction not supported by the CPU was executed";
          break;
        case kX86InstMMX2:
          pszExceptionName = "An integer SSE (Pentium III/Athlon) instruction not supported by the CPU was executed";
          break;
        case kX86InstSSE:
          pszExceptionName =
            "A floating-point SSE (Pentium III/Athlon XP) instruction not supported by the CPU was executed";
          break;
        case kX86InstSSE2:
          pszExceptionName = "An SSE2 (Pentium 4/Athlon 64) instruction not supported by the CPU was executed";
          break;
        case kX86InstP6:
          pszExceptionName = "A Pentium Pro instruction not supported by the CPU was executed";
          break;
        case kX86InstUnknown:
...
public:
  VDCrashUIThread(HANDLE hThread, const EXCEPTION_POINTERS *pExc, const char *scopeinfo, bool allowForcedExit)
    : mhThread(hThread), mpExc(pExc), mpScopeInfo(scopeinfo), mbAllowForcedExit(allowForcedExit)
  {}
...
public:
  VDDebugCrashTextOutputFile(const char *pszFilename, const wchar_t *pwszFilename)
    : mhFile(CreateFileW(pwszFilename, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL)), mNext(0),
...
long VDDebugInfoLookupRVA(VDDebugInfoContext *pctx, unsigned rva, char *buf, int buflen)
{
  ...
  if (pr < pr_limit)
  {
    ...
    return wsprintf(buf, "%s%s%s%s", class_name ? class_name : "", class_name ? "::" : "", prefix, fn_name) >= 0 ? rva :
                                                                                                                   -1;
  }
...
{
  typedef NTSTATUS(WINAPI * tpNtQueryInformationThread)(
    HANDLE ThreadHandle,
  ...
  const tpNtQueryInformationThread pNtQueryInformationThread =
    (tpNtQueryInformationThread)GetProcAddress(GetModuleHandle("ntdll"), "NtQueryInformationThread");
...
NT_TIB *VDGetThreadTibW32(HANDLE hThread, const CONTEXT *pContext)
{
  ...
  return (
    NT_TIB
      *)(ldtEnt.BaseLow + ((uintptr)ldtEnt.HighWord.Bytes.BaseMid << 16) + ((uintptr)ldtEnt.HighWord.Bytes.BaseHi << 24));
}
#endif


static void VDDebugCrashDumpCallStack(
  VDDebugCrashTextOutput &        out,
  HANDLE                          hThread,
  const EXCEPTION_POINTERS *const pExc,
  const void *                    pDebugSrc)
{
  const CONTEXT *const pContext = (const CONTEXT *)pExc->ContextRecord;
  HANDLE               hprMe    = GetCurrentProcess();
  int                  limit    = 100;
  ...
  {
    out.WriteF(
      "Incorrect VirtualDub.vdi file (build %d) for this version of VirtualDub -- call stack unavailable.\n",
    ...
    if (fValid)
    {
      ...
      else
      {
        ...
        if (LookupModuleByAddress(mi, szName, pModules, data, &meminfo))
        {
          ...
          if (pExportName)
            out.WriteF(
              PTR_08lx ": %s!%s [" PTR_08lx "+%lx+%lx]\n",
              data,
              mi.name,
              pExportName,
              mi.base,
              fnbase,
              (long)(data - mi.base - fnbase));
          else
  ...
  const char *              pszScopeInfo)
{
  ...
  if (GetVersionEx(&ovi))
  {
    ...
    out.WriteF(
      "Windows %d.%d (Windows %s %s build %d) [%s]\n",
      ovi.dwMajorVersion,
      ovi.dwMinorVersion,
      version,
      build,
  ...
  tpGlobalMemoryStatusEx pGlobalMemoryStatusEx =
    (tpGlobalMemoryStatusEx)GetProcAddress(hmodKernel32, "GlobalMemoryStatusEx");
  ...
  {
    out.WriteF(
      "Memory status: virtual free %uM/%uM, commit limit %uM, physical total %uM\n",
...
protected:
  VDCrashDialog(HANDLE hThread, const EXCEPTION_POINTERS *pExc, const char *pszScopeInfo, bool allowForcedExit)
    : mhThread(hThread), mpExc(pExc), mpszScopeInfo(pszScopeInfo), mbAllowForcedExit(allowForcedExit)
  {}
...
public:
  ...
  {
    return (VDCrashResponse)DialogBoxParam(
      g_hInst, MAKEINTRESOURCE(IDD_CRASH), hwndParent, StaticDlgProc, (LPARAM) static_cast<VDCrashDialog *>(this));
  }
