...
AVIStripeSystem::AVIStripeSystem(const char *szFile)
{
  ...
  try
  {
    ...
    for (cur = 0; cur < stripe_cnt; cur++)
    {
      ...
      {
        case 'm':
          cMode[0] = AVIStripe::MODE_MASTER;
          break;
        case 'i':
          cMode[0] = AVIStripe::MODE_INDEX;
          break;
        case 'v':
          cMode[0] = AVIStripe::MODE_VIDEO;
          break;
        case 'a':
          cMode[0] = AVIStripe::MODE_AUDIO;
          break;
        case 'b':
          cMode[0] = AVIStripe::MODE_BOTH;
          break;
        default:
...
AVIStripeIndexLookup::AVIStripeIndexLookup(IAVIReadStream *pasIndex)
{
  ...
  try
  {
    ...
    if (-1 == (lStart = (long)pasIndex->Start()) || -1 == (lEnd = (long)pasIndex->End()))
    ...
    while (lCur < lEnd)
    {
      ...
      err = pasIndex->Read(
        lCur, lEnd - lCur, asieptr, sizeof(AVIStripeIndexEntry) * (lEnd - lCur), &lActualBytes, &lActualSamples);

      if (err == AVIERR_OK && !lActualSamples)
        err = AVIERR_FILEREAD;

      if (err != AVIERR_OK)
        throw MyAVIError("AVIStripeIndex", err);
...
AVIStripeIndexEntry *AVIStripeIndexLookup::lookup(long sample)
{
  ...
  while (l <= r)
  {
    ...
    {
      //			_RPT2(0,"\tFound in stripe %ld at %ld\n", index_table[pivot].lStripe+1,
      //index_table[pivot].lStripeSample);
