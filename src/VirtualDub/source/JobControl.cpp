...
VDJobQueue::VDJobQueue()
  : mJobCount(0), mJobNumber(1), mpRunningJob(NULL), mbRunning(false), mbRunAll(false), mbRunAllStop(false),
    mbModified(false), mbBlocked(false), mbOrderModified(false), mbAutoRun(false), mbDistributedMode(false),
    mJobIdToRun(0), mLastSignature(0), mLastRevision(0)
{
...
bool VDJobQueue::Load(IVDStream *stream, bool merge)
{
  ...
  try
  {
    ...
    if (merge)
    {
      ...
      // mLastRevision, mbOrderModified ? "" : " not", newRevision);
      ...
      dstQueue.erase(
        std::remove_if(dstQueue.begin(), dstQueue.end(), std::bind2nd(std::equal_to<VDJob *>(), (VDJob *)NULL)),
        dstQueue.end());
...
void VDJobQueue::Save(IVDStream *stream, uint64 signature, uint32 revision, bool resetJobRevisions)
{
  ...
  for (; it != itEnd; ++it)
  {
    ...
    if (!resetJobRevisions)
    {
      ...
      output.FormatLine(
        "// $revision %x %x", vdj->mCreationRevision ? vdj->mCreationRevision : revision, vdj->mChangeRevision);
    }

    if (
      state == VDJob::kStateInProgress || state == VDJob::kStateAborting || state == VDJob::kStateCompleted ||
      state == VDJob::kStateError)
    {
    ...
    output.FormatLine(
      "// $start_time %08lx %08lx", (unsigned long)(vdj->mDateStart >> 32), (unsigned long)vdj->mDateStart);
    output.FormatLine("// $end_time %08lx %08lx", (unsigned long)(vdj->mDateEnd >> 32), (unsigned long)vdj->mDateEnd);

    for (VDJob::tLogEntries::const_iterator it(vdj->mLogEntries.begin()), itEnd(vdj->mLogEntries.end()); it != itEnd;
         ++it)
    {
...
bool VDJobQueue::RunAllNext()
{
  ...
  if (key.getBool(g_szRegKeyShutdownWhenFinished))
  {
    ...
    if (do_shutdown)
    {
      if (!VDInitiateSystemShutdownWithUITimeout(
            shutdownMode,
