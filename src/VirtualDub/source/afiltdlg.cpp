...
public:
  ...
  const VDPluginDescription *GetFilter() const
  {
    return mpSelectedFilter;
  }
...
VDDialogAddAudioFilterW32::VDDialogAddAudioFilterW32(IVDDialogAddAudioFilterCallbackW32 *pParent)
  : VDDialogFrameW32(IDD_AF_LIST), mpSelectedFilter(NULL), mpParent(pParent)
{
...
bool VDDialogAddAudioFilterW32::OnLoaded()
{
  ...
  if (r2.right <= GetSystemMetrics(SM_CXSCREEN) && r2.bottom <= GetSystemMetrics(SM_CYSCREEN))
    SetWindowPos(
      mhdlg, NULL, r2.left, r2.top, r2.right - r2.left, r2.bottom - r2.top, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
  ...
  for (std::vector<VDPluginDescription *>::const_iterator it(mAudioFilters.begin()), itEnd(mAudioFilters.end());
       it != itEnd;
       ++it)
  {
...
VDAudioFilterPreviewThread::VDAudioFilterPreviewThread() : mbRequestExit(false), mbSuspend(false), mbResume(false)
{
...
class VDDialogAudioFiltersW32 : public VDDialogFrameW32,
                                public IVDDialogAddAudioFilterCallbackW32,
                                public IVDFilterGraphControlCallback
{
public:
...
VDDialogAudioFiltersW32::VDDialogAudioFiltersW32(VDAudioFilterGraph &afg, AudioSource *pAS)
  : VDDialogFrameW32(IDD_AF_SETUP), mpGraphControl(NULL), mAddDialog(this), mGraph(afg), mpAudio(pAS), mpPosition(NULL),
    mbPreviewActive(false), mbPreviewWasActive(false)
{}
...
void VDDialogAudioFiltersW32::LoadGraph(IVDFilterGraphControl *pSrcGraph, const VDAudioFilterGraph &graph)
{
  ...
  {
    for (std::vector<VDAudioFilterGraph::FilterConnection>::const_iterator it(graph.mConnections.begin()),
         itEnd(graph.mConnections.end());
         it != itEnd;
         ++it)
    {
