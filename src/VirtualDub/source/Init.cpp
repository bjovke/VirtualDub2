...
static void crash3()
{
  ...
  for (int i = 0; i < 10; ++i)
  {
    vdprotected1("in foo iteration %d", int, i)
      //			volatile VDProtectedAutoScope1<int> autoscope(VDProtectedAutoScopeData1<int>(__FILE__,
      //__LINE__, "in foo iteration %d", i));
...
static void crash()
{
  vdprotected("deliberately trying to crash") crash2();
}
...
bool Init(HINSTANCE hInstance, int nCmdShow, VDCommandLine &cmdLine)
{
  ...
  if (cmdLine.FindAndRemoveSwitch(L"console"))
  {
    ...
    VDLog(
      kVDLogInfo,
      VDswprintf(L"Copyright (C) Avery Lee 1998-2009. Licensed under GNU General Public License\n", 1, &version_num));
  }
  ...
  if (portableAltFile || cmdLine.FindAndRemoveSwitch(L"portable") || VDDoesPathExist(portableRegPath.c_str()))
  {
    ...
    if (!resetAll && VDDoesPathExist(portableRegPath.c_str()))
    {
      ...
      catch (const MyError &err)
      {
        ...
        message.sprintf(
          "There was an error loading the settings file:\n\n%s\n\nDo you want to continue? If so, settings will be "
          "reset to defaults and may not be saved.",
          err.c_str());
  ...
  if (pluginsFailed)
    guiSetStatus(
      "Autoloaded %d filter(s) (%d failed). Check the log for details.", 255, pluginsSucceeded, pluginsFailed);
...
bool InitApplication(HINSTANCE hInstance)
{
  ...
  if (!RegisterClippingControl())
    return false;
  if (!RegisterPositionControl())
    return false;
  if (!RegisterLevelControl())
    return false;
  if (!RegisterHexEditor())
    return false;

  ATOM RegisterAudioDisplayControl();
  if (!RegisterAudioDisplayControl())
    return false;

  if (!VDRegisterVideoDisplayControl())
    return false;
  if (!RegisterFilterGraphControl())
    return false;
  if (!RegisterLogWindowControl())
    return false;
  if (!RegisterRTProfileDisplayControl())
    return false;
  if (!VDRegisterRTProfileDisplayControl2())
    return false;
  if (!RegisterVideoWindow())
    return false;

  extern bool VDRegisterUIFrameWindow();
  if (!VDRegisterUIFrameWindow())
    return false;

  extern bool VDRegisterParameterCurveControl();
  if (!VDRegisterParameterCurveControl())
    return false;

  extern bool VDUIRegisterHotKeyExControl();
  if (!VDUIRegisterHotKeyExControl())
    return false;
...
int ProcessCommandLine::showHelp()
{
  const char *msg =
    ...
    "  /capchannel <ch> [<freq>] Set capture channel (opt. frequency in MHz), use antenna:<n> or cable:<n> to force "
    "mode\n"
  ...
  try
  {
    ...
    while (cmdLine.GetNextArgument(it, token, isSwitch))
    {
      ...
      else
      {
        ...
        else if (!wcscmp(token, L"cmd"))
        {
          ...
          token2 += L';';
          if (execute)
            RunScriptMemory((char *)VDTextWToA(token2).c_str(), -1, false, true);
        }
        ...
        else if (!wcscmp(token, L"edit"))
        {
          ...
          if (execute && g_projectui)
            g_projectui->edit_token = token;
        }
        ...
        else if (!wcscmp(token, L"ExportViaEncoderSet"))
        {
          ...
          if (
            !cmdLine.GetNextNonSwitchArgument(it, token) || !cmdLine.GetNextNonSwitchArgument(it, path2) ||
            !cmdLine.GetNextNonSwitchArgument(it, set))
          ...
          if (execute)
          {
            ...
            if (req.fileInput == L"*")
            {
              ...
              {
                throw MyError(
                  "Command line error: \"/ExportViaEncoderSet * <dst_file> <set_name>\" requires source video");
              }
              SetProject(req, g_project);
            }
    ...
    if (!argsFound && g_consoleMode)
      throw MyError(
        ...
        "the program interactively, launch " VD_PROGRAM_EXEFILE_NAMEA
        " directly.\n"
        "\n"
        "Usage: " VD_PROGRAM_CLIEXE_NAMEA
        " ( /<switches> | video-file ) ...\n"
