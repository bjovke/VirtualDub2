...
VDRenderOutputBuffer::VDRenderOutputBuffer(VDRenderOutputBufferTracker *tracker) : mpTracker(tracker)
{
  mTimelineFrame = -1;
}
...
VDThreadedVideoCompressor::VDThreadedVideoCompressor()
  : mpThreads(NULL), mThreadCount(0), mbClientFlushInProgress(false), mBarrier(0), mFrameSkipCounter(0),
    mpAllocator(new VDRenderPostCompressionBufferAllocator), mFramesSubmitted(0), mFramesProcessed(0),
    mFramesBufferedInFlush(0), mbFlushInProgress(false), mbLoopDetectedDuringFlush(false), mInputBufferCount(0),
...
bool VDThreadedVideoCompressor::ExchangeBuffer(
  VDRenderOutputBuffer *          buffer,
  VDRenderPostCompressionBuffer **ppOutBuffer)
{
  bool success = false;
...
bool VDThreadedVideoCompressor::ProcessFrame(
  ...
  FrameTrackingQueue *  frameTrackingQueue)
{
  ...
  try
  {
    ...
    valid = pCompressor->CompressFrame(
      pOutputBuffer->mOutputBuffer.data(), pBuffer->mpBase, packedSize, pOutputBuffer->packetInfo);
    ...
    if (!pOutputBuffer->packetInfo.keyframe && mThreadCount > 1 && packedSize > 0)
      throw MyError(
        "The video compressor was unable to produce a key frame only sequence. Multithreaded compression must be "
        "disabled for this video codec and current compression settings.");
...
VDThreadedVideoCompressorSlave::VDThreadedVideoCompressorSlave() : VDThread("Video compressor") {}

VDThreadedVideoCompressorSlave::~VDThreadedVideoCompressorSlave() {}

void VDThreadedVideoCompressorSlave::Init(
  VDThreadedVideoCompressor *parent,
  IVDVideoCompressor *       compressor /*, VDPixmapBuffer* buffer*/)
{
  mpParent     = parent;
