...
VDFilterFrameClientRequest::VDFilterFrameClientRequest() : mpRequest(NULL), mpClient(NULL), mpAllocator(NULL)
{
...
VDFilterFrameRequest::VDFilterFrameRequest()
  : mbComplete(false), mbSuccessful(false), mbCacheable(true), mbStealable(true), mBatchNumber(0), mpError(NULL),
    mpResultBuffer(NULL), mpExtraData(NULL)
{}
...
void VDFilterFrameRequestAllocator::ValidateState()
{
  ...
  for (ClientRequests::const_iterator it(mIdleClientRequests.begin()), itEnd(mIdleClientRequests.end()); it != itEnd;
       ++it)
  {
