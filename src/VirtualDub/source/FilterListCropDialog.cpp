...
VDFilterClippingDialog::VDFilterClippingDialog(
  FilterInstance *   pFiltInst,
  VDFilterChainDesc *pFilterChainDesc,
  sint64             initialTimeUS)
  : VDDialogFrameW32(IDD_FILTER_CLIPPING), mpFilterChainDesc(pFilterChainDesc), mpFilterInst(pFiltInst), mMinSizeW(0),
    mMinSizeH(0), mInitialTimeUS(initialTimeUS)
{}
  ...
  if (mpFilterChainDesc && inputVideo)
  {
    ...
    try
    {
      ...
      mFilterSys.prepareLinearChain(
        mpFilterChainDesc, px.w, px.h, px, pVSS->getRate(), pVSS->getLength(), inputVideo->getPixelAspectRatio());
