...
void VDFilterAccelInterleaveYUV_SSE2(
  ...
  ptrdiff_t   srcYPitch,
  const void *srcCb,
  ptrdiff_t   srcCbPitch,
  const void *srcCr,
  ptrdiff_t   srcCrPitch,
  uint32      w,
  uint32      h);

VDFilterAccelEngineMessage::VDFilterAccelEngineMessage()
  : mpCleanup(NULL), mbRePoll(false), mbCompleted(false), mpCompleteSignal(NULL)
{}

VDFilterAccelEngineDispatchQueue::VDFilterAccelEngineDispatchQueue() : mbActive(true), mpProfChan(NULL) {}
...
VDFilterAccelEngine::VDFilterAccelEngine()
  : VDThread("3D accel worker"), mpTC(NULL), mpTP(NULL), mpFPConvertRGBToYUV(NULL), mpFPConvertYUVToRGB(NULL),
    mpFPNull(NULL), mpFPExtractPlane(NULL), mpFPClear(NULL), mpVP(NULL), mpVPClear(NULL), mpVF(NULL), mpVFC(NULL),
    mpVB(NULL), mpIB(NULL), mpRS(NULL), mpBS(NULL)
{
...
bool VDFilterAccelEngine::InitCallback2(bool visibleDebugWindow)
{
  ...
  if (visibleDebugWindow)
  {
    ...
    mhwnd = CreateWindow(
      MAKEINTATOM(mWndClass),
      "VirtualDub 3D filter acceleration debug window",
      dwStyle,
      CW_USEDEFAULT,
      CW_USEDEFAULT,
      r.right - r.left,
  ...
  static const uint16 kIndices[] = {
    ...
    0,    1,    3,    3,    1,    4,    1,    2,    4,    4,    2,    5,    3,    4,    6,    6,    4,    7,
    4,    5,    7,    7,    5,    8,

    0x00, 0x01, 0x02, 0x02, 0x01, 0x03, 0x04, 0x05, 0x06, 0x06, 0x05, 0x07, 0x08, 0x09, 0x0A, 0x0A, 0x09, 0x0B,
    0x0C, 0x0D, 0x0E, 0x0E, 0x0D, 0x0F, 0x10, 0x11, 0x12, 0x12, 0x11, 0x13, 0x14, 0x15, 0x16, 0x16, 0x15, 0x17,
    0x18, 0x19, 0x1A, 0x1A, 0x19, 0x1B, 0x1C, 0x1D, 0x1E, 0x1E, 0x1D, 0x1F, 0x20, 0x21, 0x22, 0x22, 0x21, 0x23,
    0x24, 0x25, 0x26, 0x26, 0x25, 0x27, 0x28, 0x29, 0x2A, 0x2A, 0x29, 0x2B, 0x2C, 0x2D, 0x2E, 0x2E, 0x2D, 0x2F,
    0x30, 0x31, 0x32, 0x32, 0x31, 0x33, 0x34, 0x35, 0x36, 0x36, 0x35, 0x37, 0x38, 0x39, 0x3A, 0x3A, 0x39, 0x3B,
    0x3C, 0x3D, 0x3E, 0x3E, 0x3D, 0x3F,
  };
  ...
  static const VDTFilterMode kFilterModes[3] = {kVDTFilt_Point, kVDTFilt_Bilinear, kVDTFilt_BilinearMip};
...
void VDFilterAccelEngine::ShutdownCallback2()
{
  ...
  vdsaferelease <<= mpFPConvertYUVToRGB, mpFPConvertRGBToYUV, mpFPNull, mpFPExtractPlane, mpFPClear, mpIB, mpVB, mpVFC,
    mpVF, mpBS, mpRS, mpVP, mpVPClear;
...
void VDFilterAccelEngine::Upload(
  VDFilterFrameBufferAccel *dst,
  VDFilterFrameBuffer *     src,
        ...
        if (msg.mbYUV)
        {
          ...
          if (SSE2_enabled)
          {
            VDFilterAccelInterleaveYUV_SSE2(
              dst, lockData.mPitch, srcY, msg.mPitch, srcCb, msg.mPitch2, srcCr, msg.mPitch3, w, h);
          }
          else
          {
...
bool VDFilterAccelEngine::BeginDownload(
...
VDFilterAccelStatus VDFilterAccelEngine::EndDownload(VDFilterAccelEngineDownloadMsg *msg)
{
  ...
  return msg->mbSuccess ? kVDFilterAccelStatus_OK :
                          msg->mbDeviceLost ? kVDFilterAccelStatus_DeviceLost : kVDFilterAccelStatus_Failed;
}

bool VDFilterAccelEngine::Download(
  VDFilterFrameBuffer *        dst,
  const VDPixmapLayout &       dstLayout,
  VDFilterFrameBufferAccel *   src,
  bool                         srcYUV,
...
void VDFilterAccelEngine::DownloadCallback2a(DownloadMsg &msg)
{
  ...
  else
  {
    ...
    {
      static const float kExtractFPConstants[2][3][8] = {// RGB -> YUV
      ...
      const float us[2] = {0.0f, (float)w * invTexWidth};

      const float vs[2] = {0.0f, (float)h * invTexHeight};
...
void VDFilterAccelEngine::DownloadCallback2b(DownloadMsg &msg)
{
  ...
  else
  {
    ...
    VDMemcpyRect(
      msg.mpDst[1], msg.mDstPitch[1], (const char *)lockData.mpData + lockData.mPitch * h, lockData.mPitch, w, h);
    VDMemcpyRect(
      msg.mpDst[2], msg.mDstPitch[2], (const char *)lockData.mpData + lockData.mPitch * (h * 2), lockData.mPitch, w, h);
  }
...
bool VDFilterAccelEngine::Convert(
  VDFilterFrameBufferAccel *dst,
  const VDPixmapLayout &    dstLayout,
  VDFilterFrameBufferAccel *src,
  const VDPixmapLayout &    srcLayout,
...
void VDFilterAccelEngine::ConvertCallback2(ConvertMsg &msg)
{
  ...
  const float xs[4] = {0, 0.5f, (float)imgw - 0.5f, (float)(imgw + borw)};

  const float ys[4] = {0, 0.5f, (float)imgh - 0.5f, (float)(imgh + borh)};
...
bool VDFilterAccelEngine::DrawQuads(
  IVDTVertexFormat *   vf,
...
bool VDFilterAccelEngine::FillRects(
  uint32         rectCount,
  const VDXRect *rects,
  uint32         colorARGB,
  uint32         w,
  ...
  uint32         bh)
{
  ...
  const uint8 c[4] = {
    (uint8)(colorARGB >> 16), (uint8)(colorARGB >> 8), (uint8)(colorARGB >> 0), (uint8)(colorARGB >> 24)};
