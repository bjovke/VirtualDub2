...
public:
  ...
  void ThreadRun()
  {
    ...
    while (close == 0)
    {
      int vd, sys;
      reader.read(vd, sys);
      if (g_pVDEventProfiler && sys != -1)
        g_pVDEventProfiler->InsertSample(IVDEventProfiler::sample_cpu, sys);
      Sleep(20);
    }
  }
};
      ...
      if (hRSRC = FindResource(NULL, (LPSTR)lParam, "STUFF"))
      {
        ...
        if (hGlobal = LoadResource(NULL, hRSRC))
        {
          ...
          if (lpData = LockResource(hGlobal))
          {
            ...
            s += 2;
            SendMessage(
              GetDlgItem(hDlg, IDC_CHANGES), WM_SETFONT, (WPARAM)GetStockObject(ANSI_FIXED_FONT), MAKELPARAM(TRUE, 0));
...
static bool DetectDriver(const char *pszName)
{
  ...
  {
    if (
      WideCharToMultiByte(CP_ACP, 0, info.szDriver, -1, szDriverANSI, sizeof szDriverANSI, NULL, NULL) &&
...
void VDShowChangeLog(VDGUIHandle hParent)
{
  ...
  DialogBoxParam(
    g_hInst,
    MAKEINTRESOURCE(IDD_CHANGE_LOG),
    (HWND)hParent,
    VDShowChangeLogDlgProcW32,
    (LPARAM)MAKEINTRESOURCE(IDR_CHANGES));
  FreeLibrary(hmod);
}
...
void VDShowReleaseNotes(VDGUIHandle hParent)
{
  ...
  DialogBoxParam(
    ...
    (HWND)hParent,
    VDShowChangeLogDlgProcW32,
    (LPARAM)MAKEINTRESOURCE(IDR_RELEASE_NOTES));
  FreeLibrary(hmod);
}
