...
#define VD_FOURCC(fcc) \
  (((fcc & 0xff000000) >> 24) + ((fcc & 0xff0000) >> 8) + ((fcc & 0xff00) << 8) + ((fcc & 0xff) << 24))
...
public:
  ...
  inline bool Activate(VDGUIHandle hParent)
  {
...
VDDialogAudioConversionW32::VDDialogAudioConversionW32(DubOptions &opts, AudioSource *pSource)
  : VDDialogBaseW32(IDD_AUDIO_CONVERSION), mOpts(opts), mpSource(pSource), mbSource16Bit(true), mbSourceFloat(false),
    mbSourcePrecisionKnown(false), sourceChannels(0)
{
  if (pSource)
  {
...
void VDDialogAudioConversionW32::RecomputeBandwidth()
{
  ...
  if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_NOCHANGE))
    bps = mpSource ? mpSource->getWaveFormat()->mSamplingRate : 0;
  else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_11KHZ))
    bps = 11025;
  else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_22KHZ))
    bps = 22050;
  else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_44KHZ))
    bps = 44100;
  else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_8KHZ))
    bps = 8000;
  else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_16KHZ))
    bps = 16000;
  else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_48KHZ))
    bps = 48000;
  ...
  switch (message)
  {
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam))
      {
        ...
        case IDOK:
          if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_NOCHANGE))
            mOpts.audio.new_rate = 0;
          else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_8KHZ))
            mOpts.audio.new_rate = 8000;
          else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_11KHZ))
            mOpts.audio.new_rate = 11025;
          else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_16KHZ))
            mOpts.audio.new_rate = 16000;
          else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_22KHZ))
            mOpts.audio.new_rate = 22050;
          else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_44KHZ))
            mOpts.audio.new_rate = 44100;
          else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_48KHZ))
            mOpts.audio.new_rate = 48000;
          else if (IsDlgButtonChecked(mhdlg, IDC_SAMPLINGRATE_CUSTOM))
          {
            BOOL valid = FALSE;
          ...
          if (IsDlgButtonChecked(mhdlg, IDC_PRECISION_NOCHANGE))
            mOpts.audio.newPrecision = DubAudioOptions::P_NOCHANGE;
          else if (IsDlgButtonChecked(mhdlg, IDC_PRECISION_8BIT))
            mOpts.audio.newPrecision = DubAudioOptions::P_8BIT;
          else if (IsDlgButtonChecked(mhdlg, IDC_PRECISION_16BIT))
            mOpts.audio.newPrecision = DubAudioOptions::P_16BIT;

          if (IsDlgButtonChecked(mhdlg, IDC_CHANNELS_NOCHANGE))
            mOpts.audio.newChannels = DubAudioOptions::C_NOCHANGE;
          else if (IsDlgButtonChecked(mhdlg, IDC_CHANNELS_MONO))
            mOpts.audio.newChannels = DubAudioOptions::C_MONO;
          else if (IsDlgButtonChecked(mhdlg, IDC_CHANNELS_STEREO))
            mOpts.audio.newChannels = DubAudioOptions::C_STEREO;
          else if (IsDlgButtonChecked(mhdlg, IDC_CHANNELS_LEFT))
            mOpts.audio.newChannels = DubAudioOptions::C_MONOLEFT;
          else if (IsDlgButtonChecked(mhdlg, IDC_CHANNELS_RIGHT))
            mOpts.audio.newChannels = DubAudioOptions::C_MONORIGHT;
...
void VDDialogAudioConversionW32::ReinitDialog()
{
  ...
  {
    case 0:
      CheckDlgButton(mhdlg, IDC_SAMPLINGRATE_NOCHANGE, TRUE);
      break;
    case 8000:
      CheckDlgButton(mhdlg, IDC_SAMPLINGRATE_8KHZ, TRUE);
      break;
    case 11025:
      CheckDlgButton(mhdlg, IDC_SAMPLINGRATE_11KHZ, TRUE);
      break;
    case 16000:
      CheckDlgButton(mhdlg, IDC_SAMPLINGRATE_16KHZ, TRUE);
      break;
    case 22050:
      CheckDlgButton(mhdlg, IDC_SAMPLINGRATE_22KHZ, TRUE);
      break;
    case 44100:
      CheckDlgButton(mhdlg, IDC_SAMPLINGRATE_44KHZ, TRUE);
      break;
    case 48000:
      CheckDlgButton(mhdlg, IDC_SAMPLINGRATE_48KHZ, TRUE);
      break;
    default:
...
VDDialogSelectVideoFormatW32::VDDialogSelectVideoFormatW32(int format)
  : VDDialogFrameW32(IDD_SELECT_VIDEO_FORMAT), mFormat(format)
{
...
void VDDialogSelectVideoFormatW32::RebuildList()
{
  ...
  if (sort.col != -1)
    std::sort(mFormatItems.begin(), mFormatItems.end(), sort);
...
public:
  ...
  inline VDDialogVideoDepthW32(VDPixmapFormatEx &opts, int type, int lockFormat, COMPVARS2 *compVars)
    : VDDialogFrameW32(TemplateFromType(type)), mOpts(opts), mbInputBrowsePending(false), mType(type),
      mLockFormat(lockFormat), compVars(compVars)
  {
    didChanges = false;
  }
  ...
  void setCapture(BITMAPINFOHEADER *bm)
  {
    capSrc = bm;
  }
...
protected:
  ...
  {
    if (type == DepthDialog_input)
      return IDD_VIDEO_DECFORMAT;
  ...
  switch (message)
  {
    ...
    case WM_USER + 200:
      if (mbInputBrowsePending)
      {
        ...
        if (VDPixmapFormatCombine(mInputFormat) == vd2::kPixFormat_Y8_FR)
          fmt = vd2::kPixFormat_Y8_FR;
  ...
  if (extcode == BN_CLICKED)
  {
    ...
    if (mType != DepthDialog_input)
    {
      ...
      if (ref_changed)
      {
        ...
        {
          if (mInputFormat.format == 0)
            mInputFormat.format = nsVDPixmap::kPixFormat_RGB888;
          if (mInputFormat.colorRangeMode == 0)
            mInputFormat.colorRangeMode = vd2::kColorRangeMode_Limited;
          if (mInputFormat.colorSpaceMode == 0)
            mInputFormat.colorSpaceMode = vd2::kColorSpaceMode_601;
        }
        ApplyChanges();
        SyncControls();
...
void VDDialogVideoDepthW32::OnDataExchange(bool write)
{
  if (write)
  {
    mOpts = mInputFormat;
    if (enableReference)
      g_dubOpts.video.outputReference = outputReference;
  }
  else
  {
    outputReference = g_dubOpts.video.outputReference;
    enableReference = mType == DepthDialog_output;
    if (g_dubOpts.video.mode < DubVideoOptions::M_FULL)
...
void VDDialogVideoDepthW32::InitFinalFormat()
{
  ...
  if (mType == DepthDialog_output)
  {
    ...
    if (mLockFormat != -1)
      make.option.format = mLockFormat;
...
void VDDialogVideoDepthW32::SyncInputColor()
{
  ...
  if (mType == DepthDialog_output)
  {
    ...
    if (outputReference == 0)
      enableDefault = false;
    if (outputReference == 1)
      enableDefault = VDPixmapFormatMatrixType(make.dec) != 0;
    if (outputReference == 2)
      enableDefault = VDPixmapFormatMatrixType(make.flt) != 0;
    bool use_ref = false;
    if (mInputFormat.format == 0 && mLockFormat == -1)
      use_ref = true;
    if (mInputFormat.colorRangeMode == 0 && enable)
      use_ref = true;
    if (mInputFormat.colorSpaceMode == 0 && enable)
      use_ref = true;
  ...
  {
    VDPixmapFormatEx format = mInputFormat;
    if (!enableDefault && !format.colorSpaceMode)
      format.colorSpaceMode = vd2::kColorSpaceMode_601;
    if (!enableDefault && !format.colorRangeMode)
      format.colorRangeMode = vd2::kColorRangeMode_Limited;
...
bool VDDisplayVideoDepthDialog(
  VDGUIHandle       hParent,
  VDPixmapFormatEx &opts,
  int               type,
  int               lockFormat,
  COMPVARS2 *       compVars,
  BITMAPINFOHEADER *capSrc)
...
static const long waveBufferSizeArray[] = {
  8 * 1024,    12 * 1024,   16 * 1024,   24 * 1024,       32 * 1024,       48 * 1024,       64 * 1024,
  96 * 1024,   128 * 1024,  192 * 1024,  256 * 1024,      384 * 1024,      512 * 1024,      768 * 1024,
  1024 * 1024, 1536 * 1024, 2048 * 1024, 3 * 1024 * 1024, 4 * 1024 * 1024, 6 * 1024 * 1024, 8 * 1024 * 1024};
...
static const long audioBufferSizeArray[] = {1, 2, 3, 4, 6, 8, 10, 12, 16, 20, 24, 32, 48, 64};
...
void VDDialogPerformanceOptions::OnDataExchange(bool write)
{
  ...
  else
  {
    ...
    TBSetValue(
      IDC_OUTPUT_BUFFER,
      NearestLongValue(
        VDPreferencesGetRenderOutputBufferSize(), outputBufferSizeArray, ELEMENTS(outputBufferSizeArray)));
    ...
    TBSetValue(
      IDC_WAVE_INPUT_BUFFER,
      NearestLongValue(VDPreferencesGetRenderWaveBufferSize(), waveBufferSizeArray, ELEMENTS(waveBufferSizeArray)));
    ...
    TBSetValue(
      IDC_VIDEO_BUFFERS,
      NearestLongValue(VDPreferencesGetRenderVideoBufferCount(), pipeBufferCountArray, ELEMENTS(pipeBufferCountArray)));
    ...
    TBSetValue(
      IDC_AUDIO_BUFFER,
      NearestLongValue(
        VDPreferencesGetRenderAudioBufferSeconds(), audioBufferSizeArray, ELEMENTS(audioBufferSizeArray)));
...
{
public:
  inline VDDialogVideoFrameRateW32(DubOptions &opts, IVDVideoSource *pVS, AudioSource *pAS)
    : VDDialogBaseW32(IDD_VIDEO_FRAMERATE), mOpts(opts), mpVideo(pVS), mpAudio(pAS)
  {}
  ...
  if (mpVideo)
  {
    ...
    if (mpAudio && mpAudio->getLength())
    {
      VDFraction framerate =
        VDFraction((double)pVSS->getLength() * mpAudio->getRate().asDouble() / mpAudio->getLength());
...
VDDialogVideoRangeW32::VDDialogVideoRangeW32(
  DubOptions &      opts,
  const VDFraction &frameRate,
  VDPosition        frameCount,
  VDPosition &      startSel,
  VDPosition &      endSel)
  : VDDialogBaseW32(IDD_VIDEO_CLIPPING), mOpts(opts), mSelStart(startSel), mSelEnd(endSel), mFrameRate(frameRate),
    mFrameCount(frameCount), mTotalTimeMS(0), mbReentry(false)
{
...
void VDDialogVideoRangeW32::MSToFrames(UINT idFrames, UINT idMS)
{
  ...
  SetDlgItemInt(
    ...
    IDC_LENGTH_MS,
    (UINT)(
      mTotalTimeMS - GetDlgItemInt(mhdlg, IDC_END_MS, NULL, FALSE) - GetDlgItemInt(mhdlg, IDC_START_MS, NULL, FALSE)),
    TRUE);
  SetDlgItemInt(
    mhdlg,
    IDC_LENGTH_FRAMES,
    (UINT)(
      mFrameCount - GetDlgItemInt(mhdlg, IDC_END_FRAMES, NULL, FALSE) -
      GetDlgItemInt(mhdlg, IDC_START_FRAMES, NULL, FALSE)),
    TRUE);
  mbReentry = false;
}
...
void VDDialogVideoRangeW32::LengthFrames()
{
  ...
  SetDlgItemInt(mhdlg, IDC_END_MS, (UINT)(mTotalTimeMS - ms - GetDlgItemInt(mhdlg, IDC_START_MS, NULL, TRUE)), TRUE);
  SetDlgItemInt(
    mhdlg, IDC_END_FRAMES, (UINT)(mFrameCount - frames - GetDlgItemInt(mhdlg, IDC_START_FRAMES, NULL, TRUE)), TRUE);
  mbReentry = false;
}
  ...
  switch (message)
  {
    case WM_INITDIALOG:
      SetDlgItemInt(
        mhdlg,
        IDC_START_MS,
        VDClampToUint32(mOpts.video.mSelectionStart.ResolveToMS(mFrameCount, mFrameRate, false)),
        FALSE);
      SetDlgItemInt(
        mhdlg,
        IDC_END_MS,
        VDClampToUint32(mOpts.video.mSelectionEnd.ResolveToMS(mFrameCount, mFrameRate, true)),
        FALSE);
...
bool VDDisplayVideoRangeDialog(
  VDGUIHandle       hParent,
  DubOptions &      opts,
  const VDFraction &frameRate,
  VDPosition        frameCount,
...
{
public:
  inline VDDialogJumpToPositionW32(
    ...
    : VDDialogBaseW32(useSelect ? IDD_JUMPTOFRAME1 : (usePos ? IDD_JUMPTOFRAME2 : IDD_JUMPTOFRAME)),
      mFrame(currentFrame), mpVideo(pVS), mRealRate(realRate)
  {
    mbUsePos = usePos;
    title    = 0;
    cmd_jump = false;
  ...
  VDPosition Activate(VDGUIHandle hParent)
  {
    return ActivateDialog(hParent) ? mFrame : -1;
  }
...
void VDDialogJumpToPositionW32::ReinitDialog()
{
  ...
  if (mbUsePos)
  {
    ...
    SetWindowPos(
      mhdlg, 0, 0, 0, r2.right - r2.left, r2.bottom - r2.top + pos_h + r1.top - r0.bottom, SWP_NOMOVE | SWP_NOZORDER);
  }
  else
  {
    mpPosition = 0;
...
VDPosition VDDisplayJumpToPositionDialog(
  VDGUIHandle       hParent,
  VDPosition        currentFrame,
  IVDVideoSource *  pVS,
  const VDFraction &realRate)
...
VDPosition VDDisplayJumpToPositionDialog2(
  VDGUIHandle       hParent,
  VDPosition        currentFrame,
  IVDVideoSource *  pVS,
  const VDFraction &realRate)
...
{
public:
  inline VDDialogErrorModeW32(const char *pszSettingsKey, IVDStreamSource *pSource)
    : VDDialogBaseW32(IDD_ERRORMODE), mpszSettingsKey(pszSettingsKey), mpSource(pSource)
  {}
...
void VDDialogErrorModeW32::ReinitDialog()
{
  ...
  EnableWindow(
    GetDlgItem(mhdlg, IDC_ERROR_CONCEAL),
    !mpSource || mpSource->isDecodeErrorModeSupported(DubSource::kErrorModeConceal));
  EnableWindow(
    GetDlgItem(mhdlg, IDC_ERROR_DECODE),
    !mpSource || mpSource->isDecodeErrorModeSupported(DubSource::kErrorModeDecodeAnyway));
...
DubSource::ErrorMode VDDisplayErrorModeDialog(
  VDGUIHandle                hParent,
  IVDStreamSource::ErrorMode oldMode,
  const char *               pszSettingsKey,
  IVDStreamSource *          pSource)
{
...
const struct VDDialogFileTextInfoW32::FieldEntry VDDialogFileTextInfoW32::kFields[] = {
  {VD_FOURCC('ISBJ'), "Subject"},         {VD_FOURCC('IART'), "Artist (Author)"},
  {VD_FOURCC('ICOP'), "Copyright"},       {VD_FOURCC('IARL'), "Archival Location"},
  {VD_FOURCC('ICMS'), "Commissioned"},    {VD_FOURCC('ICMT'), "Comments"},
  {VD_FOURCC('ICRD'), "Creation Date"},   {VD_FOURCC('ICRP'), "Cropped"},
  {VD_FOURCC('IDIM'), "Dimensions"},      {VD_FOURCC('IDPI'), "Dots Per Inch"},
  {VD_FOURCC('IENG'), "Engineer"},        {VD_FOURCC('IGNR'), "Genre"},
  {VD_FOURCC('IKEY'), "Keywords"},        {VD_FOURCC('ILGT'), "Lightness"},
  {VD_FOURCC('IMED'), "Medium"},          {VD_FOURCC('INAM'), "Name"},
  {VD_FOURCC('IPLT'), "Palette Setting"}, {VD_FOURCC('IPRD'), "Product"},
  {VD_FOURCC('ISFT'), "Software"},        {VD_FOURCC('ISHP'), "Sharpness"},
  {VD_FOURCC('ISRC'), "Source"},          {VD_FOURCC('ISRF'), "Source Form"},
  {VD_FOURCC('ITCH'), "Technician"},
};

VDDialogFileTextInfoW32::VDDialogFileTextInfoW32(tRawTextInfo &info)
  : VDDialogBaseW32(IDD_FILE_SETTEXTINFO), mhwndEdit(NULL), mTextInfoOrig(info)
{}
...
void VDDialogFileTextInfoW32::BeginEdit(int index)
{
  ...
  mhwndEdit = CreateWindowW(
    L"EDIT", NULL, dwEditStyle, r.left, r.top, r.right - r.left, r.bottom - r.top, mhwndList, (HMENU)1, g_hInst, NULL);
...
LRESULT VDDialogFileTextInfoW32::LVWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    ...
    case WM_KEYDOWN:
      ...
      {
        int index =
          VDDualCallWindowProcW32(mOldLVProc, hwnd, LVM_GETNEXTITEM, -1, MAKELPARAM(LVNI_ALL | LVNI_SELECTED, 0));
