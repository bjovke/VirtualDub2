...
void CreateSparseAVI(const char *pszIn, const char *pszOut)
{
  try
  {
    ...
    for (;;)
    {
      ...
      if (
        !infile.readData(&fcc, 4) || !infile.readData(&dwLen, 4) || !isprint((unsigned char)(fcc >> 24)) ||
        !isprint((unsigned char)(fcc >> 16)) || !isprint((unsigned char)(fcc >> 8)) || !isprint((unsigned char)(fcc)) ||
        pos + dwLen + 8 > insize)
      {
        if (ckidcount)
        {
...
void ExpandSparseAVI(HWND hwndParent, const char *pszIn, const char *pszOut)
{
  try
  {
...

    {
      ProgressDialog pd(
        NULL,
        "Expanding sparse AVI",
        "Writing output file",
        (long)((spah.error_point + spah.error_bytes + 1023) >> 10),
        true);
      pd.setValueFormat("%ldK of %ldK");
...
void CreateExtractSparseAVI(HWND hwndParent, bool bExtract)
{
  ...
  const VDStringW infile(VDGetLoadFileName(
    VDFSPECKEY_LOADVIDEOFILE,
    (VDGUIHandle)hwndParent,
    bExtract ? L"Select sparse AVI file" : L"Select source AVI file",
    bExtract ? sparsefilter : avifilter,
    bExtract ? L"sparse" : L"avi"));

  if (!infile.empty())
  {
    const VDStringW outfile(VDGetSaveFileName(
      VDFSPECKEY_LOADVIDEOFILE,
      (VDGUIHandle)hwndParent,
      L"Select filename for output",
      bExtract ? avifilter : sparsefilter,
      bExtract ? L"avi" : L"sparse"));
    ...
    if (!outfile.empty())
    {
      ...
      try
      {
        ...
        MessageBox(
          hwndParent,
          bExtract ? "Sparse AVI expansion complete." : "Sparse AVI creation complete.",
          "VirtualDub notice",
          MB_ICONINFORMATION);
      }
