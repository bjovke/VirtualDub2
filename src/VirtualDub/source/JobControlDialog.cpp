...
VDUIJobErrorDialog::VDUIJobErrorDialog(const VDJob &job) : VDDialogFrameW32(IDD_JOBERROR), mJob(job) {}
...
VDUIJobLogDialog::VDUIJobLogDialog(const VDJob::tLogEntries &logents) : VDDialogFrameW32(IDD_JOBLOG), mLogEnts(logents)
{}
...
VDUIJobControlDialog::VDUIJobControlDialog() : VDDialogFrameW32(IDD_JOBCONTROL) {}
    ...
    switch (id)
    {
          ...
          if (vdj->GetState() != VDJob::kStateInProgress)
          {
            ...
            if (g_VDJobQueue.ListSize() > 0)
              ListView_SetItemState(
                hwndItem, index == g_VDJobQueue.ListSize() ? index - 1 : index, LVIS_SELECTED, LVIS_SELECTED);
          }
        }
      ...
      case IDC_RELOAD:
        ...
        if (!vdj->IsReloadMarkerPresent())
          MessageBox(
            mhdlg,
            "This job was created with an older version of VirtualDub and cannot be reloaded.",
            g_szError,
            MB_ICONERROR | MB_OK);
        else
          vdj->Reload();
...
bool VDUIJobControlDialog::OnMenuHit(uint32 id)
{
  static const wchar_t fileFilters[] =
    L"VirtualDub job list (*.jobs)\0"
    L"*.jobs\0"
    L"VirtualDub script (*.syl, *.vdscript)\0"
    L"*.syl;*.vdscript\0"
    L"All files (*.*)\0"
    L"*.*\0";
  ...
  try
  {
    switch (id)
    {
      case ID_FILE_LOADJOBLIST: {
        VDStringW filename(
          VDGetLoadFileName(kFileDialog_JobList, (VDGUIHandle)mhdlg, L"Load job list", fileFilters, NULL));
      ...
      case ID_FILE_SAVEJOBLIST: {
        VDStringW filename(
          VDGetSaveFileName(kFileDialog_JobList, (VDGUIHandle)mhdlg, L"Save job list", fileFilters, NULL));
      ...
      case ID_FILE_USEREMOTEJOBQUEUE:
        ...
        {
          const VDFileDialogOption opts[] = {{VDFileDialogOption::kConfirmFile, 0, NULL, 0, 0}, {0}};

          int optvals[] = {false};

          VDStringW filename(VDGetSaveFileName(
            kFileDialog_JobList, (VDGUIHandle)mhdlg, L"Use shared job list", fileFilters, NULL, opts, optvals));
      ...
      case ID_EDIT_CLEARLIST:
        if (
          IDOK !=
          MessageBox(mhdlg, "Really clear job list?", "VirtualDub job system", MB_OKCANCEL | MB_ICONEXCLAMATION))
          break;
              ...
              switch (vdj->GetState())
              {
                ...
                case VDJob::kStateAborting:
                  if (!vdj->IsLocal())
                  {
                    ...
                    msg.sprintf(
                      "This job may be running on a different instance of VirtualDub on the machine named %hs. Are you "
                      "sure you want to reset it to Waiting status?",
                      vdj->GetRunnerName());
                ...
                default:
                  SendMessage(
                    mhdlg, WM_COMMAND, MAKELONG(IDC_POSTPONE, BN_CLICKED), (LPARAM)GetDlgItem(mhdlg, IDC_POSTPONE));
              }
            }
...
void VDUIJobControlDialog::GetJobListDispInfoW(NMLVDISPINFOW *nldi)
{
  ...
  switch (nldi->item.iSubItem)
  {
...

    {
      ...
      if (!*ft)
        nldi->item.pszText = L"-";
      else if (ct.wYear != st.wYear || ct.wMonth != st.wMonth || ct.wDay != st.wDay)
      {
        swprintf(
          nldi->item.pszText,
          nldi->item.cchTextMax,
          L"%s %d %d:%02d%c",
          dow[st.wDayOfWeek],
          st.wDay,
    ...
    case 5: // status
      switch (vdj->GetState())
      {
        ...
        case VDJob::kStateInProgress:
          if (vdj->mRunnerName.empty() || vdj->IsLocal())
            ...
            swprintf(
              ...
              nldi->item.cchTextMax,
              L"In progress (%hs:%d)",
              vdj->mRunnerName.c_str(),
              (uint32)vdj->GetRunnerId());
          break;
        ...
        case VDJob::kStateCompleted:
          if (vdj->mRunnerName.empty() || vdj->IsLocal())
            swprintf(
              nldi->item.pszText, nldi->item.cchTextMax, L"Done%hs", vdj->mLogEntries.empty() ? "" : " (warnings)");
          else
            swprintf(
              nldi->item.pszText,
              nldi->item.cchTextMax,
              L"Done%hs (%hs:%d)",
              vdj->mLogEntries.empty() ? "" : " (warnings)",
              vdj->mRunnerName.c_str(),
              (uint32)vdj->GetRunnerId());
          break;
...
void VDUIJobControlDialog::OnJobQueueStatusChanged(VDJobQueueStatus status)
{
  ...
  if (g_VDJobQueue.IsAutoUpdateEnabled())
    title.append_sprintf(
      L" [%s] (%hs:%d)",
      g_VDJobQueue.GetJobFilePath(),
      g_VDJobQueue.GetRunnerName(),
      (uint32)g_VDJobQueue.GetRunnerId());
