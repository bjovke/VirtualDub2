...
bool DecodeBMPHeader(const void *pBuffer, long cbBuffer, int &w, int &h, bool &bHasAlpha)
{
  ...
  do
  {
    ...
    static const uint32 kValidMask = (1 << (1 - 1)) | (1 << (2 - 1)) | (1 << (4 - 1)) | (1 << (8 - 1)) |
                                     (1 << (16 - 1)) | (1 << (24 - 1)) | (1 << (32 - 1));
...
void DecodeBMP(const void *pBuffer, long cbBuffer, const VDPixmap &vb)
{
  ...
  {
    case 1:
      px.format = kPixFormat_Pal1;
      break;
    case 2:
      px.format = kPixFormat_Pal2;
      break;
    case 4:
      px.format = kPixFormat_Pal4;
      break;
    case 8:
      px.format = kPixFormat_Pal8;
      break;
    case 16:
      px.format = kPixFormat_XRGB1555;
      break;
    case 24:
      px.format = kPixFormat_RGB888;
      break;
    case 32:
      px.format = kPixFormat_XRGB8888;
      break;
  }
...
bool VDIsJPEGHeader(const void *pv, uint32 len)
{
  ...
  if (len >= 32 && buf[0] == 0xFF && buf[1] == 0xD8)
  { // x'FF' SOI
...
bool VDIsMayaIFFHeader(const void *pv, uint32 len)
{
  ...
  if (
    buf[0] == 'F' && buf[1] == 'O' && buf[2] == 'R' && buf[3] == '4' && buf[8] == 'C' && buf[9] == 'I' &&
    buf[10] == 'M' && buf[11] == 'G')
  {
    return true;
  }
  ...
  if (bIsJPG)
  {
    ...
    switch (vb.format)
    {
      case nsVDPixmap::kPixFormat_XRGB1555:
        format = IVDJPEGDecoder::kFormatXRGB1555;
        break;
      case nsVDPixmap::kPixFormat_RGB888:
        format = IVDJPEGDecoder::kFormatRGB888;
        break;
      case nsVDPixmap::kPixFormat_XRGB8888:
        format = IVDJPEGDecoder::kFormatXRGB8888;
        break;
    }
