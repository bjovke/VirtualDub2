  ...
  bool HandleUIEvent(IVDUIBase *pBase, IVDUIWindow *pWin, uint32 id, eEventType type, int item)
  {
    switch (type)
    {
      ...
      case kEventDetach:
        ...
        if (GetValue(100))
          mPrefs.mOldPrefs.fDisplay |= Preferences::kDisplayDither16;
        if (!GetValue(101))
          mPrefs.mOldPrefs.fDisplay |= Preferences::kDisplayDisableDX;
        if (GetValue(102))
          mPrefs.mOldPrefs.fDisplay |= Preferences::kDisplayUseDXWithTS;
        if (GetValue(103))
          mPrefs.mOldPrefs.fDisplay |= Preferences::kDisplayEnableD3D;
        if (GetValue(104))
          mPrefs.mOldPrefs.fDisplay |= Preferences::kDisplayEnableOpenGL;
        if (GetValue(105))
          mPrefs.mOldPrefs.fDisplay |= Preferences::kDisplayEnableD3DFX;
        if (GetValue(106))
          mPrefs.mOldPrefs.fDisplay |= Preferences::kDisplayEnableVSync;
        ...
        mPrefs.mDisplaySecondaryMode =
          (VDPreferences2::DisplaySecondaryMode)((VDPreferences2::kDisplaySecondaryModeCount - 1) - GetValue(111));
    ...
    switch (type)
    {
      ...
      case kEventDetach:
        ...
        if (GetValue(100))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_FORCE;
        if (GetValue(200))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_FPU;
        if (GetValue(201))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_MMX;
        if (GetValue(202))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_INTEGER_SSE;
        if (GetValue(203))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_SSE;
        if (GetValue(204))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_SSE2;
        if (GetValue(205))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_3DNOW;
        if (GetValue(206))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_3DNOW_EXT;
        if (GetValue(207))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_SSE3;
        if (GetValue(208))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_SSSE3;
        if (GetValue(209))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_SSE4_1;
        if (GetValue(210))
          mPrefs.mEnabledCPUFeatures |= PreferencesMain::OPTF_AVX;
        if (GetValue(211))
          mPrefs.mEnabledCPUFeatures |= CPUF_SUPPORTS_SSE42;
        if (GetValue(212))
          mPrefs.mEnabledCPUFeatures |= CPUF_SUPPORTS_AVX2;
        if (GetValue(213))
          mPrefs.mEnabledCPUFeatures |= CPUF_SUPPORTS_AVX512F;
    ...
    switch (type)
    {
      case kEventAttach: {
        ...
        if (pTrackbar)
        {
          ...
          pWin->SetValue(
            mPrefs.mOldPrefs.scene.iCutThreshold ? 256 - ((mPrefs.mOldPrefs.scene.iCutThreshold + 8) >> 4) : 0);
        }
        ...
        pBase->ExecuteAllLinks();
        {
          ...
          if (fr2 != mPrefs.mImageSequenceFrameRate)
            sprintf(
              buf,
              "%u/%u (~%.7f)",
              mPrefs.mImageSequenceFrameRate.getHi(),
              mPrefs.mImageSequenceFrameRate.getLo(),
              mPrefs.mImageSequenceFrameRate.asDouble());
      ...
      case kEventAttach:
...

        {
          ...
          if (list)
          {
            ...
            PlaybackDeviceKeys::const_iterator it(
              std::find(mPlaybackDeviceKeys.begin(), mPlaybackDeviceKeys.end(), mPrefs.mAudioPlaybackDeviceKey));
  ...
  bool HandleUIEvent(IVDUIBase *pBase, IVDUIWindow *pWin, uint32 id, eEventType type, int item)
  {
    ...
    else if (id == 101 && type == kEventSelect)
    {
      ...
      if (pSubDialog)
      {
        ...
        {
          case 0:
            pSubDialog->SetCallback(new VDDialogPreferencesGeneral(mPrefs), true);
            break;
          case 1:
            pSubDialog->SetCallback(new VDDialogPreferencesDisplay(mPrefs), true);
            break;
          case 2:
            pSubDialog->SetCallback(new VDDialogPreferencesScene(mPrefs), true);
            break;
          case 3:
            pSubDialog->SetCallback(new VDDialogPreferencesCPU(mPrefs), true);
            break;
          case 4:
            pSubDialog->SetCallback(new VDDialogPreferencesAVI(mPrefs), true);
            break;
          case 5:
            pSubDialog->SetCallback(new VDDialogPreferencesTimeline(mPrefs), true);
            break;
          case 6:
            pSubDialog->SetCallback(new VDDialogPreferencesDub(mPrefs), true);
            break;
          case 7:
            pSubDialog->SetCallback(new VDDialogPreferencesDiskIO(mPrefs), true);
            break;
          case 8:
            pSubDialog->SetCallback(new VDDialogPreferencesImages(mPrefs), true);
            break;
          case 9:
            pSubDialog->SetCallback(new VDDialogPreferencesThreading(mPrefs), true);
            break;
          case 10:
            pSubDialog->SetCallback(new VDDialogPreferencesPlayback(mPrefs), true);
            break;
          // deprecated:	pSubDialog->SetCallback(new VDDialogPreferencesAccel(mPrefs), true); break;
          case 11:
            pSubDialog->SetCallback(new VDDialogPreferencesBatch(mPrefs), true);
            break;
          case 12:
            pSubDialog->SetCallback(new VDDialogPreferencesAutoRecover(mPrefs), true);
            break;
          case 13:
            pSubDialog->SetCallback(new VDDialogPreferencesStartup(mPrefs), true);
            break;
          case 14:
            pSubDialog->SetCallback(new VDDialogPreferencesHistory(mPrefs), true);
            break;
        }
      }
    }
...
void LoadPreferences()
{
  ...
  if (dwSize)
  {
    if (dwSize > sizeof g_prefs)
      dwSize = sizeof g_prefs;
  ...
  bool init_zoom = false;
  {
    ...
    if (key.getInt("Input pane size", 0))
      init_zoom = true;
    if (key.getInt("Output pane size", 0))
      init_zoom = true;
  }
  ...
  g_prefs2.mRenderOutputBufferSize =
    std::max<uint32>(65536, std::min<uint32>(0x10000000, key.getInt("Render: Output buffer size", 2097152)));
  g_prefs2.mRenderWaveBufferSize =
    std::max<uint32>(65536, std::min<uint32>(0x10000000, key.getInt("Render: Wave buffer size", 65536)));
  g_prefs2.mRenderVideoBufferCount =
    std::max<uint32>(1, std::min<uint32>(65536, key.getInt("Render: Video buffer count", 32)));
  g_prefs2.mRenderAudioBufferSeconds =
    std::max<uint32>(1, std::min<uint32>(32, key.getInt("Render: Audio buffer seconds", 2)));
  g_prefs2.mRenderThrottlePercent =
    std::max<uint32>(10, std::min<uint32>(100, key.getInt("Render: Default throttle percent", 100)));
  ...
  g_prefs2.mFileAsyncDefaultMode =
    std::min<uint32>(IVDFileAsync::kModeCount - 1, key.getInt("File: Async mode", IVDFileAsync::kModeAsynchronous));
  ...
  g_prefs2.mDisplaySecondaryMode = (VDPreferences2::DisplaySecondaryMode)key.getEnumInt(
    "Display: Secondary monitor mode", VDPreferences2::kDisplaySecondaryModeCount);
...
{
  if (!g_pShadowRegistry)
    g_pShadowRegistry = new VDRegistryProviderMemory;
  VDSavePreferences();
}
...
void VDPreferencesUpdated()
{
  ...
  VDVideoDisplaySetMonitorSwitchingDXEnabled(
    g_prefs2.mDisplaySecondaryMode == VDPreferences2::kDisplaySecondaryMode_AutoSwitch);
}
