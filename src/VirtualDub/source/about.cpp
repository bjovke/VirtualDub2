...
void RenderTriangle(Pixel16 *dst, long dstpitch, Pixel16 *tex, TriPt *pt1, TriPt *pt2, TriPt *pt3, uint32 s) {
        ...
        while(y < y2) {
                ...
                while(x1 < x2) {
                        ...
                        dst[x1] = (uint16)(((((p&0x007c1f)*s + 0x004010) & 0x0f83e0) + (((p&0x0003e0)*s + 0x000200) &
0x007c00)) >> 5);
      ...
      /*
                                      ...
                                      if (hdc = GetDC(hDlg)) {
                                              if (g_hdcAboutDisplay = CreateCompatibleDC(hdc)) {
                                                      if (g_hbmAboutDisplay = CreateDIBSection(g_hdcAboutDisplay, (const
         BITMAPINFO *)&g_bihAboutDisplay, DIB_RGB_COLORS, &g_pvAboutDisplay, NULL, 0)) { g_hgoAboutDisplay =
         SelectObject(g_hdcAboutDisplay, g_hbmAboutDisplay);

                                                              BitBlt(g_hdcAboutDisplay, 0, 0, r.right, r.bottom, hdc, 0,
         0, SRCCOPY); GdiFlush();

                                                              if (tex = new Pixel16[32*32]) {
                                                                      g_pvAboutDisplayBack =
         malloc(((r.right+3)&~3)*2*r.bottom);

                                                                      if (g_pvAboutDisplayBack) {
                                                                              g_vbAboutSrc.init(g_pvAboutDisplayBack,
         r.right, r.bottom, 16); g_vbAboutDst.init(g_pvAboutDisplay, &g_bihAboutDisplay); g_vbAboutSrc.BitBlt(0, 0,
         &g_vbAboutDst, 0, 0, -1, -1);

                                                                              // Hide all controls.

                                                                              EnumChildWindows(hDlg,
         HideAllButOKCANCELProc, 0);

                                                                              // Grab the VirtualDub icon.

                                                                              HICON hico = LoadIcon(g_hInst,
         MAKEINTRESOURCE(IDI_VIRTUALDUB));

                                                                              RECT rFill = {0,0,32,32};
                                                                              FillRect(g_hdcAboutDisplay, &rFill,
         (HBRUSH)(COLOR_3DFACE+1)); DrawIcon(g_hdcAboutDisplay, 0, 0, hico);

                                                                              GdiFlush();

                                                                              VBitmap(tex, 32, 32, 16).BitBlt(0, 0,
         &g_vbAboutDst, 0, 0, 32, 32);

                                                                              g_vbAboutDst.BitBlt(0, 0, &g_vbAboutSrc,
         0, 0, 32, 32);
...

                                                                              {
                                                                                      ...
                                                                                      if (r.right > r.bottom)
                                                                                              rs = r.bottom /
         (3.6f*3.0f); else rs = r.right / (3.6f*3.0f);

                                                                                      for(i=0; i<8; i++) {
                                                                                              vx[i][0] = i&1 ? -rs :
         +rs; vx[i][1] = i&2 ? -rs : +rs; vx[i][2] = i&4 ? -rs : +rs;
                                                                                      }

                                                                                      cubeside = rs + rs;

                                                                                      rBounce.left = rBounce.top =
         (int)ceil(rs*1.8); rBounce.right = r.right - rBounce.left; rBounce.bottom = r.bottom - rBounce.top;

                                                                                      xpos =
         rand()%(rBounce.right-rBounce.left) + rBounce.left; ypos = rand()%(rBounce.bottom-rBounce.top) + rBounce.top;
                                                                                      xvel = (rand()&2)-1;
                                                                                      yvel = (rand()&2)-1;

                                                                                      rDirtyLast.top = rDirtyLast.left =
         0; rDirtyLast.right = rDirtyLast.bottom = 0;
                                                                              }

                                                                              InvalidateRect(hDlg, NULL, TRUE);

                                                                              if (TIMERR_NOERROR == timeBeginPeriod(10))
         { bTimerSet = true; bRender = true;

                                                                                      mmTimer = timeSetEvent(10, 10,
         AboutTimerProc, (DWORD_PTR)hDlg, TIME_PERIODIC|TIME_CALLBACK_FUNCTION);
                                                                              }
                                                                      }
                                                              }
                                                      }
                  ...
                  break;
                      ...
                      case WM_APP:
                              if (g_pvAboutDisplayBack && bRender) {
                                      ...
                                      g_vbAboutDst.BitBlt(rDirty2.left, rDirty2.top, &g_vbAboutSrc, rDirty2.left,
         rDirty2.top, rDirty2.right+1-rDirty2.left, rDirty2.bottom+1-rDirty2.top);
                                      ...
                                      for(int f=0; f<6; f++) {
                                              ...
                                              float shade = normal[0]*lightdir[0] + normal[1]*lightdir[1] +
         normal[2]*lightdir[2];

                                              uint32 alpha = VDRoundToInt((1.5f + 0.5f*shade) * 127.5f);

                                              RenderTriangle((Pixel16 *)g_vbAboutDst.Address32(0,0),
         -g_vbAboutDst.pitch/2, tex, v+0, v+1, v+2, alpha); RenderTriangle((Pixel16 *)g_vbAboutDst.Address32(0,0),
         -g_vbAboutDst.pitch/2, tex, v+2, v+1, v+3, alpha);
                                      }
