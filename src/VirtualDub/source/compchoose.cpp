...
VDUIDialogChooseVideoCompressorW32::VDUIDialogChooseVideoCompressorW32(COMPVARS2 *cv, BITMAPINFOHEADER *src)
  : VDDialogFrameW32(IDD_VIDEOCOMPRESSION), mhCodec(NULL), mSelect(-1), mCodecStateId(-1), mpCurrent(NULL),
    mpCompVars(cv), mpSrcFormat(src)
{
  mCodecList.OnSelectionChanged() +=
    mdelSelChanged.Bind(this, &VDUIDialogChooseVideoCompressorW32::OnCodecSelectionChanged);
  mCapture         = false;
  enable_all       = true;
  filter_format    = 0;
  filter_mode      = 1;
...
bool VDUIDialogChooseVideoCompressorW32::OnLoaded()
{
  ...
  else
  {
    ...
    VDString s;
    if (filter_format)
      s = VDPixmapFormatPrintSpec(filter_format);
    else
      s = print_fourcc(mpSrcFormat->biCompression);
  ...
  vdprotected1("querying video codec \"%.64s\"", const char *, debug_id)
  {
    ...
    if (filter_format)
    {
...

      {
        ...
        {
          if (VDPixmapFormatGroup(format) != filter_group)
            continue;
...
int VDUIDialogChooseVideoCompressorW32::testDIBFormat(
  EncoderHIC *      plugin,
...
void VDUIDialogChooseVideoCompressorW32::UpdateEnables()
{
  ...
  bool enable;
  enable = !!(dwFlags & (VIDCF_CRUNCH | VIDCF_QUALITY)); // Strange but true: Windows expects to be able to crunch even
                                                         // if only the quality bit is set.
...
static int g_xres[] = {160, 176, 320, 352, 640, 720};

static int g_yres[] = {120, 144, 240, 288, 480, 576};

static int g_depths[] = {16, 24, 32, 32, 32, 48, 48, 64, 64, 12, 16, 16, 16, 24, 24, 24, 24, 30, 32, 32, 32, 32, 64};

static int g_depths_fcc[] = {
  0,
  0,
  0,
  VDMAKEFOURCC('r', '2', '1', '0'),
  VDMAKEFOURCC('R', '1', '0', 'k'),
  ...
  VDMAKEFOURCC('v', '3', '0', '8'),
  VDMAKEFOURCC('P', '0', '1', '0'),
  VDMAKEFOURCC('P', '0', '1', '6'),
  VDMAKEFOURCC('v', '2', '1', '0'),
  VDMAKEFOURCC('P', '2', '1', '0'),
  VDMAKEFOURCC('P', '2', '1', '6'),
  VDMAKEFOURCC('v', '4', '1', '0'),
  VDMAKEFOURCC('Y', '4', '1', '0'),
  VDMAKEFOURCC('Y', '4', '1', '6')};

static const wchar_t *g_depths_id[] = {L"rgb16",   L"rgb",  L"rgba", L"r210", L"R10k", L"b48r", L"BGR[48]", L"b64a",
                                       L"BRA[64]", L"YV12", L"YV16", L"YUY2", L"HDYC", L"YV24", L"v308",    L"P010",
                                       L"P016",    L"v210", L"P210", L"P216", L"v410", L"Y410", L"Y416"};
...
void VDUIDialogChooseVideoCompressorW32::UpdateFormat()
{
  ...
  if (mCapture)
  {
    ...
    {
      VDStringW s;
      if (!make.compDib.empty())
        s = printW_fourcc(make.compDib->biCompression);
  ...
  if (!mhCodec)
  {
    LBClear(IDC_SIZE_RESTRICTIONS);
    if (uncommon_raw)
      LBAddString(IDC_SIZE_RESTRICTIONS, L"This format layout is uncommon.");
  }
...
void ChooseCaptureCompressor(HWND hwndParent, COMPVARS2 *lpCompVars, BITMAPINFOHEADER *bihInput)
{
  ...
  if (bihInput)
  {
    ...
    if (!dlg.filter_format)
      dlg.enable_all = false;
  }
