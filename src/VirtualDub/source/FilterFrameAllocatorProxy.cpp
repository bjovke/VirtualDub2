...
VDFilterFrameAllocatorProxy::VDFilterFrameAllocatorProxy(const VDFilterFrameAllocatorProxy &src)
  : mSizeRequired(src.mSizeRequired), mBorderWRequired(src.mBorderWRequired), mBorderHRequired(src.mBorderHRequired),
    mAccelMode(src.mAccelMode), mpAllocator(src.mpAllocator), mpLink(NULL)
{
  if (mpAllocator)
    mpAllocator->AddRef();
}
