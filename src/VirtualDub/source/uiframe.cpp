...
VDUIFrame::VDUIFrame(HWND hwnd)
  : mpClient(NULL), mhAccel(NULL), mNestCount(0), mbDetachClient(false), mbDetachEngine(false), mbDestroy(false),
    mhwnd(hwnd), mRefCount(1), mpRegistryName(NULL)
{
...
{
  if (
    !msg.hwnd || (msg.message != WM_KEYDOWN && msg.message != WM_SYSKEYDOWN && msg.message != WM_KEYUP &&
  ...
  if (p->mpClient)
  {
    ...
    {
      case WM_CHAR:
        return p->mpClient->Intercept_WM_CHAR(msg.wParam, msg.lParam);
      case WM_KEYDOWN:
        return p->mpClient->Intercept_WM_KEYDOWN(msg.wParam, msg.lParam);
      case WM_KEYUP:
        return p->mpClient->Intercept_WM_KEYUP(msg.wParam, msg.lParam);
      case WM_SYSKEYDOWN:
        return p->mpClient->Intercept_WM_SYSKEYDOWN(msg.wParam, msg.lParam);
      case WM_SYSKEYUP:
        return p->mpClient->Intercept_WM_SYSKEYUP(msg.wParam, msg.lParam);
    }
  }
  ...
  else if (p = (VDUIFrame *)GetWindowLongPtr(hwnd, 0))
  {
    switch (msg)
    {
      ...
      case WM_INITMENUPOPUP:
        if (HIWORD(lParam))
        {
          VDCheckMenuItemByCommandW32(
            (HMENU)wParam, ID_SYSTEM_ALWAYSONTOP, (GetWindowLong(hwnd, GWL_EXSTYLE) & WS_EX_TOPMOST) != 0);
        }
        break;
