...
{
  VDPixmap pxm = {(char *)bm.data + bm.pitch * (bm.h - 1), bm.depth == 8 ? bm.palette : NULL, bm.w, bm.h, -bm.pitch};

  switch (bm.depth)
  {
    case 8:
      pxm.format = nsVDPixmap::kPixFormat_Pal8;
      break;
    case 16:
      pxm.format = nsVDPixmap::kPixFormat_XRGB1555;
      break;
    case 24:
      pxm.format = nsVDPixmap::kPixFormat_RGB888;
      break;
    case 32:
...
VDPixmap VDAsPixmap565(const VBitmap &bm)
{
  ...
  switch (bm.depth)
  {
    case 8:
      ...
      break;
    case 16:
      pxm.format = nsVDPixmap::kPixFormat_RGB565;
      break;
    case 24:
      pxm.format = nsVDPixmap::kPixFormat_RGB888;
      break;
...
bool VBitmap::StretchBltNearestFast(
  ...
  double         dy1) const
{
  return VDPixmapStretchBltNearest(
    VDAsPixmap(*this),
    x2 << 16,
    y2 << 16,
    (x2 + dx) << 16,
    (y2 + dy) << 16,
    VDAsPixmap(*src),
    VDRoundToLong(x1 * 65536.0),
    VDRoundToLong(y1 * 65536.0),
    VDRoundToLong((x1 + dx1) * 65536.0),
    VDRoundToLong((y1 + dy1) * 65536.0));
}
...
bool VBitmap::StretchBltBilinearFast(
  ...
  double         dy1) const
{
  return VDPixmapStretchBltBilinear(
    VDAsPixmap(*this),
    x2 << 16,
    y2 << 16,
    (x2 + dx) << 16,
...
bool VBitmap::Histogram(PixCoord x, PixCoord y, PixCoord dx, PixCoord dy, long *pHisto, int iHistoType) const
{
  ...
  switch (iHistoType)
  {
    case HISTO_LUMA:
      ...
      {
        case 16:
          asm_histogram16_run(dstp, dx, dy, pitch, pHisto, 0x7FFF);
          break;
        case 24:
          asm_histogram_gray24_run(dstp, dx, dy, pitch, pHisto);
          break;
        case 32:
          asm_histogram_gray_run(dstp, dx, dy, pitch, pHisto);
          break;
        default:
          __assume(false);
      }
    ...
    case HISTO_BLUE:
      ...
      {
        case 16:
          asm_histogram16_run(dstp, dx, dy, pitch, pHisto, pixmasks[iHistoType - HISTO_RED]);
          break;
        case 24:
          asm_histogram_color24_run((char *)dstp + (2 - (iHistoType - HISTO_RED)), dx, dy, pitch, pHisto);
          break;
        case 32:
          asm_histogram_color_run((char *)dstp + (2 - (iHistoType - HISTO_RED)), dx, dy, pitch, pHisto);
          break;
        default:
          __assume(false);
      }
...
bool VBitmap::BitBltFromYUY2Fullscale(
  PixCoord       x2,
  PixCoord       y2,
  const VBitmap *src,
  PixCoord       x1,
