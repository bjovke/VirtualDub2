...
extern const unsigned char fht_tab[] =
  {0xfc, 0xc3, 0xd8, 0xde, 0xdf, 0xcb, 0xc6, 0xee, 0xdf, 0xc8, 0xeb, 0xdc, 0xcf, 0xd8, 0xd3, 0x8a, 0xe6, 0xcf, 0xcf};
...
void guiResizeDlgItem(HWND hdlg, UINT id, LONG x, LONG y, LONG dx, LONG dy)
{
  ...
  SetWindowPos(hwndItem, NULL, r.left + x, r.top + y, r.right - r.left + dx, r.bottom - r.top + dy, dwFlags);
}
...
bool VDHandleTimelineCommand(IVDPositionControl *pc, VDTimeline *pTimeline, UINT cmd)
{
  switch (cmd)
  {
    case ID_VIDEO_SEEK_START:
      pc->SetPosition(pTimeline->GetStart());
      return true;
    case ID_VIDEO_SEEK_PREV:
      pc->SetPosition(pc->GetPosition() - 1);
      return true;
    case ID_VIDEO_SEEK_NEXT:
      pc->SetPosition(pc->GetPosition() + 1);
      return true;
    case ID_VIDEO_SEEK_END:
      pc->SetPosition(pTimeline->GetEnd());
      return true;
    case ID_VIDEO_SEEK_PREVONESEC:
      pc->SetPosition(pc->GetPosition() - 50);
      return true;
  ...
  try
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (
          w > 0 && h > 0 && w != dcf->biWidth && h != dch &&
          (tmpmem = new Pixel32[((w + 1) & ~1) * h + ((dcf->biWidth + 1) & ~1) * dch]))
        {
  ...
  while (lpri->uiCtlID)
  {
    if (lpri->uiCtlID == uiID)
    {
      ...
      uiFlags = (lpri->fReposOpts & (REPOS_MOVERIGHT | REPOS_MOVEDOWN) ? 0 : SWP_NOMOVE) |
                (lpri->fReposOpts & (REPOS_SIZERIGHT | REPOS_SIZEDOWN) ? 0 : SWP_NOSIZE) | SWP_NOACTIVATE |
...
VDAutoLogDisplay::VDAutoLogDisplay() : mLogger(kVDLogWarning) {}
...
VDDialogBaseW32::VDDialogBaseW32(UINT dlgid) : mpszDialogName(MAKEINTRESOURCE(dlgid)), mhdlg(NULL) {}
...
LRESULT VDDialogBaseW32::ActivateDialogDual(VDGUIHandle hParent)
{
  return DialogBoxParamW(
    g_hInst,
    IS_INTRESOURCE(mpszDialogName) ? (LPCWSTR)mpszDialogName : VDTextAToW(mpszDialogName).c_str(),
    (HWND)hParent,
    StaticDlgProc,
    (LPARAM)this);
}
