...
char *VDDisasmMatchRule(
  VDDisassemblyContext *pContext,
...
std::string decode_result(const unsigned char *result)
{
  ...
  while (result < result_limit)
  {
    ...
    if ((unsigned char)(c & 0x7f) < 32)
    {
      ...
      static const unsigned char static_bitfields[8] = {0x80, 0x30, 0x33, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void VDDisasmExpandRule(
  VDDisassemblyContext *pContext,
  char *                s,
  const unsigned char * result,
  const ptrdiff_t *     sp_base,
  const unsigned char * source)
{
  ...
  static const char *const reg8a[16] = {
  ...
  static const char *const regxmm[16] = {
    "xmm0",
    "xmm1",
    "xmm2",
    "xmm3",
  ...
  static const char *const *const sStaticLabels[] = {reg32, reg16, reg8, regmmx, regxmm, regcrn, regdrn, regseg, regf};
      ...
      else
      {
        ...
        switch (control_byte)
        {
          ...
          case kTarget_lx:
            ...
            break;
          case kTarget_hx:
            s += sprintf(s, "%02x%02x", (unsigned char)sp_base[c], (unsigned char)sp_base[c - 1]);
            break;
          case kTarget_x:
          ...
          case kTarget_lo:
            ...
            break;
          case kTarget_ho: {
            int x = ((unsigned char)sp_base[c] << 8) + (unsigned char)sp_base[c - 1];
          ...
          case kTarget_r163264:
            s = strtack(
              s,
              (pContext->rex & 8 ? reg64 :
                                   pContext->bAddressOverride ? reg16 : reg32)[arg + ((pContext->rex & 0x04) << 1)]);
            break;
          case kTarget_ext:
            switch (*result++)
            {
    ...
    else if ((unsigned char)c >= 0xe0)
    {
      switch (c)
      {
        ...
        case kTarget_p_F3:
          ...
          break;
        case kTarget_p_rex:
          pContext->rex = (unsigned char)sp_base[0];
          break;
      }
    }
    else
  ...
  const unsigned char *&source_end)
{
  ...
  while (rs[0] || rs[1])
  {
    ...
    if (hpr)
    {
      ...
      //pContext->physToVirtOffset, (uintptr)source + pContext->physToVirtOffset + bytes, decode_pattern(rs+1,
      //rs[0]).c_str()); 			VDDEBUG("Rule apply: addr=%p, result: %s\n", (uintptr)source + pContext->physToVirtOffset,
      //decode_result(result).c_str());
  ...
  while (pattern_len)
  {
    if (!pattern[1] && pattern[0])
    {
      if (pattern[0] & 0x80)
      {
        ...
        hp = VDDisasmApplyRuleset(
          pContext, pContext->pRuleBase + pContext->pRuleSystemOffsets[count + 1], sp, hp, source, bytes, src_end);
...
char *VDDisassemble(VDDisassemblyContext *pvdc, const unsigned char *source, int bytes, int &count)
{
  ...
  do
  {
    s = VDDisasmApplyRuleset(
      pvdc, pvdc->pRuleBase + pvdc->pRuleSystemOffsets[0], pvdc->stack, pvdc->heap, src2, bytes, src_end);
...
CodeDisassemblyWindow::CodeDisassemblyWindow(void *_code, long _length, void *_rbaseptr, void *_abaseptr)
  : code(_code), rbase(_rbaseptr), abase(_abaseptr), length(_length), pFault(0)
{
