...
VDBackfaceClass::VDBackfaceClass(const char *shortname, const char *longname)
  : mpShortName(shortname), mpLongName(longname), mObjectCount(0)
{
...
VDBackfaceService::VDBackfaceService() : mObjectCount(0), mClassCount(0), mNextInstance(1), mpCaptureString(NULL) {}
...
VDBackfaceObjectBase::VDBackfaceObjectBase(const VDBackfaceObjectBase &src) : mpClass(src.mpClass)
{
...
VDBackfaceConsole::VDBackfaceConsole() : mRefCount(0), mFont(NULL), mhmodRichEdit(NULL) {}
...
bool VDBackfaceConsole::OnCreate()
{
  ...
  mhwndLog = CreateWindowEx(
    WS_EX_CLIENTEDGE,
    "RICHEDIT",
    "",
    ES_READONLY | ES_MULTILINE | ES_AUTOVSCROLL | WS_VSCROLL | WS_VISIBLE | WS_CHILD,
    0,
    0,
    0,
    0,
  ...
  mhwndEdit = CreateWindowEx(
    WS_EX_CLIENTEDGE, "RICHEDIT", "", WS_VISIBLE | WS_CHILD, 0, 0, 0, 0, mhwnd, (HMENU)100, g_hInst, NULL);
  if (!mhwndEdit)
    return false;
