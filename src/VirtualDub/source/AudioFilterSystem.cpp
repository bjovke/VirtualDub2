...
const VDAudioFilterCallbacks g_audioFilterCallbacks =
  {VDAllocPCMWaveFormat, VDAllocCustomWaveFormat, VDXCopyWaveFormat, VDFreeWaveFormat, VDAFCallback_Wake};
...
public:
  ...
  void SetFilter(VDAudioFilterInstance *pInst, unsigned pin)
  {
    mpFilter   = pInst;
    mPinNumber = pin;
  }

  VDAudioFilterInstance *Filter() const
  {
    return mpFilter;
  }
  VDAudioFilterPinImpl *Connection() const
  {
    return mpPin;
  }
  VDAudioFilterInstance *ConnectedFilter() const
  {
    return mpPin->mpFilter;
  }
  int Number() const
  {
    return mPinNumber;
...
class VDAudioFilterInstance : protected VDAudioFilterContext,
                              public VDSchedulerNode,
                              public IVDAudioFilterInstance,
...
public:
  ...
  unsigned InputPinCount() const
  {
    return mpDefinition->mInputPins;
  }
  unsigned OutputPinCount() const
  {
    return mpDefinition->mOutputPins;
  }
  VDAudioFilterPinImpl &InputPin(unsigned p)
  {
    return mPins[p];
  }
  VDAudioFilterPinImpl &OutputPin(unsigned p)
  {
    return mPins[p + mpDefinition->mInputPins];
  }

  const VDPluginInfo *GetPluginInfo()
  {
    return mpPluginInfo;
  }
  const VDAudioFilterDefinition *GetDefinition()
  {
    return mpDefinition;
  }
  ...
  bool IsSerializedIOOnly() const
  {
    return !!(mpDefinition->mFlags & kVFAF_SerializedIO);
  }
...
VDAudioFilterPinImpl::VDAudioFilterPinImpl() : mpPin(NULL)
{
  mpFormat   = NULL;
  mpReadProc = ReadData;
}
...
uint32 __cdecl VDAudioFilterPinImpl::ReadData(
  VDAudioFilterPin *pPin0,
  void *            dst,
  uint32            samples,
  bool              bAllowFill,
  int               format)
...
VDAudioFilterInstance::VDAudioFilterInstance(VDPluginDescription *pDesc)
  : mpDescription(pDesc), mpPluginInfo(NULL), mbPrepared(false)
{
    ...
    if (pEnt->idx == idx)
    {
      ...
      switch (var.GetType())
      {
        case VDPluginConfigVariant::kTypeU32:
          mpDefinition->mpVtbl->mpSetParam(this, idx, &var.GetU32(), sizeof(uint32));
          break;
        case VDPluginConfigVariant::kTypeS32:
          mpDefinition->mpVtbl->mpSetParam(this, idx, &var.GetS32(), sizeof(sint32));
          break;
        case VDPluginConfigVariant::kTypeU64:
          mpDefinition->mpVtbl->mpSetParam(this, idx, &var.GetU64(), sizeof(uint64));
          break;
        case VDPluginConfigVariant::kTypeS64:
          mpDefinition->mpVtbl->mpSetParam(this, idx, &var.GetS64(), sizeof(sint64));
          break;
        case VDPluginConfigVariant::kTypeDouble:
          mpDefinition->mpVtbl->mpSetParam(this, idx, &var.GetDouble(), sizeof(double));
          break;
        case VDPluginConfigVariant::kTypeAStr:
          mpDefinition->mpVtbl->mpSetParam(this, idx, var.GetAStr(), strlen(var.GetAStr()) + 1);
          break;
        case VDPluginConfigVariant::kTypeWStr:
          mpDefinition->mpVtbl->mpSetParam(this, idx, var.GetWStr(), (wcslen(var.GetWStr()) + 1) * sizeof(wchar_t));
          break;
        case VDPluginConfigVariant::kTypeBlock:
          mpDefinition->mpVtbl->mpSetParam(this, idx, var.GetBlockPtr(), var.GetBlockLen());
          break;
        default:
          VDASSERT(false);
      }
...
bool VDAudioFilterInstance::Service()
{
  ...
  for (i = 0; i < mpDefinition->mInputPins; ++i)
  {
    ...
    unsigned samples =
      inpinconn.Filter()->mOutputBuffers[inpinconn.Number()].getLevel() / inpinconn.mpFormat->mBlockSize;
...
bool VDAudioFilterInstance::GetInputPinConnection(
  unsigned                 inputPin,
  IVDAudioFilterInstance *&pFilt,
  unsigned &               outputPin)
{
...
bool VDAudioFilterInstance::GetOutputPinConnection(
  unsigned                 outputPin,
  IVDAudioFilterInstance *&pFilt,
  unsigned &               inputPin)
{
...
void VDAudioFilterInstance::DumpStatus()
{
  ...
  for (unsigned j = 0; j < mpDefinition->mOutputPins; ++j)
  {
    ...
    VDDEBUG2(
      "            Output pin %d: %d/%d bytes (%s)\n",
      j,
      buffer.getLevel(),
      buffer.getSize(),
      pin.mbEnded ? "ended" : "active");
  }
}
...
VDAudioFilterSystem::VDAudioFilterSystem() : mpIOScheduler(NULL), mpFastScheduler(NULL) {}
...
void VDAudioFilterSystem::Connect(
  IVDAudioFilterInstance *pFilterOut,
  unsigned                nPinOut,
  IVDAudioFilterInstance *pFilterIn,
  unsigned                nPinIn)
{
  ...
  VDDEBUG(
    "[AudioFilter] %s[%d] -> %s[%d]\n",
    VDTextWToA(pFilterIn->GetPluginInfo()->mpName).c_str(),
    nPinIn,
    VDTextWToA(pFilterOut->GetPluginInfo()->mpName).c_str(),
    nPinOut);
...
void VDAudioFilterSystem::LoadFromGraph(
  const VDAudioFilterGraph &             graph,
  std::vector<IVDAudioFilterInstance *> &filterPtrs)
{
  ...
  for (std::list<VDAudioFilterGraph::FilterEntry>::const_iterator it(graph.mFilters.begin()),
       itEnd(graph.mFilters.end());
       it != itEnd;
       ++it)
  {
    ...
    if (pDef->mInputPins != (uint32)f.mInputPins || pDef->mOutputPins != (uint32)f.mOutputPins)
      throw MyError(
        "Audio filter \"%s\" has a different number of pins than specified in filter graph.",
        VDTextWToA(pDesc->mName).c_str());
...
void VDAudioFilterSystem::Prepare(VDAudioFilterInstance *pInst, bool mustSucceed)
{
  ...
  for (i = 0, n = pInst->InputPinCount(); i < n; ++i)
  {
    if (!pInst->InputPin(i).IsConnected())
      throw MyError(
        "Input pin %d of audio filter \"%s\" is unconnected.", i, VDTextWToA(pInst->GetPluginInfo()->mpName).c_str());
  ...
  for (i = 0, n = pInst->OutputPinCount(); i < n; ++i)
  {
    if (!pInst->OutputPin(i).IsConnected())
      throw MyError(
        "Output pin %d of audio filter \"%s\" is unconnected.", i, VDTextWToA(pInst->GetPluginInfo()->mpName).c_str());
  ...
  if (rv == kVFAPrepare_BadFormat)
    throw MyError(
      "Audio filter \"%s\" cannot handle its input. Check that the filter is designed to handle the audio format you "
  ...
  for (i = 0, n = pInst->InputPinCount(); i < n; ++i)
  {
    ...
    VDDEBUG(
      "AudioFilterSystem: Filter %s pin %d: size %d\n",
      VDTextWToA(pInst->GetPluginInfo()->mpName).c_str(),
      i,
      pin.mBufferSize);
  }
