...
FrameSubset::FrameSubset(const FrameSubset &src) : mTimeline(src.mTimeline)
{
  invalidateCache();
}
...
void FrameSubset::addRangeMerge(sint64 start, sint64 len, bool bMask, int source)
{
  ...
  while (it != itEnd)
  {
    if (start + len < it->start)
    { // Isolated -- insert
      mTimeline.insert(it, FrameSubsetNode(start, len, bMask, source));
      invalidateCache();
      return;
    }
    else if (start + len >= it->start && start <= it->start + it->len)
    { // Overlap!

      if (start + len > it->start + it->len)
      { // < A [ B ] > or [ B <] A > cases
