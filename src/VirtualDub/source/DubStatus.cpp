...
{
  static long bounds[] = {
    0,        100,      200,      500,       1024,      2048,      5120,       10240,
    20480,    51200,    102400,   204800,    512000,    1048576,   2097152,    5242880,
    10485760, 20971520, 52428800, 104857600, 209715200, 524288000, 1073741824, 2147483647,
  };
...
DubStatus::DubStatus()
  : iLastTitleMode(TITLE_IDLE), lLastTitleProgress(0), hwndStatus(NULL), mNextSamplePoint(0), mSampleCount(0),
    lFrameFirstIndex(0), lFrameLastIndex(0), fShowStatusWindow(true), fFrozen(false), mpPositionCallback(NULL),
    mProgress(0), thisRef(0)
{
...
void DubStatus::StatusTimerProc(HWND hWnd)
{
  ...
  if (!totalASamples || !totalVSamples || pvinfo->fAudioOnly)
    nProgress *= 2;
  ...
  if (nFramesProcessed)
  {
    ...
    if (prevPt.mFramesProcessed != (uint32)nFramesProcessed)
    {
      ...
      currentPt.mFramesProcessed = (uint32)nFramesProcessed; // Yes, this truncates.  We'll be okay as long as no more
                                                             // than 2^32-1 frames are processed per interval.
...
INT_PTR CALLBACK DubStatus::StatusMainDlgProc(HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (message)
  {
    case WM_INITDIALOG: {
      ...
      SetWindowPos(
        hdlg, HWND_TOP, thisPtr->rStatusChild.left, thisPtr->rStatusChild.top, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE);
...
INT_PTR CALLBACK DubStatus::StatusPerfDlgProc(HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (message)
  {
    ...
    case WM_TIMER: {
      ...
      VDSetWindowTextFW32(
        GetDlgItem(hdlg, IDC_STATIC_VIDEOBUFFER), L"%u/%u", status.mVideoBuffersActive, status.mVideoBuffersTotal);
      VDSetWindowTextFW32(GetDlgItem(hdlg, IDC_STATIC_VIDEOREQUESTS), L"%u", status.mVideoRequestsActive);
      VDSetWindowTextFW32(
        GetDlgItem(hdlg, IDC_STATIC_AUDIOBUFFER),
        L"%u/%uKB",
        (status.mAudioBufferInUse + 512) >> 10,
        (status.mAudioBufferTotal + 512) >> 10);
  ...
  switch (message)
  {
    ...
    case WM_TIMER:
...

      {
        ...
        if (!totalASamples || !totalVSamples || thisPtr->pvinfo->fAudioOnly)
          nProgress *= 2;
      ...
      switch (nm->code)
      {
        case TCN_SELCHANGE: {
          ...
          if (iTab >= 0)
          {
            ...
            thisPtr->hwndStatusChild =
              CreateDialogParam(g_hInst, tabs[iTab].rsrc, hdlg, tabs[iTab].dProc, (LPARAM)thisPtr);
          }
        }
          return TRUE;
      }
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam))
      {
        ...
        case IDC_ABORT:
          ...
          if (
            thisPtr->pDubber->IsPreviewing() || !VDPreferencesIsRenderAbortConfirmEnabled() ||
            IDOK ==
              MessageBox(
                hdlg, "Stop the operation at this point?", "VirtualDub Warning", MB_ICONEXCLAMATION | MB_OKCANCEL))
          {
...
void DubStatus::SetLastPosition(VDPosition pos, bool fast_update)
{
  if (mpPositionCallback)
    mpPositionCallback(
      pvinfo->start_src,
      pos < pvinfo->start_src ? pvinfo->start_src : pos > pvinfo->end_src ? pvinfo->end_src : pos,
      pvinfo->end_src,
      mProgress,
      fast_update,
      mpPositionCallbackCookie);
}
...
bool DubStatus::ToggleFrame(bool fFrameOutput)
{
  if (fFrameOutput)
  {
    if (hwndStatus)
      PostMessage(
        GetDlgItem(hwndStatus, IDC_DRAW_OUTPUT),
        BM_SETCHECK,
        !opt->video.fShowOutputFrame ? BST_CHECKED : BST_UNCHECKED,
        0);
  ...
  else
  {
    if (hwndStatus)
      PostMessage(
        GetDlgItem(hwndStatus, IDC_DRAW_INPUT),
        BM_SETCHECK,
        !opt->video.fShowInputFrame ? BST_CHECKED : BST_UNCHECKED,
        0);
