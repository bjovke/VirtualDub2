...
VDDubVideoProcessor::VDDubVideoProcessor()
  : mActivePaths(kPathInitial), mReactivatedPaths(0), mpOptions(NULL), mpStatusHandler(NULL), mpVInfo(NULL),
    mbVideoPushEnded(false), mbVideoEnded(false), mbPreview(false), mbShowLast(false), mbFirstFrame(true),
    mppCurrentAction(NULL), mpCB(NULL), mpVideoFrameMap(NULL), mpVideoFrameSource(NULL), mpVideoRequestQueue(NULL),
    mpVideoPipe(NULL), mpIODirect(NULL), mbSourceStageThrottled(false), mbFilterStageThrottled(false),
    mpVideoFilters(NULL), mFrameProcessAheadCount(0), mBatchNumber(0), mpVideoCompressor(NULL),
    mpThreadedVideoCompressor(NULL), mVideoFramesDelayed(0), mbFlushingCompressor(false), mpVideoOut(NULL),
    mpVideoImageOut(NULL), mExtraOutputBuffersRequired(0), mpLoopThrottle(NULL), mpFrameBufferTracker(NULL),
    mpDisplayBufferTracker(NULL), mFramesToDrop(0), mpProcDisplay(NULL), mThreadPriority(VDThread::kPriorityDefault)
{}
...
void VDDubVideoProcessor::SetVideoCompressor(IVDVideoCompressor *pCompressor, int threadCount)
{
  ...
  if (pCompressor)
  {
    ...
    mpThreadedVideoCompressor->OnFrameComplete() +=
      mThreadedCompressorDelegate(this, &VDDubVideoProcessor::OnAsyncCompressDone);
...
void VDDubVideoProcessor::DumpStatus(VDTextOutputStream &os)
{
  if (mpVideoPipe)
  {
    ...
    if (nextRead)
    {
      ...
      os.FormatLine(
        "    %d bytes | srcidx %d | stream %lld | display %lld | target %lld | flags:%s%s%s%s%s%s | droptype %d | "
        "final %d",
        nextRead->mLength,
        nextRead->mSrcIndex,
  ...
  for (PendingSourceFrames::const_iterator it(mPendingSourceFrames.begin()), itEnd(mPendingSourceFrames.end());
       it != itEnd;
       ++it)
  {
  ...
  for (PendingOutputFrames::const_iterator it(mPendingOutputFrames.begin()), itEnd(mPendingOutputFrames.end());
       it != itEnd;
       ++it)
  {
    ...
    os.FormatLine(
      "  Frame %u | Batch %u | Hold %u | Null %u | Direct %u",
...
VDDubVideoProcessor::VideoWriteResult VDDubVideoProcessor::GetVideoOutputBuffer(VDRenderOutputBuffer **ppBuffer)
{
  ...
  for (;;)
  {
    ...
    bool successful = mpDisplayBufferTracker ? mpDisplayBufferTracker->AllocFrame(100, ~pBuffer) :
                                               mpFrameBufferTracker->AllocFrame(100, ~pBuffer);
...
bool VDDubVideoProcessor::CheckForThreadedCompressDone()
{
  ...
  WriteFinishedVideoFrame(
    pOutBuffer->mOutputBuffer.data(), pOutBuffer->mOutputSize, pOutBuffer->packetInfo, false, NULL);
  return true;
}
...
VDDubVideoProcessor::VideoWriteResult VDDubVideoProcessor::ProcessVideoFrame()
{
  ...
  const VDPixmapLayout &layout = (mpOptions->video.mode == DubVideoOptions::M_FULL) ?
                                   mpVideoFilters->GetOutputLayout() :
                                   mpVideoFilters->GetInputLayout();
...
VDDubVideoProcessor::VideoWriteResult VDDubVideoProcessor::WriteFinishedVideoFrame(
...
void VDDubVideoProcessor::WriteFinishedVideoFrame(
  const void *          data,
  uint32                size,
  VDPacketInfo &        packetInfo,
  bool                  renderEnabled,
