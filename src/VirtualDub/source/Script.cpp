...
void RunScript(const wchar_t *name, void *hwnd)
{
  static const wchar_t fileFilters[] =
    L"All scripts\0"
    L"*.vdscript;*.vcf;*.syl;*.jobs;*.vdproject\0"
    L"VirtualDub configuration file\0"
    L"*.vcf\0"
    L"VirtualDub script\0"
    L"*.vdscript;*.syl\0"
    L"VirtualDub job queue\0"
    L"*.jobs\0"
    L"VirtualDub project\0"
    L"*.vdproject\0"
    L"All Files (*.*)\0"
    L"*.*\0";
  ...
  if (!name)
  {
    filenameW = VDGetLoadFileName(
      VDFSPECKEY_SCRIPT, (VDGUIHandle)hwnd, L"Load configuration script", fileFilters, L"vdscript", 0, 0);
  ...
  catch (const VDScriptError &cse)
  {
    ...
    throw MyError(
      "Error during script execution at line %d, column %d: %s\n\n"
      "    %.*s<!>%.50s",
      lineno,
      pos + 1,
...
void RunScriptMemory(const char *mem, int start_line, bool stopAtReloadMarker, bool partial)
{
  ...
  catch (const VDScriptError &cse)
  {
    ...
    if (start_line == -1)
    {
      MyTextError err;
      err.setf(
        "Error during script execution at offset %d: %s\n\n"
        "%s",
        pos + 1,
        isi->TranslateScriptError(cse),
        text.c_str());
...
static const VDScriptObject obj_VDParameterCurve = {"VDParameterCurve", NULL, obj_VDParameterCurve_functbl, NULL};
...
static void func_VDVFiltInst_GetClipping(IVDScriptInterpreter *isi, VDScriptValue *argv, int argc)
{
  ...
  switch (argv[0].asInt())
  {
    ...
    case 3:
      ...
      break;
    case 4:
      argv[0] = ent->mpInstance->IsPreciseCroppingEnabled();
      break;
    default:
...
static VDScriptValue obj_VDVFiltInst_lookup(
  IVDScriptInterpreter *isi,
...
static const VDScriptFunctionDef obj_VDVFilters_instance_functbl[] = {{func_VDVFilters_instance, "[]", "vi"}, {NULL}};
...
static VDScriptValue obj_VDVFilters_lookup(
  IVDScriptInterpreter *isi,
  const VDScriptObject *thisPtr,
  void *                lpVoid,
  char *                szName)
...
static const VDScriptObject obj_VDVFilters = {"VDVideoFilters", obj_VDVFilters_lookup, obj_VDVFilters_functbl, NULL};
...
static void func_VDVideo_SetDepth(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  {
    case 16:
      new_depth1 = nsVDPixmap::kPixFormat_XRGB1555;
      break;
    case 24:
      new_depth1 = nsVDPixmap::kPixFormat_RGB888;
      break;
    case 32:
      new_depth1 = nsVDPixmap::kPixFormat_XRGB8888;
      break;
    default:
      return;
  }
  ...
  {
    case 16:
      new_depth2 = nsVDPixmap::kPixFormat_XRGB1555;
      break;
    case 24:
      new_depth2 = nsVDPixmap::kPixFormat_RGB888;
      break;
    case 32:
      new_depth2 = nsVDPixmap::kPixFormat_XRGB8888;
      break;
    default:
      return;
  }
...
static void func_VDVideo_SetInputFormat(IVDScriptInterpreter *, VDScriptValue *argv, int argc)
{
  ...
  if (format >= nsVDPixmap::kPixFormat_Max_Standard)
...
static void func_VDVideo_SetInputMatrix(IVDScriptInterpreter *, VDScriptValue *argv, int argc)
{
  ...
  if (colorSpace >= vd2::kColorSpaceModeCount)
    colorSpace = vd2::kColorSpaceMode_None;
  if (colorRange >= vd2::kColorRangeModeCount)
    colorRange = vd2::kColorRangeMode_None;
...
static void func_VDVideo_GetFrameRate(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  {
    case 0:
      arglist[0] = VDScriptValue(g_dubOpts.video.frameRateDecimation);
      return;
    case 1:
      if (g_dubOpts.video.mFrameRateAdjustLo)
        arglist[0] = VDScriptValue((int)VDRoundToInt(
          (double)g_dubOpts.video.mFrameRateAdjustLo / (double)g_dubOpts.video.mFrameRateAdjustHi * 1000000.0));
...
static void func_VDVideo_SetFrameRate(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  if (arg_count > 2)
  {
    if (arglist[2].asInt())
      throw MyError(
        "Inverse telecine (IVTC) is no longer supported as a pipeline parameter and has been moved to a video filter.");
  }
...
static void func_VDVideo_GetRange(IVDScriptInterpreter *isi, VDScriptValue *arglist, int arg_count)
{
  ...
  arglist[0] = VDScriptValue(
    arglist[0].asInt() ? g_dubOpts.video.mSelectionEnd.ResolveToMS(timelineLen, frameRate, true) :
                         g_dubOpts.video.mSelectionStart.ResolveToMS(timelineLen, frameRate, false));
}
...
static void func_VDVideo_GetCompression(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  if (g_Vcompression.dwFlags & ICMF_COMPVARS_VALID)
  {
    ...
    {
      case 0:
        arglist[0] = VDScriptValue((int)g_Vcompression.fccHandler);
        return;
      case 1:
        arglist[0] = VDScriptValue(g_Vcompression.lKey);
        return;
      case 2:
        arglist[0] = VDScriptValue(g_Vcompression.lQ);
        return;
      case 3:
        arglist[0] = VDScriptValue(g_Vcompression.lDataRate);
        return;
    }
  }
...
static const VDScriptObjectDef obj_VDVideo_objtbl[] = {{"filters", &obj_VDVFilters}, {NULL}};

static VDScriptValue obj_VirtualDub_video_lookup(
  IVDScriptInterpreter *isi,
...
void SetFilterParam(void *lpVoid, unsigned idx, const VDPluginConfigVariant &v)
{
  ...
  const VDXPluginConfigEntry *pEnt =
    GetFilterParamEntry(((VDAudioFilterDefinition *)lock->mpInfo->mpTypeSpecificInfo)->mpConfigInfo, idx);

  if (!pEnt)
    throw MyError(
      "VDAFiltInst: Audio filter \"%s\" does not have a parameter with id %d",
      VDTextWToA(pFilt->mFilterName).c_str(),
      idx);
    ...
    case VDXPluginConfigEntry::kTypeU32:
      switch (v.GetType())
      {
        ...
        case VDPluginConfigVariant::kTypeU64:
          var.SetU32((uint32)v.GetU64());
          return;
        case VDPluginConfigVariant::kTypeS64:
          var.SetU32((uint32)v.GetS64());
          return;
      }
      break;
    case VDXPluginConfigEntry::kTypeS32:
      switch (v.GetType())
      {
        ...
        case VDPluginConfigVariant::kTypeS32:
          ...
          return;
        case VDPluginConfigVariant::kTypeU64:
          var.SetS32((sint32)v.GetU64());
          return;
        case VDPluginConfigVariant::kTypeS64:
          var.SetS32((sint32)v.GetS64());
          return;
      }
      break;
  ...
  throw MyError(
    "VDAFiltInst: Type mismatch on audio filter \"%s\" param %d (\"%s\")",
    VDTextWToA(pFilt->mFilterName).c_str(),
    idx,
    VDTextWToA(pEnt->name).c_str());
}
}; // namespace
...
static const VDScriptObject obj_VDAFiltInst = {"VDAudio", NULL, obj_VDAFiltInst_functbl, NULL};
...
static const VDScriptFunctionDef obj_VDAFilters_instance_functbl[] = {{func_VDAFilters_instance, "[]", "vi"}, {NULL}};
...
static void func_VDAFilters_Add(IVDScriptInterpreter *isi, VDScriptValue *argv, int argc)
{
  ...
  const VDAudioFilterDefinition *pDef =
    reinterpret_cast<const VDAudioFilterDefinition *>(pDesc->mpInfo->mpTypeSpecificInfo);
...
static void func_VDAFilters_Connect(IVDScriptInterpreter *isi, VDScriptValue *argv, int argc)
{
  ...
  if (dstfilt <= srcfilt || dstfilt >= nfilts)
    throw MyError(
      "VDAFilters.Connect(): Invalid target filter number %d (should be %d-%d)", dstfilt, srcfilt + 1, nfilts - 1);
...
static VDScriptValue obj_VDAFilters_lookup(
  IVDScriptInterpreter *isi,
  const VDScriptObject *thisPtr,
  void *                lpVoid,
  char *                szName)
...
static const VDScriptFunctionDef obj_VDAFilters_functbl[] = {
  ...
  {
    func_VDAFilters_Connect,
    "Connect",
    "0iiii",
  },
  {NULL}};

static const VDScriptObject obj_VDAFilters = {"VDAudio", obj_VDAFilters_lookup, obj_VDAFilters_functbl, NULL};
...
static void func_VDAudio_GetInterleave(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  {
    case 0:
      arglist[0] = VDScriptValue(g_dubOpts.audio.enabled);
      return;
    case 1:
      arglist[0] = VDScriptValue(g_dubOpts.audio.preload);
      return;
    case 2:
      arglist[0] = VDScriptValue(g_dubOpts.audio.interval);
      return;
    case 3:
      arglist[0] = VDScriptValue(g_dubOpts.audio.is_ms);
      return;
    case 4:
      arglist[0] = VDScriptValue(g_dubOpts.audio.offset);
      return;
  }
...
static void func_VDAudio_GetClipMode(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  {
    case 0:
      arglist[0] = VDScriptValue(g_dubOpts.audio.fStartAudio);
      return;
    case 1:
      arglist[0] = VDScriptValue(g_dubOpts.audio.fEndAudio);
      return;
  }
...
static void func_VDAudio_GetConversion(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  {
    case 0:
      arglist[0] = VDScriptValue(g_dubOpts.audio.new_rate);
      return;
    case 1:
      arglist[0] = VDScriptValue(g_dubOpts.audio.newPrecision);
      return;
    case 2:
      arglist[0] = VDScriptValue(g_dubOpts.audio.newChannels);
      return;
  }
...
static const VDScriptObjectDef obj_VDAudio_objtbl[] = {{"filters", &obj_VDAFilters}, {NULL}};

static VDScriptValue obj_VirtualDub_audio_lookup(
  IVDScriptInterpreter *isi,
  const VDScriptObject *obj,
  void *                lpVoid,
  char *                szName)
...
static VDScriptValue obj_VDSubset_lookup(
  IVDScriptInterpreter *isi,
  const VDScriptObject *obj,
  void *                lpVoid,
  char *                szName)
...
static const VDScriptObject obj_VDSubset = {"VDSubset", obj_VDSubset_lookup, obj_VDSubset_functbl};
...
static const VDScriptFunctionDef obj_VDParams_functbl[] = {{func_VDParams, "[]", "vi"}, {NULL}};

static const VDScriptObject obj_VDParams = {"VDParams", NULL, obj_VDParams_functbl, NULL};
...
static const VDScriptObject obj_VDProject = {"VDProject", NULL, obj_VDProject_functbl, NULL};
...
static void func_VirtualDub_SaveAVI(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  for (int i = 0;; i++)
  {
    ...
    if (!driver->GetDriver()->EnumFormats(i, filter, ext, name))
    {
      VDString drv = VDTextWToA(g_FileOutDriver);
      throw MyError(
        "Cannot save video with '%s / %s': no such format supported by driver", drv.c_str(), g_FileOutFormat.c_str());
    }
...
static void func_VirtualDub_SaveAudio(IVDScriptInterpreter *, VDScriptValue *arglist, int arg_count)
{
  ...
  for (int i = 0;; i++)
  {
    ...
    if (!driver->GetDriver()->EnumFormats(i, filter, ext, name))
    {
      VDString drv = VDTextWToA(g_AudioOutDriver);
      throw MyError(
        "Cannot save audio with '%s / %s': no such format supported by driver", drv.c_str(), g_AudioOutFormat.c_str());
    }
...
static void func_VirtualDub_SaveRawVideo(IVDScriptInterpreter *isi, VDScriptValue *arglist, int arg_count)
{
  ...
  if (
    !format.mOutputFormat || format.mOutputFormat >= nsVDPixmap::kPixFormat_Max_Standard ||
...
static VDScriptValue obj_VirtualDub_lookup(
  IVDScriptInterpreter *isi,
  const VDScriptObject *obj,
  void *                lpVoid,
  char *                szName)
...
static const VDScriptObject obj_VirtualDub = {"VDApplication", &obj_VirtualDub_lookup, obj_VirtualDub_functbl};
