...
VDDubProcessThread::VDDubProcessThread()
  : VDThread("Processing"), opt(NULL), mpInterleaver(NULL), mpParent(NULL), mpAVIOut(NULL), mpVideoOut(NULL),
    mpAudioOut(NULL), mpOutputSystem(NULL), mpAudioPipe(NULL), mpAudioCorrector(NULL), mpAudioStats(NULL),
    mbAudioPresent(false), mbAudioEnded(false), mAudioSamplesWritten(0), mpVideoPipe(NULL), mbVideoEnded(false),
    mbVideoPushEnded(false), mpVInfo(NULL), mpBlitter(NULL), mpStatusHandler(NULL), mbPreview(false),
    mbFirstPacket(false), mbError(false), mbCompleted(false), mpAbort(NULL), mpCurrentAction("starting up"),
    mActivityCounter(0)
{}
...
void VDDubProcessThread::Init(
  const DubOptions &      opts,
  const VDRenderFrameMap *frameMap,
  DubVideoStreamInfo *    pvsi,
  IVDDubberOutputSystem * pOutputSystem,
  AVIPipe *               pVideoPipe,
  VDAudioPipeline *       pAudioPipe,
  VDStreamInterleaver *   pStreamInterleaver)
{
  opt            = &opts;
  mpVInfo        = pvsi;
...
void VDDubProcessThread::ThreadRun()
{
  ...
  if (VDPreferencesGetRenderInhibitSystemSleepEnabled())
    VDSetThreadExecutionStateW32(
      mbPreview ? ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED | ES_CONTINUOUS : ES_SYSTEM_REQUIRED | ES_CONTINUOUS);
...
bool VDDubProcessThread::WriteAudio(int stream, sint32 count)
{
  ...
  if (mpAudioPipe->IsVBRModeEnabled())
  {
    ...
    while (1)
    {
      if (useTime && totalDuration >= count)
        break;
      if (!useTime && totalSamples >= count)
        break;
    ...
    while (totalBytes < bytes)
    {
      ...
      if (!tc)
      {
        if (mpAudioPipe->isInputClosed())
        {
          ...
          {
            VDLogF(
              kVDLogWarning, L"Back end: The audio stream is ending without any audio samples having been received.");
          }
