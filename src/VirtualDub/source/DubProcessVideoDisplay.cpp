...
IVDVideoDecompressor *VDFindVideoDecompressorEx(
  uint32                       fccHandler,
  const VDAVIBitmapInfoHeader *hdr,
  uint32                       hdrlen,
  bool                         preferInternal);
...
bool AsyncDecompressorFailedCallback(int pass, sint64 timelinePos, void *pDisplayAsVoid, void *, bool aborting)
{
  ...
  pVideoDisplay->SetSourceMessage(
    L"Unable to display compressed video: no decompressor is available to decode the compressed video.");
  return false;
}
...
bool AsyncDecompressorUpdateCallback(
  int    pass,
  sint64 timelinePos,
  void * pDisplayAsVoid,
  void * pPixmapAsVoid,
...
VDDubVideoProcessorDisplay::VDDubVideoProcessorDisplay()
  : mpOptions(NULL), mpVideoCompressor(NULL), mpLoopThrottle(NULL), mbInputLocked(false), mpBlitter(NULL),
    mpInputDisplay(NULL), mpOutputDisplay(NULL), mRefreshFlag(true), mbVideoDecompressorEnabled(false),
    mbVideoDecompressorPending(false), mbVideoDecompressorErrored(false)
{}
...
void VDDubVideoProcessorDisplay::UnlockAndDisplay(bool forceDisplay, VDRenderOutputBuffer *pBuffer, bool outputValid)
{
  ...
  bool renderOutputFrame = (renderFrame || mbVideoDecompressorEnabled) && mpOutputDisplay &&
                           mpOptions->video.mode == DubVideoOptions::M_FULL && mpOptions->video.fShowOutputFrame &&
                           outputValid;
  bool renderAnyFrame = renderFrame && mpOutputDisplay &&
                        (mpOptions->video.fShowInputFrame || mpOptions->video.fShowOutputFrame) && outputValid;
  ...
  if (renderOutputFrame)
  {
    if (mbVideoDecompressorEnabled)
    {
      if (mpVideoDecompressor && !mbVideoDecompressorErrored && !mbVideoDecompressorPending)
      {
        ...
        mpBlitter->postAPC(
          BUFFERID_OUTPUT, output_frame, AsyncDecompressorUpdateCallback, mpOutputDisplay, &mVideoDecompBuffer);
      }
    }
    else
    {
...
void VDDubVideoProcessorDisplay::CheckForDecompressorSwitch()
{
  ...
  if (mbVideoDecompressorEnabled)
  {
    ...
    mpVideoDecompressor               = VDFindVideoDecompressorEx(
      0, pbih, mpVideoCompressor->GetOutputFormatSize(), VDPreferencesIsPreferInternalVideoDecodersEnabled());
      ...
      else
      {
        try
        {
          ...
          VDMakeBitmapCompatiblePixmapLayout(
            ...
            format,
            variant,
            mpVideoDecompressor->GetTargetFormatPalette(),
            pbih->biSizeImage);
...
void VDDubVideoProcessorDisplay::UpdateDecompressedVideo(const void *data, uint32 size, bool isKey)
{
  ...
  if (!mbVideoDecompressorPending && size)
  {
    try
    {
      ...
      if (mpVideoDecompressor->GetAlpha())
        mVideoDecompBuffer.info.alpha_type = FilterModPixmapInfo::kAlphaMask;
      int variant = mpVideoDecompressor->GetTargetFormatVariant();
...
bool VDDubVideoProcessorDisplay::AsyncReinitDisplayCallback(
  int    pass,
  sint64 timelinePos,
  void * pThisAsVoid,
  void *,
...
bool VDDubVideoProcessorDisplay::StaticAsyncUpdateOutputCallback(
  int    pass,
  sint64 timelinePos,
  void * pThisAsVoid,
  void * pBuffer,
  bool   aborting)
{
  return ((VDDubVideoProcessorDisplay *)pThisAsVoid)
    ->AsyncUpdateOutputCallback(pass, timelinePos, (VDRenderOutputBuffer *)pBuffer, aborting);
}

bool VDDubVideoProcessorDisplay::StaticAsyncUpdateInputCallback(
  int    pass,
  sint64 timelinePos,
  void * pThisAsVoid,
  void * pos,
...
bool VDDubVideoProcessorDisplay::AsyncUpdateOutputCallback(
  int                   pass,
  VDPosition            timelinePos,
    ...
    if (nFieldMode)
    {
      switch (nFieldMode)
      {
        ...
        case DubVideoOptions::kPreviewFieldsBobTFF:
          pBuffer->mFlags =
            baseFlags | IVDVideoDisplay::kBobEven | IVDVideoDisplay::kAllFields | IVDVideoDisplay::kAutoFlipFields;
          break;

        case DubVideoOptions::kPreviewFieldsBobBFF:
          pBuffer->mFlags =
            baseFlags | IVDVideoDisplay::kBobOdd | IVDVideoDisplay::kAllFields | IVDVideoDisplay::kAutoFlipFields;
          break;

        case DubVideoOptions::kPreviewFieldsNonIntTFF:
          pBuffer->mFlags = baseFlags | IVDVideoDisplay::kEvenFieldOnly | IVDVideoDisplay::kAutoFlipFields |
                            IVDVideoDisplay::kSequentialFields;
          break;
