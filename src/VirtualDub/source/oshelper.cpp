...
void VDShowHelp(HWND hwnd, const wchar_t *filename)
{
  try
  {
    ...
    if (VDDoesPathExist(helpFileADS.c_str()))
    {
      int rv = MessageBox(
        hwnd,
        "VirtualDub has detected that its help file, VirtualDub.chm, has an Internet Explorer download location marker "
        "on it. This may prevent the help file from being displayed properly, resulting in \"Action canceled\" errors "
        "being displayed. Would you like to remove it?",
        "VirtualDub warning",
...
VDStringW VDLoadStringW32(uint32 uID, bool doSubstitutions)
{
  ...
  HRSRC hrsrc =
    FindResourceEx(NULL, RT_STRING, MAKEINTRESOURCE(1 + (uID >> 4)), MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL));
  if (hrsrc)
  {
...
void VDSubstituteStrings(VDStringW &s)
{
  ...
  for (;;)
  {
    ...
    switch (c)
    {
      ...
      case L'm':
        t.append(
          L"Description and updates: <a "
          L"href=\"https://sourceforge.net/p/vdfiltermod/wiki/Home/\">sourceforge.net/projects/vdfiltermod</a>\nBug "
          L"reports: <a "
          L"href=\"https://sourceforge.net/p/vdfiltermod/tickets/\">sourceforge.net/p/vdfiltermod/tickets</a>");
        break;
    }
...
bool VDInitiateSystemShutdownWithUITimeout(VDSystemShutdownMode mode, const wchar_t *reason, uint32 timeout)
{
  ...
  tpInitiateSystemShutdownExW pInitiateSystemShutdownExW =
    (tpInitiateSystemShutdownExW)GetProcAddress(hmodK32, "InitiateSystemShutdownExW");
  if (!pInitiateSystemShutdownExW)
    return false;

  return 0 != pInitiateSystemShutdownExW(
                NULL,
                (LPWSTR)reason,
                timeout,
                TRUE,
...
bool VDEnableCPUTracking()
{
  ...
  if ((rc = RegOpenKeyEx(HKEY_DYN_DATA, "PerfStats\\StartStat", 0, KEY_READ, &hOpen)) == ERROR_SUCCESS)
  {
    // query to get data size
    if ((rc = RegQueryValueEx(hOpen, "KERNEL\\CPUUsage", NULL, &dwType, NULL, &cbData)) == ERROR_SUCCESS)
    {
      pByte = (LPBYTE)allocmem(cbData);

      rc = RegQueryValueEx(hOpen, "KERNEL\\CPUUsage", NULL, &dwType, pByte, &cbData);
...
bool VDDisableCPUTracking()
{
  ...
  if ((rc = RegOpenKeyEx(HKEY_DYN_DATA, "PerfStats\\StopStat", 0, KEY_READ, &hOpen)) == ERROR_SUCCESS)
  {
    // query to get data size
    if ((rc = RegQueryValueEx(hOpen, "KERNEL\\CPUUsage", NULL, &dwType, NULL, &cbData)) == ERROR_SUCCESS)
    {
...
VDCPUUsageReader::VDCPUUsageReader() : hkeyKernelCPU(NULL) {}
