...
void VDStreamInterleaver::InitStream(
  int    stream,
  sint32 nPreload,
...
VDStreamInterleaver::Action VDStreamInterleaver::PushStreams()
{
  ...
  for (;;)
  {
    ...
    for (int i = mNonIntStream; i < nStreams; ++i)
    {
      ...
      //			VDDEBUG("Dub/Interleaver: Stream #%d: feeding %d samples (%4I64x, %I64d + %I64d ->
...
VDLoopThrottle::VDLoopThrottle()
  : mThrottleFactor(1.0f), mActivityRatio(1.0f), mWaitDepth(0), mWaitTime(0), mLastTime(0), mbLastTimeValid(false),
    mWindowIndex(0), mWaitTimeWindowSum(0), mActiveTimeWindowSum(0), mSuspendState(false), mSuspendRequested(false)
{
