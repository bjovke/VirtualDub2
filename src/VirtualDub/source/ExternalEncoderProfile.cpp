...
VDExtEncProfile::VDExtEncProfile()
  : mOutputFilename(L"%(outputname).audio"), mType(kVDExtEncType_Audio), mInputFormat(kVDExtEncInputFormat_Raw),
    mbCheckReturnCode(true), mbLogStdout(true), mbLogStderr(false), mbPredeleteOutputFile(false),
    mbBypassCompression(false)
{}

VDExtEncSet::VDExtEncSet() : mbProcessPartialOutput(false), mbUseOutputAsTemp(false) {}
...
{
  for (ExtEncProfiles::const_iterator it(g_VDExtEncProfiles.begin()), itEnd(g_VDExtEncProfiles.end()); it != itEnd;
       ++it)
  {
...
{
  for (ExtEncSets::const_iterator it(g_VDExtEncSets.begin()), itEnd(g_VDExtEncSets.end()); it != itEnd; ++it)
  {
    VDExtEncSet *eset = *it;
...
void VDLoadExternalEncoderProfiles()
{
  ...
  while (const char *name = keyIt.Next())
  {
    if (!strncmp(name, "Profile ", 8))
    {
      ...
      profile->mInputFormat =
        (VDExtEncInputFormat)subKey.getEnumInt("Input Format", kVDExtEncInputFormatCount, kVDExtEncInputFormat_Raw);
