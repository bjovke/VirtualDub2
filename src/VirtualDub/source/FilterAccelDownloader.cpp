...
VDFilterAccelDownloader::VDFilterAccelDownloader() : mpReadbackBuffer(NULL) {}
...
void VDFilterAccelDownloader::Init(
  VDFilterAccelEngine * engine,
  ...
  const VDPixmapLayout *sourceLayoutOverride)
{
  ...
  if (!mpReadbackBuffer)
    throw MyError(
      "Unable to allocate 3D acceleration readback buffer for frame size: %ux%u", outputLayout.w, outputLayout.h);
}
...
VDFilterAccelDownloader::RunResult VDFilterAccelDownloader::RunRequests(const uint32 *batchNumberLimit, int index)
{
  ...
  mpEngine->BeginDownload(
    &mDownloadMsg, dstbuf, mLayout, srcbuf, mSourceLayout.format == nsVDXPixmap::kPixFormat_VDXA_YUV, mpReadbackBuffer);
...
bool VDFilterAccelDownloader::InitNewRequest(
...
void VDFilterAccelDownloader::StaticInitCallback(
  VDFilterAccelEngineDispatchQueue *queue,
  VDFilterAccelEngineMessage *      message)
{
  CallbackMsg &msg = *static_cast<CallbackMsg *>(message);

  msg.mpThis->mpReadbackBuffer = msg.mpThis->mpEngine->CreateReadbackBuffer(
    msg.mpThis->mLayout.w, msg.mpThis->mLayout.h, msg.mpThis->mSourceLayout.format);
}

void VDFilterAccelDownloader::StaticShutdownCallback(
  VDFilterAccelEngineDispatchQueue *queue,
  VDFilterAccelEngineMessage *      message)
{
  CallbackMsg &msg = *static_cast<CallbackMsg *>(message);
...
void VDFilterAccelDownloader::StaticCleanupCallback(
  VDFilterAccelEngineDispatchQueue *queue,
  VDFilterAccelEngineMessage *      message)
{
