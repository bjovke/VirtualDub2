...
FrameserverSession::~FrameserverSession()
{
  ...
  if (hArena != INVALID_HANDLE_VALUE)
    CloseHandle(hArena);
}
...
Frameserver::Frameserver(
  ...
  : aSrc(audio), vSrc(video), mSubset(subset)
{
  opt  = xopt;
  hwnd = hwndParent;
...
void Frameserver::Go(IVDubServerLink *ivdsl, char *name)
{
      ...
      if (!vSrc->setTargetFormat(nsVDPixmap::kPixFormat_RGB888))
        if (!vSrc->setTargetFormat(nsVDPixmap::kPixFormat_XRGB1555))
          if (!vSrc->setTargetFormat(nsVDPixmap::kPixFormat_Pal8))
            throw MyError(
              "The decompression codec cannot decompress to an RGB format. This is very unusual. Check that any "
              "\"Force YUY2\" options are not enabled in the codec's properties.");
  ...
  filters.initLinearChain(
    NULL, 0, &g_filterChain, mpVideoFrameSource, px.w, px.h, px, px.palette, vInfo.mFrameRatePreFilter, -1, srcFAR);
  ...
  mVideoFrameMap.Init(
    vsrcs,
    vInfo.start_src,
    vInfo.mFrameRateTimeline / vInfo.mFrameRate,
    &mSubset,
    vInfo.end_dst,
    opt->video.mbUseSmartRendering,
    opt->video.mode == DubVideoOptions::M_NONE,
    opt->video.mbPreserveEmptyFrames,
    &filters,
    false,
    false);
  ...
  if (aSrc)
    AudioTranslateVideoSubset(
      audioset,
      videoset,
      vInfo.mFrameRateTimeline,
      aSrc->getWaveFormat(),
      !opt->audio.fEndAudio && (videoset.empty() || videoset.back().end() == pVSS->getEnd()) ? aSrc->getEnd() : 0,
      NULL);
  ...
  mFrameSize =
    VDPixmapCreateLinearLayout(mFrameLayout, nsVDPixmap::kPixFormat_RGB888, outputLayout.w, outputLayout.h, 4);
  ...
  if (
    hwndStatus =
      CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_SERVER), hwnd, Frameserver::StatusDlgProc, (LPARAM)this))
  {
  ...
  if (server_index < 0)
    throw MyError("Couldn't create frameserver");
}
    ...
    case WM_INITDIALOG:
...

      {
        ...
        if (
          RegOpenKeyEx(
            HKEY_CLASSES_ROOT,
            "CLSID\\{894288E0-0948-11D2-8109-004845000EB5}\\InProcServer32\\AVIFile",
            0,
            KEY_QUERY_VALUE,
            &hkey) == ERROR_SUCCESS)
        {
...
LRESULT Frameserver::SessionAudio(LPARAM lParam, WPARAM lStart)
{
  ...
  if (cbBuffer > fs->arena_size - 8)
    cbBuffer = fs->arena_size - 8;
...
void ActivateFrameServerDialog(HWND hwnd, const char *server)
{
  static wchar_t fileFilters[] =
    L"VirtualDub AVIFile signpost (*.vdr,*.avi)\0"
    L"*.vdr;*.avi\0"
    L"All files\0"
    L"*.*\0";
  ...
  else
  {
    if (!DialogBoxParam(
          ...
          MAKEINTRESOURCE(IDD_SERVER_SETUP),
          hwnd,
          FrameServerSetupDlgProc,
          (LPARAM)szServerName))
      return;
  }
  ...
  try
  {
    vdrefptr<Frameserver> fs(
      new Frameserver(inputVideo, inputAudio, hwnd, &g_dubOpts, g_project->GetTimeline().GetSubset()));

    if (!server || !*server)
    {
      const VDStringW fname(VDGetSaveFileName(
        kFileDialog_Signpost,
        (VDGUIHandle)hwnd,
        L"Save .VDR signpost for AVIFile handler",
        fileFilters,
        g_prefs.main.fAttachExtension ? L"vdr" : NULL,
        0,
        0));
