...
struct ACMEnumeratorData
{
  ...
  ACMEnumeratorData() : mbCurrentSelectedFormatMatchesHint(false) {}
};
...
BOOL CALLBACK ACMFormatEnumerator(HACMDRIVERID hadid, LPACMFORMATDETAILS pafd, DWORD_PTR dwInstance, DWORD fdwSupport)
{
  ...
  {
    if (
      pafd->pwfx->wFormatTag == WAVE_FORMAT_PCM ||
      (pafd->pwfx->cbSize == pData->pwfexSelect->cbSize &&
  ...
  {
    pafe->fCompatible =
      !acmStreamOpen(NULL, pData->had, pData->pwfexSrc, pafd->pwfx, NULL, 0, 0, ACM_STREAMOPENF_QUERY) ||
      !acmStreamOpen(
        NULL, pData->had, pData->pwfexSrc, pafd->pwfx, NULL, 0, 0, ACM_STREAMOPENF_QUERY | ACM_STREAMOPENF_NONREALTIME);

    if (
      pafd->pwfx->nChannels == pData->pwfexSrc->nChannels &&
...
BOOL /*ACMFORMATTAGENUMCB*/ CALLBACK
ACMFormatTagEnumerator(HACMDRIVERID hadid, LPACMFORMATTAGDETAILS paftd, DWORD_PTR dwInstance, DWORD fdwSupport)
{
  ACMEnumeratorData *pData = (ACMEnumeratorData *)dwInstance;
    ...
    if (index != LB_ERR)
    {
      ...
      if (!pData->fAttemptedWeird && pData->pwfexSrc)
      {
        ...
        if (!acmFormatSuggest(
              ...
              pData->pwfex,
              pData->cbwfex,
              ACM_FORMATSUGGESTF_NCHANNELS | ACM_FORMATSUGGESTF_NSAMPLESPERSEC | ACM_FORMATSUGGESTF_WFORMATTAG))
        {
          afd.dwFormatIndex = 0;
          afd.fdwSupport    = 0;
...
static void AudioChooseDisplaySpecs(HWND hdlg, WAVEFORMATEX *pwfex)
{
  ...
  if (pwfex)
    wsprintf(buf, "%ld bytes", pwfex->nBlockAlign);
  SetDlgItemText(hdlg, IDC_STATIC_BYTESPERBLOCK, buf);

  if (pwfex)
    wsprintf(buf, "%ld bytes/sec", pwfex->nAvgBytesPerSec);
...
WAVEFORMATEX *AudioChooseCompressor(
  HWND           hwndParent,
  WAVEFORMATEX * pwfexOld,
  WAVEFORMATEX * pwfexSrc,
  VDStringA &    shortNameHint,
  vdblock<char> &config,
  bool           enable_plugin)
{
  ACMChooserData data;
  ...
  if (!DialogBoxParam(
        g_hInst, MAKEINTRESOURCE(IDD_AUDIOCOMPRESSION), hwndParent, AudioChooseCompressionDlgProc, (LPARAM)&data))
    return pwfexOld;
  else
  {
    if (pwfexOld)
