...
bool CheckFormatSizeCompatibility(int format, int w, int h)
{
  ...
  if (
    (w & ((1 << formatInfo.qwbits) - 1)) || (h & ((1 << formatInfo.qhbits) - 1)) ||
    (w & ((1 << formatInfo.auxwbits) - 1)) || (h & ((1 << formatInfo.auxhbits) - 1)))
  {
    return false;
  }
...
public:
  ...
  VDEvent<IDubber, bool> &Stopped()
  {
    return mStoppedEvent;
  }
};
...
Dubber::Dubber(DubOptions *xopt)
  : mpIOThread(0), mpIODirect(0), mIOThreadCounter(0), mpAudioFilterGraph(NULL), mStopLock(0), mpVideoPipe(NULL),
    mpVideoRequestQueue(NULL), mLastProcessingThreadCounter(0), mProcessingThreadFailCount(0), mLastIOThreadCounter(0),
    mIOThreadFailCount(0), mbBackground(false)
{
  mOptions = *xopt;
...
void VDTranslateSubsetDirectMode(
  FrameSubset &          dst,
  const FrameSubset &    src,
  IVDVideoSource *const *pVideoSources,
  VDPosition &           selectionStart,
  VDPosition &           selectionEnd)
{
    ...
    if (!itNew->bMask)
    {
      if (start < itNew->start)
      {
        ...
        while (it2 != dst.begin())
        {
          ...
          if (it2->start >= start || !it2->bMask)
          { // within extension range: absorb
          ...
          { // before extension range and masked: split merge
...
void InitAudioStreamValuesStatic(DubAudioStreamInfo &aInfo, AudioSource *audio, const DubOptions *opt)
{
  ...
  if (opt->audio.mode > DubAudioOptions::M_NONE)
  {
    ...
    if (change_precision || change_layout)
    {
      ...
      aInfo.is_stereo =
        opt->audio.newChannels == DubAudioOptions::C_STEREO ||
      ...
      aInfo.single_channel =
        (opt->audio.newChannels == DubAudioOptions::C_MONOLEFT ||
         opt->audio.newChannels == DubAudioOptions::C_MONORIGHT);
    }
  }
}
...
void Dubber::InitAudioConversionChain()
{
  ...
  for (uint32 i = 0; i < audioSourceCount; ++i)
  {
    ...
    else
    {
      ...
      if (!(audioStream = new_nothrow AudioStreamSource(
              asrc, asrc->getEnd() - aInfo.start_src, mOptions.audio.mode > DubAudioOptions::M_NONE, aInfo.start_us)))
        throw MyMemoryError();
    ...
    if (i > 0)
    {
      ...
      {
        audioStream =
          new_nothrow AudioStreamConverter(audioStream, format1->mSampleBits > 8, format1->mChannels > 1, false);
        mAudioStreams.push_back(audioStream);
      }
  ...
  if (
    !mOptions.audio.fEndAudio &&
    (inputSubsetActive->empty() || inputSubsetActive->back().end() >= vInfo.mTimelineSourceLength))
    applyTail = true;

  if (!(audioStream = new_nothrow AudioSubset(
          sourceStreams,
          mOptions.audio.mbApplyVideoTimeline ? inputSubsetActive : NULL,
          vInfo.mFrameRateTimeline,
          offset,
          applyTail)))
    throw MyMemoryError();
  ...
  if (!bUseAudioFilterGraph)
  {
    ...
    if (aInfo.resampling)
    {
      if (!(audioStream = new_nothrow AudioStreamResampler(
              audioStream,
              mOptions.audio.new_rate ? mOptions.audio.new_rate : aSrc->getWaveFormat()->mSamplingRate,
              mOptions.audio.fHighQuality)))
        throw MyMemoryError();
  ...
  {
    if (!(pCompressor = new_nothrow AudioCompressor(
            audioStream,
            (const VDWaveFormat *)&*mAudioCompressionFormat,
            mAudioCompressionFormat.size(),
            mAudioCompressionFormatHint.c_str(),
            mAudioCompressionConfig)))
      throw MyMemoryError();
  ...
  if (pCompressor && mpOutputSystem)
  {
    ...
    if (mbDoVideo)
    {
      ...
      if (!sc.use_offsets)
        pCompressor->SkipSource(si.initial_padding);
    }
...
void Dubber::InitOutputFile()
{
  ...
  if (mbDoAudio)
  {
    ...
    mpOutputSystem->SetAudio(
      si, audioStream->GetFormat(), audioStream->GetFormatLen(), mOptions.audio.enabled, audioStream->IsVBR());
  }
  ...
  if (mbDoVideo)
  {
    ...
    if (!make.error.empty())
      throw MyError(make.error.c_str());
    ...
    if (mpVideoCompressor && make.use_vc_format)
    {
      ...
      if (!mpVideoCompressor->Query(&driverLayout, NULL))
        throw MyError(
          "Unable to initialize video compression. Check that the video codec is compatible with the output video "
          "frame size and that the settings are correct, or try a different one.");
    ...
    if (!driverLayout.format)
    {
      ...
      else if (mOptions.video.mode == DubVideoOptions::M_FASTREPACK)
      {
        ...
        if (make.srcDib.empty())
          throw MyError(
            "Unable to initialize video compression: The selected output format is not compatible with the Windows "
            "video codec API. Choose a different format.");
    ...
    if (mpVideoCompressor)
    {
      ...
      if (mOptions.video.mbUseSmartRendering)
      {
        ...
        stream_mode = IVDXStreamSourceV5::kStreamModeDirectCopy | IVDXStreamSourceV5::kStreamModeUncompress |
                      IVDXStreamSourceV5::kStreamModePlayForward;
        ...
        if (!qresult)
          throw MyError(
            "Cannot initialize smart rendering: The selected video codec is able to compress the source video, but "
            "cannot match the same compressed format.");
      ...
      if (driverLayout.format)
      {
        ...
        mpVideoCompressor->Start(
          driverLayout, outputFormatInfo, &*outputFormat, outputFormat.size(), vInfo.mFrameRate, vInfo.end_proc_dst);
        outputFormat.assign(
          (const VDAVIBitmapInfoHeader *)mpVideoCompressor->GetOutputFormat(),
          mpVideoCompressor->GetOutputFormatSize());
      }
      else
      {
        mpVideoCompressor->Start(
          &*mpCompressorVideoFormat,
          mpCompressorVideoFormat.size(),
          &*outputFormat,
          outputFormat.size(),
          vInfo.mFrameRate,
          vInfo.end_proc_dst);
...

    {
      ...
      const VDPixmapLayout &bmout =
        mOptions.video.mode >= DubVideoOptions::M_FULL ? filters.GetOutputLayout() : filters.GetInputLayout();
      ...
      {
        VDMakeBitmapCompatiblePixmapLayout(
          mVideoFilterOutputPixmapLayout, bmout.w, bmout.h, outputFormatID, outputVariantID, bmout.palette);
...
void Dubber::InitDirectDraw()
{
  ...
  if (mOptions.video.mode == DubVideoOptions::M_SLOWREPACK)
  {
    static const int kFormats[] = {
      nsVDPixmap::kPixFormat_YUV420_Planar, nsVDPixmap::kPixFormat_YUV422_UYVY, nsVDPixmap::kPixFormat_YUV422_YUYV};
    ...
    for (int i = 0; i < sizeof(kFormats) / sizeof(kFormats[0]); ++i)
    {
      ...
      if (it == itEnd)
      {
        if (mpInputDisplay->SetSource(
              false, mVideoSources.front()->getTargetFormat(), 0, 0, false, mOptions.video.previewFieldMode > 0))
        {
...
void Dubber::InitSelectInputFormat()
{
  ...
  VDLogAppMessage(
    kVDLogInfo,
    kVDST_Dub,
    (mOptions.video.mode > DubVideoOptions::M_FASTREPACK) ? kVDM_FullUsingInputFormat : kVDM_SlowRecompressUsingFormat,
    1,
    &s);
}
...
void Dubber::Init(
  ...
  AudioSource *const *   pAudioSources,
  uint32                 nAudioSources,
  IVDDubberOutputSystem *pOutputSystem,
  void *                 videoCompVars,
  const FrameSubset *    pfs,
  const VDFraction &     frameRateTimeline)
{
  ...
  if (
    pOutputSystem->IsVideoCompressionEnabled() && pOutputSystem->AcceptsVideo() &&
    mOptions.video.mode > DubVideoOptions::M_NONE && compVars && (compVars->dwFlags & ICMF_COMPVARS_VALID) &&
    compVars->driver)
    mpVideoCompressor =
      VDCreateVideoCompressorVCM(compVars->driver, compVars->lDataRate * 1024, compVars->lQ, compVars->lKey, false);
  ...
  VDConvertSelectionTimesToFrames(
    mOptions, *inputSubsetActive, frameRateTimeline, selectionStartFrame, selectionEndFrame);
  ...
  if (!mVideoSources.empty() && mOptions.video.mode == DubVideoOptions::M_NONE)
  {
    ...
    VDTranslateSubsetDirectMode(
      *newSubset, *inputSubsetActive, mVideoSources.data(), selectionStartFrame, selectionEndFrame);
  ...
  AudioSource *audioSrc = mAudioSources.empty() ? NULL : mAudioSources.front();
  InitVideoStreamValuesStatic(
    vInfo, vSrc, audioSrc, &mOptions, inputSubsetActive, &selectionStartFrame, &selectionEndFrame);
  InitAudioStreamValuesStatic(aInfo, audioSrc, &mOptions);
  ...
  filters.prepareLinearChain(
    &g_filterChain, px.w, px.h, px, vInfo.mFrameRatePreFilter, srcFrames, vSrc->getPixelAspectRatio());
  ...
  if (mbDoVideo && mOptions.video.mode >= DubVideoOptions::M_FULL)
  {
    ...
    filters.initLinearChain(
      mProcessThread.GetVideoFilterScheduler(),
      fPreview ? VDXFilterStateInfo::kStateRealTime | VDXFilterStateInfo::kStatePreview : 0,
      &g_filterChain,
      mpVideoFrameSource,
      px.w,
      px.h,
      px,
      px.palette,
      vInfo.mFrameRatePreFilter,
      srcFrames,
      vSrc->getPixelAspectRatio());
    ...
    if (!CheckFormatSizeCompatibility(outputFormat, output.w, output.h))
    {
      ...
      throw MyError(
        "The output frame size is not compatible with the selected output format. (%dx%d, %s)",
        output.w,
        output.h,
        formatInfo.name);
    }
...
void Dubber::Go(int iPriority)
{
  ...
  if (!mVideoSources.empty() && mVideoSources[0]->isSyncDecode())
  {
    if (!(mpIODirect = new_nothrow VDDubIOThread(
            this, mVideoSources, 0, mpVideoPipe, NULL, aInfo, vInfo, mIOThreadCounter, mpVideoRequestQueue, fPreview)))
      throw MyMemoryError();
  }
...
void Dubber::UpdateFrames()
{
  ...
  if (mLiveLockMessages < kLiveLockMessageLimit && !mStopLock)
  {
    ...
    else if (mLastIOThreadCounter && (curTime - mIOThreadFailCount - 30000) < 3600000)
    { // 30s to 1hr
    ...
    else if (mLastProcessingThreadCounter && (curTime - mProcessingThreadFailCount - 30000) < 3600000)
    { // 30s to 1hr
...
void MakeOutputFormat::init(DubOptions &opts, IVDVideoSource *vs)
{
  ...
  if (mode <= DubVideoOptions::M_FASTREPACK)
  {
    option = 0;
    if (vs)
      dec = VDPixmapFormatNormalize(vs->getSourceFormat());
  }
  else
  {
    if (vs)
      dec = VDPixmapFormatNormalize(vs->getTargetFormat());
  }
...
{
  if (mode == DubVideoOptions::M_NONE)
    return;
  if (!compVars)
    return;
...
void MakeOutputFormat::combine()
{
  ...
  if (!out.format)
    out.format = s.format;
  if (!out.colorSpaceMode)
    out.colorSpaceMode = s.colorSpaceMode;
  if (!out.colorRangeMode)
    out.colorRangeMode = s.colorRangeMode;
...
{
  if (os && os->IsVideoImageOutputRequired())
    return; // not going to use DIB
...

  {
    for (int test = 0; test < test_list.size(); test++)
    {
      ...
      int              svariant  = 0;
      if (vc_fccHandler)
        fourcc_codec_input(vc_fccHandler, sformat, svariant);

      for (int variant = 0; variant <= variants; ++variant)
      {
        if (variant == 0 && format == sformat)
          variant = svariant;
        if (variant == 0)
          continue;
        if (!accept_format(format, variant))
          continue;
  ...
  {
    error =
      "Unable to initialize video compression. Check that the video codec is compatible with the output video frame "
      "size and that the settings are correct, or try a different one.";
  }
  else
  {
    error =
      "Unable to initialize video compression: The selected output format is not compatible with the Windows video "
      "codec API. Choose a different format.";
  }
}
...
bool MakeOutputFormat::accept_format(int format, int variant)
{
  ...
  if (!os_format.empty() && os_format != "avi")
    switch (format)
    {
      ...
      case kPixFormat_RGB888:
        ...
        break;
      case kPixFormat_XRGB8888:
        if (variant == kBitmapVariant_0_rgba)
          return false;
        break;
  ...
  if (os_format == "mov" || os_format == "mov+faststart" || os_format == "mp4" || os_format == "mp4+faststart")
    switch (format)
    {
      case kPixFormat_Y16:
      case kPixFormat_Y8:
      case kPixFormat_Y8_FR:
        return false;
      case kPixFormat_RGB888:
        if (variant != kBitmapVariant_24BG)
          return false;
        break;
    }

  if (os_format == "matroska")
    switch (format)
    {
      case kPixFormat_RGB888:
        if (variant != kBitmapVariant_RV24)
          return false;
        break;
      // disable some
      case kPixFormat_B64A:
