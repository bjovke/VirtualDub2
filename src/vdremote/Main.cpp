...
const GUID CDECL CLSID_AVIFile = {0x00020000, 0, 0, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46};

const GUID CDECL another_IID_IAVIFile = {0x00020020, 0, 0, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46};

const GUID CDECL another_IID_IAVIStream = {0x00020021, 0, 0, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46};

const GUID CDECL CLSID_Avisynth = {0xE6D6B700, 0x124D, 0x11D4, 0x86, 0xF3, 0xDB, 0x80, 0xAF, 0xD9, 0x87, 0x78};
...
const GUID CDECL CLSID_CAVIFileRemote = {0x894288e0, 0x0948, 0x11d2, {0x81, 0x09, 0x00, 0x48, 0x45, 0x00, 0x0e, 0xb5}};
const GUID CDECL
  CLSID_CAVIStreamRemote = {0x91379540, 0x0948, 0x11d2, {0x81, 0x09, 0x00, 0x48, 0x45, 0x00, 0x0e, 0xb5}};
...
STDAPI DllCanUnloadNow()
{
  _RPT2(
    0,
    "DllCanUnloadNow(): CAVIFileRemote %ld, CAVIStreamRemote %ld\n",
    CAVIFileRemote::gRefCnt,
    CAVIStreamRemote::gRefCnt);
...
STDMETHODIMP CAVIFileRemote::Load(LPCOLESTR lpszFileName, DWORD grfMode)
{
  ...
  WideCharToMultiByte(
    AreFileApisANSI() ? CP_ACP : CP_OEMCP, 0, lpszFileName, -1, filename, sizeof filename, NULL, NULL);
...
STDMETHODIMP CAVIFileRemote::Save(LPCSTR szFile, AVICOMPRESSOPTIONS FAR *lpOptions, AVISAVECALLBACK lpfnCallback)
{
  _RPT1(0, "%p->CAVIFileRemote::Save()\n", this);
  ...
  try
  {
    ...
    if (16 == mmioRead(hmmio, buf, 16))
    {
      ...
      if (!_stricmp(buf, "#avisynth"))
      {
        ...
        if (FAILED(
              CoCreateInstance(CLSID_Avisynth, NULL, CLSCTX_INPROC_SERVER, another_IID_IAVIFile, (void **)&pafTunnel)))
          return (HRESULT)E_FAIL;
    ...
    if (mmerr == MMIOERR_CHUNKNOTFOUND)
    {
      ...
      if (FAILED(
            CoCreateInstance(CLSID_AVIFile, NULL, CLSCTX_INPROC_SERVER, another_IID_IAVIFile, (void **)&pafTunnel)))
        return (HRESULT)E_FAIL;
    ...
    else if (mmerr != MMSYSERR_NOERROR)
...
STDMETHODIMP CAVIStreamRemote::Read(
  LONG   lStart,
  LONG   lSamples,
  LPVOID lpBuffer,
  LONG   cbBuffer,
  ...
  else
  {
    ...
    if (!lpBuffer)
    {
      if (plSamples)
        *plSamples = 1;
      if (plBytes)
        *plBytes = bmihFormat->biSizeImage;
    }
...
STDMETHODIMP CAVIStreamRemote::Write(
  LONG   lStart,
  LONG   lSamples,
  LPVOID lpBuffer,
  LONG   cbBuffer,
