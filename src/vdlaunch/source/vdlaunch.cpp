...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
  ...
  if (
    !DuplicateHandle(hProcess, hStdInput, hProcess, &hStdInput2, 0, TRUE, DUPLICATE_SAME_ACCESS) ||
  ...
  if (IsUnicodeAPIAvailable())
  {
    ...
    const wchar_t *exepathW =
      ipcData->programOffsetW ? (const wchar_t *)((const char *)ipcData + ipcData->programOffsetW) : NULL;
    const wchar_t *commandLineW =
      ipcData->commandLineOffsetW ? (const wchar_t *)((const char *)ipcData + ipcData->commandLineOffsetW) : NULL;
  ...
  if (success)
  {
    ...
    HANDLE h[2] = {pi.hProcess, hParentProcess};
