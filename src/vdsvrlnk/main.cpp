...
typedef NET_API_STATUS(APIENTRY *LPNETSERVERGETINFONT)(LPWSTR servername, DWORD level, LPBYTE *bufptr);
...
static void InitComputerAlias()
{
  ...
  /*
      ...
      if (hDll = LoadLibrary("svrapi.dll")) {
              ...
              if (fpNetServerGetInfo && !((LPNETSERVERGETINFO95)fpNetServerGetInfo)(NULL, 50, (char *)&si, sizeof si,
     &needed_size)) { strncpy(heap->comp_name, si.si_95.sv50_name, sizeof heap->comp_name); heap->comp_name[sizeof
     heap->comp_name-1] = 0;
  ...
  */
  ...
  if (!fpNetServerGetInfo && (hDll = LoadLibrary("netapi32.dll")))
  {
    ...
    {
      WideCharToMultiByte(
        CP_ACP, 0, (LPWSTR)psi100->sv100_name, -1, heap->comp_name, sizeof heap->comp_name, NULL, NULL);
...
static BOOL InitSharedSpace()
{
  ...
  hHeap = CreateFileMapping(
    INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(VDubSharedHeap), "VirtualDub Server System Window");
...
BOOL CVDubAnimConnection::init()
{
  ...
  hArena = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, lArenaSize, buf);
...
BOOL CVDubAnimConnection::readStreamInfo(AVISTREAMINFO *lpsi, BOOL fAudio, long *lpFirst, long *lpLast)
{
  if (
    VDSRVERR_OK ==
    SendMessage((HWND)LongToHandle(frameserver->hwndServer), VDSRVM_REQ_STREAMINFO, !!fAudio, dwSessionID))
  {
    if (lpsi)
...
int CVDubAnimConnection::readVideo(long lSample, void *lpBuffer)
{
  ...
  if (
    VDSRVERR_OK !=
    (err = SendMessage((HWND)LongToHandle(frameserver->hwndServer), VDSRVM_REQ_FRAME, lSample, dwSessionID)))
    return err;
...
int CVDubAnimConnection::readAudio(
  long  lSample,
  long  lCount,
  void *lpBuffer,
  long  cbBuffer,
  ...
  long *lplSamples)
{
  ...
  *(long *)(arena + 4) = cbBuffer;
  ...
  if (
    VDSRVERR_OK != (err = SendMessage(
                      (HWND)LongToHandle(frameserver->hwndServer),
                      lpBuffer ? VDSRVM_REQ_AUDIO : VDSRVM_REQ_AUDIOINFO,
                      lSample,
                      dwSessionID)))
    return err;
