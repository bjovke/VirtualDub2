...
VDDisplayNodeContext3D::VDDisplayNodeContext3D()
  : mpVPTexture(NULL), mpVPTexture2T(NULL), mpVPTexture3T(NULL), mpVFTexture(NULL), mpVFTexture2T(NULL),
    mpVFTexture3T(NULL), mpFPBlit(NULL), mpSSPoint(NULL), mpSSBilinear(NULL)
{}
...
{
  vdsaferelease <<= mpFPBlit, mpVFTexture, mpVFTexture2T, mpVFTexture3T, mpVPTexture, mpVPTexture2T, mpVPTexture3T,
    mpSSBilinear, mpSSPoint;
}
...
VDDisplayImageNode3D::VDDisplayImageNode3D()
  : mpPaletteTex(NULL), mpVF(NULL), mpVP(NULL), mpFP(NULL), mpVB(NULL), mTexWidth(0), mTexHeight(0), mTex2Width(0),
    mTex2Height(0), mbBilinear(true)
{
...
bool VDDisplayImageNode3D::Init(IVDTContext &ctx, VDDisplayNodeContext3D &dctx, uint32 w, uint32 h, uint32 format)
{
  ...
  switch (format)
  {
    ...
    case nsVDPixmap::kPixFormat_XRGB1555: {
      ...
      else
      {
        ...
        if (l8a8 || r8g8)
        {
          ...
          if (!ctx.CreateFragmentProgram(
                kVDTPF_MultiTarget,
                l8a8 ? VDTDataView(g_VDDispFP_BlitRGB16_L8A8) : VDTDataView(g_VDDispFP_BlitRGB16_R8G8),
                &mpFP))
          {
            Shutdown();
            return false;
    ...
    case nsVDPixmap::kPixFormat_YUV422_YUYV_709_FR:
      ...
      switch (format)
      {
        case nsVDPixmap::kPixFormat_YUV422_UYVY:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitUYVY_601_LR) : VDTDataView(g_VDDispFP_BlitUYVYRBSwap_601_LR);
          break;

        case nsVDPixmap::kPixFormat_YUV422_UYVY_FR:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitUYVY_601_FR) : VDTDataView(g_VDDispFP_BlitUYVYRBSwap_601_FR);
          break;

        case nsVDPixmap::kPixFormat_YUV422_UYVY_709:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitUYVY_709_LR) : VDTDataView(g_VDDispFP_BlitUYVYRBSwap_709_LR);
          break;

        case nsVDPixmap::kPixFormat_YUV422_UYVY_709_FR:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitUYVY_709_FR) : VDTDataView(g_VDDispFP_BlitUYVYRBSwap_709_FR);
          break;

        case nsVDPixmap::kPixFormat_YUV422_YUYV:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitYUYV_601_LR) : VDTDataView(g_VDDispFP_BlitYUYVRBSwap_601_LR);
          break;

        case nsVDPixmap::kPixFormat_YUV422_YUYV_FR:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitYUYV_601_FR) : VDTDataView(g_VDDispFP_BlitYUYVRBSwap_601_FR);
          break;

        case nsVDPixmap::kPixFormat_YUV422_YUYV_709:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitYUYV_709_LR) : VDTDataView(g_VDDispFP_BlitYUYVRBSwap_709_LR);
          break;

        case nsVDPixmap::kPixFormat_YUV422_YUYV_709_FR:
          program =
            mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitYUYV_709_FR) : VDTDataView(g_VDDispFP_BlitYUYVRBSwap_709_FR);
          break;
      }
    ...
    case nsVDPixmap::kPixFormat_Y8_FR:
      if (ctx.IsFormatSupportedTexture2D(kVDTF_R8))
      {
        ...
        if (!ctx.CreateFragmentProgram(
              kVDTPF_MultiTarget,
              format == nsVDPixmap::kPixFormat_Y8_FR ? VDTDataView(g_VDDispFP_BlitY_FR) :
    ...
    case nsVDPixmap::kPixFormat_Pal8:
      if (ctx.IsFormatSupportedTexture2D(kVDTF_R8))
      {
        ...
        if (!ctx.CreateFragmentProgram(
              kVDTPF_MultiTarget,
              mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitPal8RBSwap) : VDTDataView(g_VDDispFP_BlitPal8),
              &mpFP))
        {
          Shutdown();
          return false;
  ...
  if (!mpFP)
  {
    ...
    if (!ctx.CreateFragmentProgram(
          kVDTPF_MultiTarget,
          mbRenderSwapRB ? VDTDataView(g_VDDispFP_BlitRBSwap) : VDTDataView(g_VDDispFP_Blit),
          &mpFP))
    {
      Shutdown();
      return false;
...
void VDDisplayImageNode3D::Load(const VDPixmap &px)
{
  ...
  if (mRenderMode == kRenderMode_BlitPal8)
  {
    ...
    mRenderMode == kRenderMode_BlitY || mRenderMode == kRenderMode_BlitUYVY || mRenderMode == kRenderMode_BlitRGB16 ||
    mRenderMode == kRenderMode_BlitRGB16Direct || mRenderMode == kRenderMode_BlitRGB24)
  {
...
void VDDisplayImageNode3D::Draw(IVDTContext &ctx, VDDisplayNodeContext3D &dctx)
{
  ...
  switch (mRenderMode)
  {
    ...
    case kRenderMode_BlitRGB16: {
      ...
      IVDTTexture *const tex[2] = {mpImageTex[0], mpPaletteTex};
    ...
    case kRenderMode_BlitYCbCr: {
      ...
      IVDTSamplerState *ss[3] = {ss0, ss0, ss0};
...
VDDisplayBufferNode3D::VDDisplayBufferNode3D() : mpVB(NULL), mpRTT(NULL), mpChildNode(NULL), mbLinear(false) {}
...
bool VDDisplayBufferNode3D::Init(
  IVDTContext &           ctx,
  VDDisplayNodeContext3D &dctx,
  uint32                  w,
  uint32                  h,
  bool                    linear,
  VDDisplayNode3D *       child)
{
...
VDDisplayStretchNode3D::VDDisplayStretchNode3D()
  : mpFP(NULL), mpVP(NULL), mpVF(NULL), mpVB(NULL), mpRTTChild(NULL), mpRTTHoriz(NULL), mpFilterTex(NULL),
    mpChildNode(NULL), mSrcW(0), mSrcH(0), mDstW(0), mDstH(0)
{}
...
bool VDDisplayStretchNode3D::Init(
  IVDTContext &           ctx,
  VDDisplayNodeContext3D &dctx,
  uint32                  srcw,
  uint32                  srch,
...
VDDisplayDriver3D::VDDisplayDriver3D()
  : mhwnd(NULL), mpContext(NULL), mpSwapChain(NULL), mpImageNode(NULL), mpRootNode(NULL), mFilterMode(kFilterBilinear),
    mbCompositionTreeDirty(false), mSource()
{}
...
void VDDisplayDriver3D::Shutdown()
{
  ...
  vdsaferelease <<= mpRootNode, mpImageNode, mpSwapChain, mpContext;
...
bool VDDisplayDriver3D::ModifySource(const VDVideoDisplaySourceInfo &info)
{
  ...
  if (
    info.pixmap.w != mSource.pixmap.w || info.pixmap.h != mSource.pixmap.h ||
