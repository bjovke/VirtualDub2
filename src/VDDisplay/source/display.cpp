...
class VDVideoDisplayWindow : public IVDVideoDisplay,
                             public IVDVideoDisplayMinidriverCallback,
                             public VDVideoDisplayClient
{
public:
...
protected:
  ...
  {
    if (mpMiniDriver)
      return mpMiniDriver->GetMaxArea(w, h);
    return false;
  }
...
void VDVideoDisplaySetFeatures(
  bool enableDirectX,
  bool enableDirectXOverlay,
  bool enableTermServ,
  bool enableOpenGL,
  bool enableDirect3D,
  bool enableDirect3DFX,
...
VDVideoDisplayWindow::VDVideoDisplayWindow(HWND hwnd, const CREATESTRUCT &createInfo)
  : mhwnd(hwnd), mhwndChild(NULL), mhLastMonitor(NULL), mhOldPalette(0), mpMiniDriver(0),
    mbMiniDriverSecondarySensitive(false), mbMiniDriverClearOtherMonitors(false), mReinitDisplayTimer(0), mpCB(0),
    mpDrawMode(0), mInhibitRefresh(0), mInhibitPaint(0), mSyncDelta(0.0f), mProcessTick(0),
    mFilterMode(kFilterAnySuitable), mDisplayMode(kDisplayDefault),
    mAccelMode(VDVideoDisplayWindow::sbEnableBackgroundFallback ? kAccelOnlyInForeground : kAccelAlways),
    mbIgnoreMouse(false), mbUseSubrect(false), mbReturnFocus(false), mbFullScreen(false), mFullScreenWidth(0),
    mFullScreenHeight(0), mFullScreenRefreshRate(0), mbDestRectEnabled(false), mDestRect(0, 0, 0, 0),
    mPixelSharpnessX(1.0f), mPixelSharpnessY(1.0f), mBackgroundColor(0), mpActiveFrame(NULL), mpLastFrame(NULL)
{
...
bool VDVideoDisplayWindow::SetSource(
  bool            bAutoUpdate,
  const VDPixmap &src,
  void *          pObject,
  ptrdiff_t       offset,
...
bool VDVideoDisplayWindow::SetSourcePersistent(
  bool            bAutoUpdate,
  const VDPixmap &src,
  bool            bAllowConversion,
  bool            bInterlaced)
...
void VDVideoDisplayWindow::PostBuffer(VDVideoDisplayFrame *p)
{
  ...
  {
    if (mPendingFrames.find(p) != mPendingFrames.end())
      return;
...
void VDVideoDisplayWindow::OnChildPaint()
{
  ...
  if (mpMiniDriver)
  {
    if (mpMiniDriver->IsValid())
      ...
      mSource.pixmap.data && mSource.bPersistent && !mpMiniDriver->Update(IVDVideoDisplayMinidriver::kModeAllFields))
      bDisplayOK = true;
  }
    ...
    case WM_SIZE:
      if (mhwndChild)
        SetWindowPos(
          ...
          LOWORD(lParam),
          HIWORD(lParam),
          SWP_NOMOVE | SWP_NOCOPYBITS | SWP_NOZORDER | SWP_NOACTIVATE);
      break;
    case WM_TIMER:
    ...
    if ((!sbEnableTS || !sbEnableTS3D || !isTermServ) && !isDrawing)
    {
      if (mAccelMode != kAccelOnlyInForeground || !mSource.bAllowConversion || bIsForeground)
      {
        ...
        if (sbEnableDX)
        {
          ...
          if (
            sbEnableSecondaryMonitorDX || sbEnableMonitorSwitchingDX ||
            !(CheckForMonitorChange(), IsOnSecondaryMonitor()))
          {
            if (!mbUseSubrect && sbEnableD3D && (sbEnableTS3D || !isTermServ))
            {
              if (sbEnableD3DFX)
                mpMiniDriver =
                  VDCreateVideoDisplayMinidriverD3DFX(!sbEnableSecondaryMonitorDX || sbEnableMonitorSwitchingDX);
              else
                mpMiniDriver = VDCreateVideoDisplayMinidriverDX9(
                  !sbEnableSecondaryMonitorDX || sbEnableMonitorSwitchingDX, sbEnableD3D9Ex);
...
bool VDVideoDisplayWindow::InitMiniDriver()
{
  ...
  RECT r;
  GetClientRect(mhwnd, &r);
  mhwndChild = CreateWindowEx(
    WS_EX_NOPARENTNOTIFY,
    (LPCTSTR)sChildWindowClass,
    "",
...
bool VDVideoDisplayWindow::CheckForMonitorChange()
{
  ...
  while (GetWindowLong(hwndTest, GWL_STYLE) & WS_CHILD)
  {
    ...
    if (r.left < rParent.left)
      r.left = rParent.left;
    if (r.right < rParent.left)
      r.right = rParent.left;
    if (r.left > rParent.right)
      r.left = rParent.right;
    if (r.right > rParent.right)
      r.right = rParent.right;
    if (r.top < rParent.top)
      r.top = rParent.top;
    if (r.bottom < rParent.top)
      r.bottom = rParent.top;
    if (r.top > rParent.bottom)
      r.top = rParent.bottom;
    if (r.bottom > rParent.bottom)
      r.bottom = rParent.bottom;
...
bool VDVideoDisplayWindow::IsOnSecondaryMonitor() const
{
  ...
  static const tpGetMonitorInfo spGetMonitorInfo =
    (tpGetMonitorInfo)GetProcAddress(GetModuleHandleA("user32"), "GetMonitorInfoA");
...
VDGUIHandle VDCreateDisplayWindowW32(
  uint32      dwExFlags,
  uint32      dwFlags,
  int         x,
  int         y,
  ...
  VDGUIHandle hwndParent)
{
  ...
  struct RemoteCreateCall
  {
    ...
    static void Dispatch(void *p0)
    {
      ...
      p->hwndResult       = CreateWindowEx(
        p->dwExFlags,
        g_szVideoDisplayControlName,
        "",
        p->dwFlags,
        p->x,
        p->y,
