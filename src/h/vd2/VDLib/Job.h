...
public:
  ...
  bool IsLocal() const
  {
    return !mpJobQueue || mpJobQueue->IsLocal(this);
  }
  ...
  const char *GetProjectSubdir() const
  {
    return mProjectSubdir.c_str();
  }
  void SetProjectSubdir(const char *name)
  {
    mProjectSubdir = name;
  }

  const char *GetProjectDir() const
  {
    return mProjectDir.c_str();
  }
  ...
  {
    return mState == kStateInProgress || mState == kStateAborting;
  }
  ...
  bool IsReloadMarkerPresent() const
  {
    return mbContainsReloadMarker;
  }
