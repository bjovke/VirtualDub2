...
VDScreenGrabberGDI::VDScreenGrabberGDI()
  : mhwnd(NULL), mWndClass(NULL), mpCB(NULL), mbCapBuffersInited(false), mhdcOffscreen(NULL), mhbmOffscreen(NULL),
    mpOffscreenData(NULL), mOffscreenSize(0), mCachedCursor(NULL), mCachedCursorHotspotX(0), mCachedCursorHotspotY(0),
    mhdcCursorBuffer(NULL), mhbmCursorBuffer(NULL), mhbmCursorBufferOld(NULL), mpCursorBuffer(NULL), mbVisible(false),
    mbDisplayPreview(false), mDisplayArea(0, 0, 0, 0), mTrackX(0), mTrackY(0), mbDrawMousePointer(true),
    mCaptureWidth(320), mCaptureHeight(240)
{
  mbExcludeSelf = true;
...
bool VDScreenGrabberGDI::AcquireFrame(bool dispatch)
{
  ...
  if (HDC hdc = GetDC(NULL))
  {
    ...
    if (mbExcludeSelf && mbVisible)
    { // nothing to hide if preview is hidden
  ...
  if (dispatch)
  {
    ...
    if (mpCB)
      mpCB->ReceiveFrame(
        globalTime, (const char *)mpOffscreenData, mCaptureWidth * 4, mCaptureWidth * 4, mCaptureHeight);
  }
