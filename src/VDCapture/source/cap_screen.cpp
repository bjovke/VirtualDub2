...
VDCaptureDriverScreenConfig::VDCaptureDriverScreenConfig()
  : mbTrackCursor(false), mbTrackActiveWindow(false), mbTrackActiveWindowClient(false), mTrackOffsetX(0),
    mTrackOffsetY(0), mbDrawMousePointer(true), mbRescaleImage(false), mbRemoveDuplicates(true),
    mMode(kVDCaptureDriverScreenMode_GDI), mRescaleW(GetSystemMetrics(SM_CXSCREEN)),
...
VDCaptureDriverScreenConfigDialog::VDCaptureDriverScreenConfigDialog(VDCaptureDriverScreenConfig &config)
  : VDDialogFrameW32(IDD_SCREENCAP_OPTS), mConfig(config)
{}
...
VDCaptureDriverScreen::VDCaptureDriverScreen()
  : mhwndParent(NULL), mhwnd(NULL), mpGrabber(NULL), mbCapBuffersInited(false), mbCaptureFramePending(false),
    mbCapturing(false), mbCaptureSetup(false), mbVisible(false), mDisplayArea(0, 0, 0, 0),
    mbAudioAnalysisEnabled(false), mbAudioAnalysisActive(false), mFramePeriod(10000000 / 30), mCaptureWidth(320),
    mCaptureHeight(240), mCaptureFormat(kVDScreenGrabberFormat_XRGB32), mpCB(NULL), mDisplayMode(kDisplayNone),
    mResponsivenessTimer(0), mResponsivenessCounter(0), mbAudioMessagePosted(false), mpAudioGrabberWASAPI(NULL),
    mPreviewFrameTimer(0), mhMixer(NULL), mhWaveIn(NULL)
{
...
bool VDCaptureDriverScreen::Init(VDGUIHandle hParent)
{
  ...
  {
    WNDCLASS wc = {
      0, StaticWndProc, 0, sizeof(VDCaptureDriverScreen *), hInst, NULL, NULL, NULL, NULL, "Riza screencap window"};
...
{
  static const int kSamplingRates[] = {8000, 11025, 12000, 16000, 22050, 24000, 32000, 44100, 48000, 96000, 192000};

  static const int kChannelCounts[] = {1, 2};

  static const int kSampleDepths[] = {8, 16};
  ...
  for (int sridx = 0; sridx < sizeof kSamplingRates / sizeof kSamplingRates[0]; ++sridx)
    for (int chidx = 0; chidx < sizeof kChannelCounts / sizeof kChannelCounts[0]; ++chidx)
      ...
      {
        WAVEFORMATEX wfex = {
          WAVE_FORMAT_PCM, kChannelCounts[chidx], kSamplingRates[sridx], 0, 0, kSampleDepths[sdidx], 0};
...
{
  static const int kSamplingRates[] = {48000, 96000, 192000};

  static const int kChannelCounts[] = {4, 6, 8};

  static const int kSampleDepths[] = {16};
    ...
    for (int chidx = 0; chidx < sizeof kChannelCounts / sizeof kChannelCounts[0]; ++chidx)
      for (int sdidx = 0; sdidx < sizeof kSampleDepths / sizeof kSampleDepths[0]; ++sdidx)
      {
        WAVEFORMATEXTENSIBLE wfex = {
          WAVE_FORMAT_EXTENSIBLE, kChannelCounts[chidx], kSamplingRates[sridx], 0, 0, kSampleDepths[sdidx], 0};
...
void VDCaptureDriverScreen::ReceiveFrame(
  uint64      timestamp,
  const void *data,
  ptrdiff_t   pitch,
  uint32      rowlen,
  uint32      rowcnt)
{
  ...
  try
  {
    if (mbCapturing)
      mpCB->CapProcessData(
        0,
        mLinearizationBuffer.data(),
        (uint32)mLinearizationBuffer.size(),
        mpGrabber->ConvertTimestampDelta(timestamp, mVideoTimeBase),
        true,
        ComputeGlobalTime());
    else
...
void VDCaptureDriverScreen::InitMixerSupport()
{
  ...
  if (MMSYSERR_NOERROR == waveInGetDevCaps(WAVE_MAPPER, &wcaps, sizeof wcaps) && wcaps.dwFormats)
  {
    ...
    if (
      wcaps.dwFormats &
      (WAVE_FORMAT_1M08 | WAVE_FORMAT_1M16 | WAVE_FORMAT_2M08 | WAVE_FORMAT_2M16 | WAVE_FORMAT_4M08 | WAVE_FORMAT_4M16))
      wfex.nChannels = 1;
    else
      wfex.nChannels = 2;
    ...
    if (
      wcaps.dwFormats &
      (WAVE_FORMAT_1M08 | WAVE_FORMAT_1S08 | WAVE_FORMAT_2M08 | WAVE_FORMAT_2S08 | WAVE_FORMAT_4M08 | WAVE_FORMAT_4S08))
      ...
      if (MMSYSERR_NOERROR == mixerOpen(&mhMixer, (UINT)hwi, 0, 0, MIXER_OBJECTF_HWAVEIN))
      {
        ...
        if (MMSYSERR_NOERROR == mixerGetLineInfo((HMIXEROBJ)mhMixer, &mixerLine, MIXER_GETLINEINFOF_COMPONENTTYPE))
        {
          ...
          else
          {
            ...
            if (
              MMSYSERR_NOERROR ==
              mixerGetControlDetails((HMIXEROBJ)mhMixer, &details, MIXER_GETCONTROLDETAILSF_LISTTEXT))
            {
              ...
              if (
                MMSYSERR_NOERROR ==
                mixerGetControlDetails((HMIXEROBJ)mhMixer, &details, MIXER_GETCONTROLDETAILSF_VALUE))
              {
...
bool VDCaptureDriverScreen::InitWaveCapture()
{
  ...
  else
  {
    ...
    if (
      MMSYSERR_NOERROR !=
      waveInOpen(&mhWaveIn, WAVE_MAPPER, aformat.data(), (DWORD_PTR)mhwnd, 0, CALLBACK_WINDOW | WAVE_FORMAT_DIRECT))
      return false;
...
bool VDCaptureDriverScreen::InitVideoBuffer()
{
  ...
  if (
    mDisplayMode && mDisplayMode != kDisplayAnalyze &&
    !mpGrabber->InitDisplay(mhwndParent, mDisplayMode == kDisplaySoftware))
  {
    ShutdownVideoBuffer();
    return false;
    ...
    case WM_APP + 19: {
      ...
      if (pThis->mbAudioMessagePosted.compareExchange(false, true) == (int)true)
      {
        ...
        try
        {
          ...
          for (;;)
          {
            uint32 actual = pThis->mpAudioGrabberWASAPI->ReadData(
              buf, (sizeof buf) - (sizeof buf) % pThis->mAudioFormat->nBlockAlign);
