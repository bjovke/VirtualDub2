...
#ifdef _M_IX86
extern "C" void __cdecl vdasm_capture_resample16_MMX(
  sint16 *      d,
  int           stride,
  const sint16 *s,
  uint32        count,
...
uint64 resample16(sint16 *d, int stride, const sint16 *s, uint32 count, uint64 accum, sint64 inc)
{
  ...
  do
  {
    ...
    uint32 v = (sint32)s2[0] * (sint32)f[0] + (sint32)s2[1] * (sint32)f[1] + (sint32)s2[2] * (sint32)f[2] +
               (sint32)s2[3] * (sint32)f[3] + (sint32)s2[4] * (sint32)f[4] + (sint32)s2[5] * (sint32)f[5] +
               (sint32)s2[6] * (sint32)f[6] + (sint32)s2[7] * (sint32)f[7] + 0x20002000;
...
namespace
{
template<unsigned N> class VDCaptureWindowedRegressionEstimator
{
public:
  VDCaptureWindowedRegressionEstimator()
  {
    Reset();
  }
...
template<unsigned N> void VDCaptureWindowedRegressionEstimator<N>::Reset()
{
  mSamples      = 0;
  mWindowOffset = 0;
...
template<unsigned N> void VDCaptureWindowedRegressionEstimator<N>::AddSample(sint64 x, sint64 y)
{
...
template<unsigned N> bool VDCaptureWindowedRegressionEstimator<N>::GetSlope(double &slope) const
{
  if (mSamples < N)
    return false;
...
template<unsigned N> bool VDCaptureWindowedRegressionEstimator<N>::GetXIntercept(double slope, double &xintercept) const
{
  xintercept = ((double)mTotal.x - (double)mTotal.y / slope) / mSamples;
  return true;
}

template<unsigned N> bool VDCaptureWindowedRegressionEstimator<N>::GetYIntercept(double slope, double &yintercept) const
{
...
namespace
{
template<class T, unsigned N> class MovingAverage
{
public:
  MovingAverage() : mPos(0), mSum(0), mbInited(false)
  {
...
public:
  ...
  double GetProjectionRate() const
  {
    return mProjectionRate;
  }
...
VDCaptureResyncFilter::VDCaptureResyncFilter()
  : mpCB(NULL), mpLog(NULL), mpProfiler(NULL), mbAdjustVideoTime(true), mbAllowDrops(true), mbAllowInserts(true),
    mInsertLimit(10), mMode(kModeNone), mbEnableFixedAudioLatency(false), mbEnableAudioClock(true),
    mAutoAudioLatencyLimit(30), mVideoLastTime(0), mVideoLastRawTime(0), mVideoLastAccurateTime(0),
    mVideoTimingWrapAdjust(0), mVideoTimingAdjust(0), mAudioBytes(0), mAudioRate(0), mInvAudioRate(0), mVideoRate(1),
    mInvVideoRate(1), mChannels(0), mpAudioDecoder16(NULL), mpAudioEncoder16(NULL), mBytesPerInputSample(0)
{}
...
void VDCaptureResyncFilter::CapBegin(sint64 global_clock)
{
  ...
  mAudioResamplingController.SetProportionalHysteresis(0.010, 0.1); // apply 10ms of hysteresis, 10% bleedoff per
                                                                    // interval
...
void VDCaptureResyncFilter::CapProcessData(
  int         stream,
  const void *data,
  uint32      size,
  sint64      timestamp,
  ...
  vdsynchronized(mcsLock)
  {
    if (stream == 0)
    {
      ...
      if (frameError < -0.75 && mbAllowDrops)
      {
        ...
        {
          if (mpLog)
            mpLog->CapEvent(nsVDCapture::kEventVideoFramesDropped, 1);
      ...
      else
      {
        ...
        if (frameError > threshold && mbAllowInserts && mVideoFramesWritten)
        {
          ...
          if (mpLog)
            mpLog->CapEvent(nsVDCapture::kEventVideoFramesInserted, framesToInsert);
    ...
    else if (stream == 1 && mMode)
    {
      ...
      if (videoTimingOK)
      {
        ...
        else if (mMode == kModeResampleAudio)
        {
          ...
          if (
            mVideoWindowedRateEstimator.GetSlope(videoRate) &&
            mVideoWindowedRateEstimator.GetYIntercept(videoRate, videoYIntercept) &&
            mAudioRealRateEstimator.GetSlope(audioRate) &&
            mAudioRealRateEstimator.GetYIntercept(audioRate, audioYIntercept))
          {
            double videoTime;
            ...
            double currentLatency =
              mCurrentLatencyAverage(videoTime * 1e-6 - (mAudioWrittenBytes + size) * mInvAudioRate);
...
void VDCaptureResyncFilter::ResampleAndDispatchAudio(
  const void *data,
  uint32      size,
  bool        key,
  sint64      global_clock,
...
void VDCaptureResyncFilter::UnpackSamples(
  sint16 *    dst,
  ptrdiff_t   dstStride,
  const void *src,
  uint32      samples,
