...
struct CapBlockIterator
{
  ...
  CapBlockIterator(CapBlock *head, int lastLimit)
    : mpCurrent(head), mNext(0), mLimit(head && head->mpNext ? CapBlock::kCount : lastLimit), mLastLimit(lastLimit)
  {}
...
VDCaptureLogFilter::VDCaptureLogFilter() : mpBlockHead(NULL), mpBlockTail(NULL) {}
    ...
    while (!!itV)
    {
      ...
      if (!vent.stream)
      {
        ...
        s += sprintf(
          ...
          ++vframes,
          vent.rep,
          vent.timestamp / 1000.0,
          vent.global_clock / 1000.0,
          vent.size,
          vent.key);
        break;
      }
...
void VDCaptureLogFilter::CapProcessData(
  int         stream,
  const void *data,
  uint32      size,
  sint64      timestamp,
