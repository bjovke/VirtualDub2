...
VDScreenGrabberGL::VDScreenGrabberGL()
  : mhwndParent(NULL), mhwndGL(NULL), mhwndGLDraw(NULL), mbCapBuffersInited(false), mOffscreenSize(0),
    mOffscreenPitch(0), mGLShaderBase(0), mGLTextureW(1), mGLTextureH(1), mGLCursorCacheTexture(0), mCachedCursor(NULL),
    mCachedCursorHotspotX(0), mCachedCursorHotspotY(0), mhdcCursorBuffer(NULL), mhbmCursorBuffer(NULL),
    mhbmCursorBufferOld(NULL), mpCursorBuffer(NULL), mbCaptureFramePending(false), mbVisible(false),
    mbDisplayPreview(false), mDisplayArea(0, 0, 0, 0), mTrackX(0), mTrackY(0), mbRescaleImage(false),
    mbDrawMousePointer(true), mbRemoveDuplicates(true), mFramePeriod(10000000 / 30), mCaptureSrcWidth(320),
    mCaptureSrcHeight(240), mCaptureDstWidth(320), mCaptureDstHeight(240),
    mCaptureFormat(kVDScreenGrabberFormat_XRGB32), mpCB(NULL), mWndClassGL(NULL)
{
...
bool VDScreenGrabberGL::Init(IVDScreenGrabberCallback *cb)
{
  ...
  if (!mWndClassGL)
  {
    ...
    WNDCLASS wcgl = {
      CS_OWNDC, StaticWndProcGL, 0, sizeof(VDScreenGrabberGL *), hInst, NULL, NULL, NULL, NULL, className.c_str()};
...
bool VDScreenGrabberGL::InitCapture(uint32 srcw, uint32 srch, uint32 dstw, uint32 dsth, VDScreenGrabberFormat format)
{
  ...
  const BITMAPINFOHEADER bihCursor = {sizeof(BITMAPINFOHEADER), cxcursor, cycursor * 2, 1, 32, BI_RGB, 0, 0, 0, 0, 0};
  ReleaseDC(NULL, hdcScreen);

  mhbmCursorBuffer = CreateDIBSection(
    mhdcCursorBuffer, (const BITMAPINFO *)&bihCursor, DIB_RGB_COLORS, (void **)&mpCursorBuffer, NULL, 0);
...
void VDScreenGrabberGL::ConvertToYV12_GL_ATIFS(int w, int h, float u, float v)
{
  ...
  for (int cmode = 0; cmode < 2; ++cmode)
  {
    ...
    float u0 = -4.5f / (float)mGLTextureW;
    float u1 = u0 + u;
    float u2 = -2.5f / (float)mGLTextureW;
    float u3 = u2 + u;
    float u4 = -0.5f / (float)mGLTextureW;
    float u5 = u4 + u;
    float u6 = +0.5f / (float)mGLTextureW;
    float u7 = u6 + u;
    float u8 = +2.5f / (float)mGLTextureW;
    float u9 = u8 + u;
...
void VDScreenGrabberGL::ConvertToYUY2_GL_NV1x(int w, int h, float u, float v)
{
...

  {
    ...
    mGL.glBegin(GL_QUADS);
    mGL.glTexCoord2f(u0, v1);
    mGL.glVertex2f(0, y0);
    mGL.glTexCoord2f(u1, v1);
    mGL.glVertex2f(w2, y0);
    mGL.glTexCoord2f(u1, v0);
    mGL.glVertex2f(w2, y1);
    mGL.glTexCoord2f(u0, v0);
    mGL.glVertex2f(0, y1);
    mGL.glEnd();
    VDASSERT(!mGL.glGetError());
  }
