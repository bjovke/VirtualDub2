...
void VDLogDS(const char *msg, HRESULT hr)
{
  VDLogF(
    kVDLogWarning,
    L"CapDShow: Failed to build filter graph: %hs (error code: %08x, %hs)\n",
    msg,
    hr,
    GetDXErrorName(hr));
}
...
extern "C" const GUID __declspec(selectany)
  CLSID_SampleGrabber = {0xc1f400a0, 0x3f08, 0x11d3, {0x9f, 0x0b, 0x00, 0x60, 0x08, 0x03, 0x9e, 0x37}};
extern "C" const GUID __declspec(selectany)
  CLSID_NullRenderer = {0xc1f400a4, 0x3f08, 0x11d3, {0x9f, 0x0b, 0x00, 0x60, 0x08, 0x03, 0x9e, 0x37}};

extern "C" const GUID __declspec(selectany)
  IID_ISampleGrabberCB = {0x0579154a, 0x2b53, 0x4994, {0xb0, 0xd0, 0xe7, 0x73, 0x14, 0x8e, 0xff, 0x85}};
extern "C" const GUID __declspec(selectany)
  IID_ISampleGrabber = {0x6b652fff, 0x11fe, 0x4fce, {0x92, 0xad, 0x02, 0x66, 0xb5, 0xd7, 0xc7, 0x8f}};
...
template<class T, const IID *T_IID> class VD_MSCOMAutoPtr
{
public:
  VD_MSCOMAutoPtr() : mp(NULL) {}

  VD_MSCOMAutoPtr(const VD_MSCOMAutoPtr &src) : mp(src.mp)
  {
    if (mp)
      mp->AddRef();
  }
...
template<class T> bool operator==(T *p, const VD_MSCOMAutoPtr<T, &__uuidof(T)> &ap)
{
  return p == ap.mp;
}
...
void Enumerate(tDeviceVector &devlist, REFCLSID devclsid, bool filter_audio = false)
{
  ...
  if (SUCCEEDED(CoCreateInstance(
        CLSID_SystemDeviceEnum, NULL, CLSCTX_INPROC_SERVER, IID_ICreateDevEnum, (void **)&pCreateDevEnum)))
  {
      ...
      if (SUCCEEDED(pM->BindToStorage(0, 0, IID_IPropertyBag, (void **)&pPropBag)))
      {
        ...
        if (SUCCEEDED(pPropBag->Read(L"FriendlyName", &varName, 0)))
        {
          ...
          for (;;)
          {
            ...
            for (vdvector<VDStringW>::const_iterator it(namesToCheck.begin()), itEnd(namesToCheck.end()); it != itEnd;
                 ++it)
            {
          ...
          bool skip = false;
          if (filter_audio && !test_audio_out(pM))
            skip = true;
          if (!skip)
...
const wchar_t *VDGetNameForPhysicalConnectorTypeDShow(PhysicalConnectorType type)
{
  switch (type)
  {
    ...
    case PhysConn_Audio_AESDigital:
      return L"Audio AES Digital";
    case PhysConn_Audio_SPDIFDigital:
      return L"Audio SPDIF Digital";
  ...
  while (!filters.empty())
  {
    ...
    if (SUCCEEDED(pFilter->EnumPins(~pEnumPins)))
    {
      for (;;)
      {
        ...
        pi.pFilter->Release();
        buf.append_sprintf(
          "    Pin \"%s\" (%s): ", VDTextWToA(pi.achName).c_str(), pi.dir == PINDIR_INPUT ? "input" : "output");
        ...
        if (SUCCEEDED(pPin->ConnectedTo(~pPinConn)))
        {
          ...
          buf.append_sprintf(
            " connected to pin \"%s\" of filter \"%s\"\r\n",
            VDTextWToA(pi2.achName).c_str(),
            VDTextWToA(fi2.achName).c_str());
          fi2.pGraph->Release();
          pi2.pFilter->Release();
        }
        else
...
bool VDSaveFilterGraphDShow(const wchar_t *filename, IFilterGraph *pGraph)
{
  ...
  HRESULT   hr =
    StgCreateDocfile(filename, STGM_CREATE | STGM_TRANSACTED | STGM_WRITE | STGM_SHARE_DENY_READ, 0, &pStorage);
  if (SUCCEEDED(hr))
  {
...
public:
  VDCapDevDSCallback(IVDCaptureDSCallback *pCB)
    : mRefCount(1), mpCallback(pCB), mBlockSamples(true), mIgnoreTimestamps(false), mTimeBase(0)
  {}
...
{
public:
  VDCapDevDSVideoCallback(IVDCaptureDSCallback *pCB) : VDCapDevDSCallback(pCB), mChannel(0) {}
...
{
public:
  VDCapDevDSAudioCallback(IVDCaptureDSCallback *pCB) : VDCapDevDSCallback(pCB), mChannel(1) {}
...
DEFINE_GUID(CLSID_AudioMaskFilter, 0x49143d5e, 0x8d36, 0x4255, 0x95, 0x4c, 0xc2, 0x35, 0xb1, 0x69, 0x81, 0xa);

VDAudioMaskFilter::VDAudioMaskFilter() : CTransformFilter("mask", 0, CLSID_AudioMaskFilter)
{
  mask0 = 1;
  mask1 = 2;
}
...
void VDAudioMaskFilter::TransformFormat(const CMediaType *mtIn, CMediaType *pMediaType)
{
  ...
  if (0)
  {
    ...
    pwfx->Format.nAvgBytesPerSec =
      pwfx->Format.nSamplesPerSec * pwfx->Format.wBitsPerSample * pwfx->Format.nChannels / 8;
...
protected:
  ...
  struct InputSource
  {
    ...
    InputSource(int crossbarInputPin, int physType, const VDStringW &name)
      : mCrossbarPin(crossbarInputPin), mPhysicalType(physType), mName(name)
    {}
  };
...
VDCaptureDriverDS::VDCaptureDriverDS(IMoniker *pVideoDevice)
  : mpCB(NULL), mDisplayMode(kDisplayNone), mVideoDeviceMoniker(pVideoDevice), mAudioDeviceIndex(0),
    mpVideoQualProp(NULL), mdwRegisterGraph(0), mCurrentVideoSource(-1), mCurrentAudioSource(-1),
    mCurrentAudioInput(-1), mpAudioCrossbar(NULL), mpVideoCrossbar(NULL), mpAudioMask(NULL), mVideoCallback(this),
    mAudioCallback(this), mGraphStateLock(2), mhwndParent(NULL), mhwndEventSink(NULL), mDisplayRect(0, 0, 0, 0),
    mUpdateLocks(0), mbUpdatePending(false), mbStartPending(false), mbAudioCaptureEnabled(true),
    mbAudioAnalysisEnabled(false), mbAudioPlaybackEnabled(false), mbGraphActive(false), mbGraphHasPreview(false),
    mbDisplayVisible(false), mbDisableClockForPreview(false), mbForceAudioRendererClock(true), mTrackedFramePeriod(0),
    mCaptureStart(0), mCaptureStopQueued(0), mCaptureThread(NULL), mpCaptureError(NULL)
{}
...
bool VDCaptureDriverDS::Init(VDGUIHandle hParent)
{
  ...
  {
    WNDCLASS wc = {
      0, StaticMessageSinkWndProc, 0, sizeof(void *), g_hInst, NULL, NULL, NULL, NULL, "Riza DirectShow event sink"};
  ...
  if (!(mhwndEventSink =
          CreateWindow((LPCTSTR)sMsgSinkClass, "", WS_POPUP, 0, 0, 0, 0, mhwndParent, NULL, g_hInst, this)))
    return false;
  ...
  DS_VERIFY(
    mpCapGraphBuilder2.CreateInstance(CLSID_CaptureGraphBuilder2, NULL, CLSCTX_INPROC_SERVER),
  ...
  DS_VERIFY(
    mVideoDeviceMoniker->BindToObject(NULL, NULL, IID_IBaseFilter, (void **)~mpCapFilt), "create capture filter");
  ...
  hr = mpCapGraphBuilder2->FindPin(
    mpCapFilt, PINDIR_OUTPUT, &PIN_CATEGORY_CAPTURE, &MEDIATYPE_Interleaved, TRUE, 0, ~mpRealCapturePin);
  if (SUCCEEDED(hr))
  {
    ...
    DS_VERIFY(
      mpCapGraphBuilder2->FindPin(mpCapSplitFilt, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pCapSplitIn),
      "find DV splitter input");
    DS_VERIFY(mpGraphBuilder->Connect(mpRealCapturePin, pCapSplitIn), "connect capture -> dv splitter");

    DS_VERIFY(
      mpCapGraphBuilder2->FindPin(mpCapSplitFilt, PINDIR_OUTPUT, NULL, &MEDIATYPE_Video, TRUE, 0, ~pCapSplitOutVideo),
      "find DV splitter video output");
    DS_VERIFY(
      mpCapGraphBuilder2->FindPin(mpCapSplitFilt, PINDIR_OUTPUT, NULL, &MEDIATYPE_Audio, TRUE, 0, ~pCapSplitOutAudio),
      "find DV splitter audio output");
  ...
  {
    hr = mpCapGraphBuilder2->FindPin(
      mpCapFilt, PINDIR_OUTPUT, &PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, TRUE, 0, ~mpRealCapturePin);
  ...
  if (key.getString("CapDShow: Transform filter name", name))
  {
    ...
    if (SUCCEEDED(hr))
    {
      ...
      DS_VERIFY(
        mpCapGraphBuilder2->FindPin(mpCapTransformFilt, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinTIn),
        "find transform filter input");
      DS_VERIFY(
        mpCapGraphBuilder2->FindPin(mpCapTransformFilt, PINDIR_OUTPUT, NULL, NULL, TRUE, 0, ~pPinTOut),
        "find transform filter output");
  ...
  // it.
  hr = mpCapGraphBuilder2->FindPin(
    mpCapFilt, PINDIR_OUTPUT, &PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, TRUE, 0, ~mpRealPreviewPin);
  ...
  if (SUCCEEDED(mpRealCapturePin->EnumMediaTypes(~pEnum)))
  {
    ...
    for (;;)
    {
      ...
      if (
        pMediaType->majortype == MEDIATYPE_Video && pMediaType->formattype == FORMAT_VideoInfo &&
  ...
  hr = mpCapGraphBuilder2->FindInterface(
    &PIN_CATEGORY_CAPTURE, &MEDIATYPE_Interleaved, mpCapFilt, IID_IAMStreamConfig, (void **)~mpVideoConfigCap);

  if (FAILED(hr))
    hr = mpCapGraphBuilder2->FindInterface(
      &PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, mpCapFilt, IID_IAMStreamConfig, (void **)~mpVideoConfigCap);

  DS_VERIFY(hr, "find video format config if");

  hr = mpCapGraphBuilder2->FindInterface(
    &PIN_CATEGORY_PREVIEW, &MEDIATYPE_Interleaved, mpCapFilt, IID_IAMStreamConfig, (void **)~mpVideoConfigPrv);

  if (FAILED(hr))
    hr = mpCapGraphBuilder2->FindInterface(
      &PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, mpCapFilt, IID_IAMStreamConfig, (void **)~mpVideoConfigPrv);
  ...
  hr = mpCapGraphBuilder2->FindPin(
    mpCapFilt, PINDIR_OUTPUT, &PIN_CATEGORY_VIDEOPORT, NULL, FALSE, 0, ~mpCapFiltVideoPortPin);
  ...
  DS_VERIFY(
    mpGraphBuilder->QueryInterface(IID_IMediaControl, (void **)~mpGraphControl), "find graph control interface");
  ...
  if (mpCapSplitFilt || mpCapTransformFilt)
  {
    ...
    if (SUCCEEDED(mpRealCapturePin->EnumMediaTypes(~pEnum)))
    {
      ...
      if (S_OK == pEnum->Next(1, &pmt, &cFetched))
      {
        ...
        {
          vformat.assign(
            &((VIDEOINFOHEADER *)pmt->pbFormat)->bmiHeader, pmt->cbFormat - offsetof(VIDEOINFOHEADER, bmiHeader));
...
struct TunerInfo
{
  bool Init(IUnknown *pTuner)
  {
    ...
    if (FAILED(mpPropSet->Get(
          PROPSETID_TUNER,
          KSPROPERTY_TUNER_MODE_CAPS,
          (KSPROPERTY *)&mTunerModeCaps + 1,
          sizeof mTunerModeCaps - sizeof(KSPROPERTY),
          &mTunerModeCaps,
          sizeof mTunerModeCaps,
          &actual)))
      return false;
...
uint32 VDCaptureDriverDS::GetTunerExactFrequency()
{
  ...
  if (FAILED(info.mpPropSet->Get(
        PROPSETID_TUNER,
        KSPROPERTY_TUNER_FREQUENCY,
        (KSPROPERTY *)&freq + 1,
        sizeof freq - sizeof(KSPROPERTY),
        &freq,
        sizeof freq,
        &actual)))
    return 0;
...
bool VDCaptureDriverDS::SetTunerExactFrequency(uint32 freq)
{
  ...
  if (FAILED(info.mpPropSet->Get(
        ...
        KSPROPERTY_TUNER_FREQUENCY,
        (KSPROPERTY *)&freq + 1,
        sizeof freqData - sizeof(KSPROPERTY),
        &freqData,
        sizeof freqData,
        &actual)))
    return 0;
  ...
  HRESULT hr = info.mpPropSet->Set(
    PROPSETID_TUNER,
    KSPROPERTY_TUNER_FREQUENCY,
    (KSPROPERTY *)&freqData + 1,
    sizeof freqData - sizeof(KSPROPERTY),
    &freqData,
    sizeof freqData);
...
int VDCaptureDriverDS::GetAudioSourceForVideoSource(int idx)
{
  ...
  {
    case PhysConn_Video_Tuner:
      audioPhysType = PhysConn_Audio_Tuner;
      break;
    case PhysConn_Video_Composite:
      audioPhysType = PhysConn_Audio_Line;
      break;
    case PhysConn_Video_SVideo:
      audioPhysType = PhysConn_Audio_Line;
      break;
    case PhysConn_Video_SCSI:
      audioPhysType = PhysConn_Audio_SCSI;
      break;
    case PhysConn_Video_AUX:
      audioPhysType = PhysConn_Audio_AUX;
      break;
    case PhysConn_Video_1394:
      audioPhysType = PhysConn_Audio_1394;
      break;
    case PhysConn_Video_USB:
      audioPhysType = PhysConn_Audio_USB;
      break;
    default:
      return -2;
  }
...
void VDCaptureDriverDS::GetAvailableAudioFormats(std::list<vdstructex<WAVEFORMATEX>> &aformats)
{
  ...
  {
    if (
      pMediaType->majortype == MEDIATYPE_Audio && pMediaType->formattype == FORMAT_WaveFormatEx &&
...
bool VDCaptureDriverDS::IsDriverDialogSupported(nsVDCapture::DriverDialog dlg)
{
  switch (dlg)
  {
    case kDialogVideoFormat:
      return mpVFWDialogs && SUCCEEDED(mpVFWDialogs->HasDialog(VfwCaptureDialog_Format));
    case kDialogVideoSource:
      return mpVFWDialogs && SUCCEEDED(mpVFWDialogs->HasDialog(VfwCaptureDialog_Source));
    ...
    case kDialogVideoCaptureFilter:
      return DisplayPropertyPages(mpCapFilt, NULL, NULL, 0);
    case kDialogVideoCapturePin:
      return DisplayPropertyPages(mpRealCapturePin, NULL, NULL, 0);
    case kDialogVideoPreviewPin:
      return DisplayPropertyPages(mpRealPreviewPin, NULL, NULL, 0);
    case kDialogAudioCaptureFilter:
      return DisplayPropertyPages(mpAudioCapFilt, NULL, NULL, 0);
    case kDialogAudioCapturePin:
      return DisplayPropertyPages(mpAudioPin, NULL, NULL, 0);
    case kDialogVideoCrossbar:
      return DisplayPropertyPages(mpCrossbar, NULL, NULL, 0);
    case kDialogVideoCrossbar2:
      return DisplayPropertyPages(mpCrossbar2, NULL, NULL, 0);
    case kDialogTVTuner:
      return DisplayPropertyPages(mpTVTuner, NULL, NULL, 0);
  }
    ...
    case kDialogVideoPreviewPin:
      ...
      switch (dlg)
      {
        case kDialogVideoCapturePin: {
          ...
          DisplayPropertyPages(
            mpRealCapturePin,
            mhwndParent,
            kInvalidVideoCapturePinGuids,
            sizeof(kInvalidVideoCapturePinGuids) / sizeof(kInvalidVideoCapturePinGuids[0]));
        }
        break;
        case kDialogVideoPreviewPin:
...
void VDCaptureDriverDS::GetPropertyInfoInt(
  uint32  id,
  sint32 &minVal,
  sint32 &maxVal,
  sint32 &step,
...
bool VDCaptureDriverDS::StartGraph()
{
  ...
  if (FAILED(hr))
  {
    ...
    VDLog(
      kVDLogWarning,
      VDswprintf(L"CapDShow: Unable to transition filter graph to run state: hr = %08x (%hs)\n", 2, &hr, &err));
...
void LogMediaType(const wchar_t *title, AM_MEDIA_TYPE *pMediaType)
{
  VDStringW s;
  s.append_sprintf(
    L"%s: major {%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x} subtype "
    L"{%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x} formattype {%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x}",
    title,
...
bool VDCaptureDriverDS::BuildGraph(bool bNeedCapture, bool bEnableAudio)
{
  ...
  if (bNeedCapture || mDisplayMode == kDisplaySoftware)
  {
    ...
    DS_VERIFY(
      mpCapGraphBuilder2->FindPin(pVideoPullFilt, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinSGIn),
      "find sample grabber input");
    DS_VERIFY(
      mpCapGraphBuilder2->FindPin(pVideoPullFilt, PINDIR_OUTPUT, NULL, NULL, TRUE, 0, ~pPinSGOut),
      "find sample grabber output");
    ...
    if (FAILED(hr))
    {
      VDLogF(
        kVDLogWarning,
        L"CapDShow: Failed to build filter graph: cannot connect sample grabber filter to capture filter (error code: "
        L"%08x)\n",
        hr);
  ...
  if (pCapturePin)
  {
    ...
    DS_VERIFY(
      mpCapGraphBuilder2->FindPin(pNullRenderer, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinNRIn),
      "find null renderer input");
  ...
  if (bNeedCapture || (bEnableAudio && mbAudioAnalysisEnabled))
  {
    ...
    if (pAudioPin && bEnableAudio)
    {
      ...
      DS_VERIFY(
        pAudioPullFilt.CreateInstance(CLSID_SampleGrabber, NULL, CLSCTX_INPROC_SERVER), "create sample grabber");
      ...
      DS_VERIFY(
        mpCapGraphBuilder2->FindPin(pAudioPullFilt, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinSGIn),
        "find sample grabber input");
      DS_VERIFY(
        mpCapGraphBuilder2->FindPin(pAudioPullFilt, PINDIR_OUTPUT, NULL, NULL, TRUE, 0, ~pPinSGOut),
        "find sample grabber output");
      ...
      if (mbAudioAnalysisEnabled)
      {
        ...
        if (SUCCEEDED(pAudioPin->QueryInterface(IID_IAMBufferNegotiation, (void **)&pBN)))
        {
          ...
          {
            VDDEBUG(
              "Riza/DShow: Warning: Unable to suggest buffer size of %u bytes to audio capture pin\n",
        ...
        if (SUCCEEDED(pPinSGIn->QueryInterface(IID_IAMBufferNegotiation, (void **)&pBN)))
        {
          ...
          {
            VDDEBUG(
              "Riza/DShow: Warning: Unable to suggest buffer size of %u bytes to sample grabber pin\n",
              (unsigned)allocProp.cbBuffer);
          }
          pBN->Release();
        }
      ...
      if (FAILED(hr))
      {
        VDLogF(
          kVDLogWarning,
          L"CapDShow: Failed to build filter graph: cannot connect sample grabber filter to audio capture filter "
          L"(error code: %08x)\n",
          hr);
  ...
  if (pAudioPin)
  {
    ...
    if (mbAudioPlaybackEnabled)
    {
      ...
      DS_VERIFY(
        mpCapGraphBuilder2->FindPin(pMask, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinMIn), "find audio mask input");
      DS_VERIFY(mpGraphBuilder->Connect(pAudioPin, pPinMIn), "connect audio mask");
      mExtraFilters.push_back(mpAudioMask);
      DS_VERIFY(
        mpCapGraphBuilder2->FindPin(pMask, PINDIR_OUTPUT, NULL, NULL, TRUE, 0, ~pPinMOut), "find audio mask output");
    ...
    if (pAudioPin)
    {
      ...
      DS_VERIFY(
        mpCapGraphBuilder2->FindPin(pRenderer, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinRIn),
        "find audio renderer input");
  ...
  if (mpRealAudioPin && mpRealAudioPin != mpAudioPin)
  {
    ...
    mExtraFilters.push_back(pRenderer);
    DS_VERIFY(
      mpCapGraphBuilder2->FindPin(pRenderer, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinRIn), "find audio renderer input");
...
void VDCaptureDriverDS::CheckForWindow()
{
  ...
  {
    if (!mpCapFiltVideoPortPin)
      VDLog(kVDLogInfo, VDStringW(L"CapDShow: %u windows found in graph.\n", mVideoWindows.size()));
    return;
  }
...

  {
    for (int i = 0; i < 3; i++)
    {
      ...
      if (!pin)
        continue;
      IBaseFilterPtr pFilter;
      if (!VDGetFilterConnectedToPinDShow(pin, ~pFilter))
        continue;
      ...
      hr = mpCapGraphBuilder2->FindInterface(
        &LOOK_DOWNSTREAM_ONLY, NULL, pFilter, IID_IVideoWindow, (void **)~mpVideoWindow);
      if (SUCCEEDED(hr))
      {
        success_pin = pin;
        break;
  ...
  {
    if (!mpRealPreviewPin)
      VDLog(kVDLogInfo, VDStringW(L"CapDShow: Preview pin not found on capture device.\n"));
    if (success_pin == mpRealCapturePin)
      VDLog(kVDLogInfo, VDStringW(L"CapDShow: Switching to software display.\n"));
  }
...
bool VDCaptureDriverDS::DisplayPropertyPages(
  IUnknown *  ptr,
  HWND        hwndParent,
  const GUID *pDisablePages,
  int         nDisablePages)
{
  if (!ptr)
    return false;
  ...
  if (cauuid.cElems)
  {
    ...
    if (n)
    {
      if (hwndParent)
      {
        HRESULT hr = OleCreatePropertyFrame(hwndParent, 0, 0, NULL, 1, &ptr, n, cauuid.pElems, 0, 0, NULL);
...
int VDCaptureDriverDS::EnumerateCrossbarSources(InputSources &sources, IAMCrossbar *pCrossbar, int output)
{
  ...
  if (SUCCEEDED(pCrossbar->get_PinCounts(&outputs, &inputs)))
  {
    ...
    for (int pin = 0; pin < inputs; ++pin)
    {
      ...
      sources.push_back(
        InputSource(pin, physType, VDStringW(VDGetNameForPhysicalConnectorTypeDShow((PhysicalConnectorType)physType))));
    }
...
VDCaptureSystemDS::VDCaptureSystemDS() : mDriverCount(0)
{
  CoInitialize(NULL);
}
...
void VDCaptureDriverDS::LoadVideoConfig(VDRegistryAppKey &key)
{
  ...
  if (n != -1)
  {
    ...
    if (FAILED(hr))
      VDLogF(kVDLogInfo, L"CapDShow: loading video filter state: (error code: %08x, %hs)\n", hr, GetDXErrorName(hr));
  }
}
...
void VDCaptureDriverDS::LoadAudioConfig(VDRegistryAppKey &key)
{
  ...
  if (n != -1)
  {
    ...
    HRESULT hr = str->Load(&d);
    if (FAILED(hr))
      VDLogF(kVDLogInfo, L"CapDShow: loading audio filter state: (error code: %08x, %hs)\n", hr, GetDXErrorName(hr));
  }
}
