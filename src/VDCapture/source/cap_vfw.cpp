...
static const CAPTUREPARMS g_defaultCaptureParms = {
  ...
  0,
  0,                   // dwMCIStartTime, dwMCIStopTime
...
protected:
  ...
  bool mbCapturing;
  bool mbVisible;
  bool mbBlockVideoFrames; ///< Prevents video frames from being sent the callback. This is used to barrier frames while
                           ///< we sent video format change events.
...
VDCaptureDriverVFW::VDCaptureDriverVFW(HMODULE hmodAVICap, int driverIndex)
  : mhwnd(NULL), mhwndParent(NULL), mhwndEventSink(NULL), mhmodAVICap(hmodAVICap), mpCB(NULL), mbCapturing(false),
    mbVisible(false), mbBlockVideoFrames(false), mbAudioAnalysisEnabled(false), mbAudioAnalysisActive(false),
    mDisplayMode(kDisplayNone), mDriverIndex(driverIndex), mPreviewFrameTimer(0), mhMixer(NULL), mhWaveIn(NULL)
{
...
bool VDCaptureDriverVFW::Init(VDGUIHandle hParent)
{
  ...
  if (!sMsgSinkClass)
  {
    WNDCLASS wc = {
      0,
      StaticMessageSinkWndProc,
      0,
      sizeof(VDCaptureDriverVFW *),
      g_hInst,
      NULL,
      NULL,
      NULL,
  ...
  typedef HWND(VFWAPI * tpcapCreateCaptureWindow)(
    LPCSTR lpszWindowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hwnd, int nID);
  const tpcapCreateCaptureWindow pcapCreateCaptureWindow =
    (tpcapCreateCaptureWindow)GetProcAddress(mhmodAVICap, "capCreateCaptureWindowA");
...
bool VDCaptureDriverVFW::InitWaveAnalysis()
{
  ...
  if (
    MMSYSERR_NOERROR !=
    waveInOpen(
      &mhWaveIn, WAVE_MAPPER, aformat.data(), (DWORD_PTR)mhwndEventSink, 0, CALLBACK_WINDOW | WAVE_FORMAT_DIRECT))
    return false;
...
LRESULT CALLBACK VDCaptureDriverVFW::PreviewCallback(HWND hwnd, VIDEOHDR *lpVHdr)
{
  ...
  if (pThis->mpCB && pThis->mDisplayMode == kDisplayAnalyze && !pThis->mbBlockVideoFrames)
  {
    try
    {
      pThis->mpCB->CapProcessData(
        -1,
        lpVHdr->lpData,
        lpVHdr->dwBytesUsed,
        (sint64)lpVHdr->dwTimeCaptured * 1000,
        0 != (lpVHdr->dwFlags & VHDR_KEYFRAME),
        0);
    }
    catch (MyError &)
    {
...
VDCaptureSystemVFW::VDCaptureSystemVFW() : mhmodAVICap(VDLoadSystemLibraryW32("avicap32")), mDriverCount(0) {}
...
void VDCaptureSystemVFW::EnumerateDrivers()
{
  ...
  typedef BOOL(VFWAPI * tpcapGetDriverDescriptionA)(
    UINT wDriverIndex, LPSTR lpszName, INT cbName, LPSTR lpszVer, INT cbVer);

  const tpcapGetDriverDescriptionA pcapGetDriverDescriptionA =
    (tpcapGetDriverDescriptionA)GetProcAddress(mhmodAVICap, "capGetDriverDescriptionA");
