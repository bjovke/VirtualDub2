...
VDVFilterWarpSharp::VDVFilterWarpSharp()
  : mVDXAPSGradient(0), mVDXAPSBlur(0), mVDXAPSFinal(0), mVDXARTGradientBuffer1(0), mVDXARTGradientBuffer2(0)
{}
...
#ifdef _M_IX86
static void __declspec(
  naked) __cdecl WarpSharpComputeGradientRow32MMX(uint8 *dst, const uint32 *src, ptrdiff_t pitch, uint32 count)
{
...
static void InterpolateBicubicRowDisplacement32(
  uint32 *      dst,
  const uint32 *src,
  ptrdiff_t     pitch,
  const int *   disprow,
  ...
  const int *   cubicTab)
{
  for (uint32 i = 0; i < w; ++i)
  {
    ...
    int r0 = ((int)((p00 >> 16) & 0xff) * ch0 + (int)((p01 >> 16) & 0xff) * ch1 + (int)((p02 >> 16) & 0xff) * ch2 +
              (int)((p03 >> 16) & 0xff) * ch3 + 128) >>
             8;
    int g0 = ((int)((p00 >> 8) & 0xff) * ch0 + (int)((p01 >> 8) & 0xff) * ch1 + (int)((p02 >> 8) & 0xff) * ch2 +
              (int)((p03 >> 8) & 0xff) * ch3 + 128) >>
             8;
    int b0 =
      ((int)((p00)&0xff) * ch0 + (int)((p01)&0xff) * ch1 + (int)((p02)&0xff) * ch2 + (int)((p03)&0xff) * ch3 + 128) >>
      8;
    int r1 = ((int)((p10 >> 16) & 0xff) * ch0 + (int)((p11 >> 16) & 0xff) * ch1 + (int)((p12 >> 16) & 0xff) * ch2 +
              (int)((p13 >> 16) & 0xff) * ch3 + 128) >>
             8;
    int g1 = ((int)((p10 >> 8) & 0xff) * ch0 + (int)((p11 >> 8) & 0xff) * ch1 + (int)((p12 >> 8) & 0xff) * ch2 +
              (int)((p13 >> 8) & 0xff) * ch3 + 128) >>
             8;
    int b1 =
      ((int)((p10)&0xff) * ch0 + (int)((p11)&0xff) * ch1 + (int)((p12)&0xff) * ch2 + (int)((p13)&0xff) * ch3 + 128) >>
      8;
    int r2 = ((int)((p20 >> 16) & 0xff) * ch0 + (int)((p21 >> 16) & 0xff) * ch1 + (int)((p22 >> 16) & 0xff) * ch2 +
              (int)((p23 >> 16) & 0xff) * ch3 + 128) >>
             8;
    int g2 = ((int)((p20 >> 8) & 0xff) * ch0 + (int)((p21 >> 8) & 0xff) * ch1 + (int)((p22 >> 8) & 0xff) * ch2 +
              (int)((p23 >> 8) & 0xff) * ch3 + 128) >>
             8;
    int b2 =
      ((int)((p20)&0xff) * ch0 + (int)((p21)&0xff) * ch1 + (int)((p22)&0xff) * ch2 + (int)((p23)&0xff) * ch3 + 128) >>
      8;
    int r3 = ((int)((p30 >> 16) & 0xff) * ch0 + (int)((p31 >> 16) & 0xff) * ch1 + (int)((p32 >> 16) & 0xff) * ch2 +
              (int)((p33 >> 16) & 0xff) * ch3 + 128) >>
             8;
    int g3 = ((int)((p30 >> 8) & 0xff) * ch0 + (int)((p31 >> 8) & 0xff) * ch1 + (int)((p32 >> 8) & 0xff) * ch2 +
              (int)((p33 >> 8) & 0xff) * ch3 + 128) >>
             8;
    int b3 =
      ((int)((p30)&0xff) * ch0 + (int)((p31)&0xff) * ch1 + (int)((p32)&0xff) * ch2 + (int)((p33)&0xff) * ch3 + 128) >>
      8;
...
static void InterpolateBicubicRowDisplacement8(
  uint8 *      dst,
  const uint8 *src,
  ptrdiff_t    pitch,
  const int *  disprow,
...
#ifdef _M_IX86
uint32 __declspec(
  naked) __cdecl InterpolateBicubicMMX(const uint32 *src, ptrdiff_t pitch, const int *horiz, const int *vert)
{
...
static void InterpolateBicubicRowDisplacement32MMX(
  uint32 *      dst,
  const uint32 *src,
  ptrdiff_t     pitch,
  const int *   disprow,
  uint32        w,
  const int *   cubicTabMMX)
{
  --src;
  for (uint32 i = 0; i < w; ++i)
  {
...
void VDVFilterWarpSharp::Run()
{
  ...
  if (pxdst.format == nsVDXPixmap::kPixFormat_XRGB8888)
    VDMemcpyRect(
      (char *)pxdst.data + pxdst.pitch * (pxdst.h - 4),
      pxdst.pitch,
      (const char *)pxsrc.data + pxsrc.pitch * (pxsrc.h - 4),
      pxsrc.pitch,
      pxsrc.w * 4,
      4);
  else
  {
    ...
    switch (pxdst.format)
    {
      ...
      default:
        VDMemcpyRect(
          (char *)pxdst.data2 + pxdst.pitch2 * (pxdst.h - 4),
          pxdst.pitch2,
          (const char *)pxsrc.data2 + pxsrc.pitch2 * (pxsrc.h - 4),
          pxsrc.pitch2,
          pxsrc.w,
          4);
        VDMemcpyRect(
          (char *)pxdst.data3 + pxdst.pitch3 * (pxdst.h - 4),
          pxdst.pitch3,
          (const char *)pxsrc.data3 + pxsrc.pitch3 * (pxsrc.h - 4),
          pxsrc.pitch3,
          pxsrc.w,
          4);
    }
...
void VDVFilterWarpSharp::StartAccel(IVDXAContext *vdxa)
{
  if (fa->src.mpPixmap->format == nsVDXPixmap::kPixFormat_VDXA_YUV)
    mVDXAPSGradient = vdxa->CreateFragmentProgram(
      kVDXAPF_D3D9ByteCodePS20, kVDFilterWarpSharpPS_GradientYUV, sizeof kVDFilterWarpSharpPS_GradientYUV);
  else
    mVDXAPSGradient = vdxa->CreateFragmentProgram(
      kVDXAPF_D3D9ByteCodePS20, kVDFilterWarpSharpPS_GradientRGB, sizeof kVDFilterWarpSharpPS_GradientRGB);

  mVDXAPSBlur =
    vdxa->CreateFragmentProgram(kVDXAPF_D3D9ByteCodePS20, kVDFilterWarpSharpPS_Blur, sizeof kVDFilterWarpSharpPS_Blur);
  mVDXAPSFinal = vdxa->CreateFragmentProgram(
    kVDXAPF_D3D9ByteCodePS20, kVDFilterWarpSharpPS_Final, sizeof kVDFilterWarpSharpPS_Final);
...
void VDVFilterWarpSharp::RunAccel(IVDXAContext *vdxa)
{
  ...
  const float v[4]  = {depth * desc.mInvTexWidth, depth * desc.mInvTexHeight, 0, 0};
...
public:
  VDVFilterDialogWarpSharp(VDVFilterConfigWarpSharp &config, IVDXFilterPreview *ifp)
    : VDDialogFrameW32(IDD_FILTER_WARPSHARP), mifp(ifp), mConfig(config), mOldConfig(config)
  {}

  const VDVFilterConfigWarpSharp &GetConfig() const
  {
    return mConfig;
  }
...
extern const VDXFilterDefinition g_VDVFWarpSharp = VDXVideoFilterDefinition<VDVFilterWarpSharp>(
  NULL,
  "warp sharp",
  "Tightens edges in an image by warping the image toward edge boundaries. (Version 1.2)\n\n[Assembly "
  "optimized][Requires MMX]");
