  ...
  uint32      alpha)
{
  ...
  do
  {
    ...
    for (uint32 x = 0; x < w16; ++x)
    {
      ...
      __m128i r0 = _mm_srli_epi16(
        _mm_add_epi16(_mm_add_epi16(_mm_mulhi_epu16(a0, coeffa0), _mm_mulhi_epu16(b0, coeffb0)), round), 8);
...
struct VDVideoFilterInterpolateConfig
{
  ...
  VDVideoFilterInterpolateConfig() : mbRateScale(true), mRateFactor(2.0), mbLerp(true) {}
};
...
VDVideoFilterInterpolateDialog::VDVideoFilterInterpolateDialog(
  VDVideoFilterInterpolateConfig &config,
  IVDXFilterPreview2 *            ifp2)
  : VDDialogFrameW32(IDD_FILTER_INTERPOLATE), mConfig(config), mifp2(ifp2)
{}
...
void VDVideoFilterInterpolate::Run()
{
  ...
  if (alpha)
  {
    ...
    {
      mpLerpFn(
        pxdst.data2, pxdst.pitch2, pxsrc1.data2, pxsrc2.data2, pxsrc1.pitch2, mVec16PerRow2, mChromaHeight, alpha);
      mpLerpFn(
        pxdst.data3, pxdst.pitch3, pxsrc1.data3, pxsrc2.data3, pxsrc1.pitch3, mVec16PerRow2, mChromaHeight, alpha);
    }
  }
  else
  {
...
#pragma warning(disable : 4505) // warning C4505: 'VDXVideoFilter::[thunk]: __thiscall
                                // VDXVideoFilter::`vcall'{44,{flat}}' }'' : unreferenced local function has been
                                // removed
