...
struct VDVideoFilterFieldDelayConfig
{
  ...
  VDVideoFilterFieldDelayConfig() : mbCurTFF(false) {}
};
...
public:
  VDVideoFilterFieldDelayDialog(VDVideoFilterFieldDelayConfig &config, IVDXFilterPreview *ifp)
    : VDDialogFrameW32(IDD_FILTER_FIELDDELAY), mConfig(config), mifp(ifp)
  {}
...
void VDVideoFilterFieldDelay::Run()
{
  ...
  VDMemcpyRect(
    ...
    pxdst.pitch * 2,
    (const char *)pxsrcb.data + pxsrcb.pitch,
    pxsrcb.pitch * 2,
    mLumaRowBytes,
    hb);
  ...
  if (mChromaRowBytes)
  {
    ...
    VDMemcpyRect(
      ...
      pxdst.pitch2 * 2,
      (const char *)pxsrcb.data2 + pxsrcb.pitch2,
      pxsrcb.pitch2 * 2,
      mChromaRowBytes,
      hb);
    ...
    VDMemcpyRect(
      ...
      pxdst.pitch3 * 2,
      (const char *)pxsrcb.data3 + pxsrcb.pitch3,
      pxsrcb.pitch3 * 2,
      mChromaRowBytes,
      hb);
  }
