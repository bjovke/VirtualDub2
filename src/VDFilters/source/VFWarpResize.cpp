...
#ifdef _M_IX86
#pragma warning(disable : 4799) // warning C4799: function '`anonymous namespace'::CubicFilterH_MMX' has no EMMS
                                // instruction
...
WarpResizeFilter::WarpResizeFilter() : mbUseMMX(false), mbShowGradientMap(false), mTargetWidth(640), mTargetHeight(480)
{}
...
template<class T, int N> class RowFilter : public IRowFilter
{
public:
  RowFilter(T *p, uint32 width, int limit) : mPos(-1), mLimit(limit), mPhase(0), mWidth(width)
  {
    for (int i = 0; i < N; ++i)
    {
...
template<int N> class SrcFilter : public IRowFilter
{
public:
  SrcFilter(void *buf, ptrdiff_t pitch, int limit) : mPos(0), mLimit(limit), mPitch(pitch), mpSrc(buf), mPhase(0)
  {
    for (int i = 0; i < N; ++i)
    {
...
template<int N> class LumaFilter : public RowFilter<sint32, N>
{
public:
  LumaFilter(IRowFilter &upstream, sint32 *buf, uint32 width, int limit)
    : RowFilter<sint32, N>(buf, width, limit), mUpstream(upstream)
  {}
...
template<int N> class BumpFilter : public RowFilter<sint32, N>
{
public:
  ...
  void ProcessRow(sint32 *dst, int pos, uint32 width)
  {
    ...
    ComputeBumpGradient(
      dst + 4, (const sint32 *)mUpstream[-2], (const sint32 *)mUpstream[-1], (const sint32 *)mUpstream[0], width - 8);
...
#ifdef _M_IX86
void __declspec(naked) __cdecl MonoSplineFilterH_MMX(
  sint32 *      dst,
  const sint32 *src,
  uint32        count,
  uint32        accum,
...
void __declspec(naked)
  MonoSplineFilterV_MMX(sint32 *dst, const sint32 *const *srcp, uint32 count, int phase, const uint32 *table)
{
  static const __declspec(align(8)) __int64 round = 0x0000200000002000;
...
template<int N> class BicubicSplineFilter1 : public RowFilter<sint32, N>
{
public:
  BicubicSplineFilter1(
    IRowFilter &  upstream,
    sint32 *      buf,
    uint32        dstwidth,
    uint32        srcwidth,
    int           limit,
    const int *   filter,
    const uint32 *filterMMX,
    bool          useMMX)
    : RowFilter<sint32, N>(buf, dstwidth, limit), mUpstream(upstream), mpFilter(filter), mpFilterMMX(filterMMX),
      mbUseMMX(useMMX)
  {
...
template<int N> class BicubicSplineFilter2 : public RowFilter<sint32, N>
{
public:
  BicubicSplineFilter2(
    IRowFilter &  upstream,
    sint32 *      buf,
    uint32        dstwidth,
    uint32        dstheight,
...
template<int N> class GradientFilter : public RowFilter<sint32, N>
{
public:
  ...
  void ProcessRow(sint32 *dst, int pos, uint32 width)
  {
    ...
    ComputeGradient(
      (sint16 *)dst, (const sint32 *)mUpstream[-2], (const sint32 *)mUpstream[-1], (const sint32 *)mUpstream[0], width);
  }
...
void WarpResizeFilter::ComputeGradientMap()
{
  ...
  BicubicSplineFilter1<4> bispline1(
    bumpfilt, buf, dstwidth, srcwidth, srcheight, mSplineTable, mSplineTableMMX, mbUseMMX);
  buf += 4 * dstwidth;

  BicubicSplineFilter2<3> bispline2(
    bispline1, buf, dstwidth + 2, dstheight, srcheight, dstheight, mSplineTable, mSplineTableMMX, mbUseMMX);
  buf += 3 * (dstwidth + 2);
...
void CubicFilterH(uint32 *dst, const uint32 *src, uint32 count, uint32 accum, uint32 accinc, const int *table)
{
  do
  {
    ...
    const uint8 *src8 = (const uint8 *)src;
    int          rawr =
      (c0 * (sint32)src8[2] + c1 * (sint32)src8[6] + c2 * (sint32)src8[10] + c3 * (sint32)src8[14] + 8192) >> 14;
    int rawg =
      (c0 * (sint32)src8[1] + c1 * (sint32)src8[5] + c2 * (sint32)src8[9] + c3 * (sint32)src8[13] + 8192) >> 14;
    int rawb =
      (c0 * (sint32)src8[0] + c1 * (sint32)src8[4] + c2 * (sint32)src8[8] + c3 * (sint32)src8[12] + 8192) >> 14;

    if ((unsigned)rawr >= 256)
      rawr = ((~rawr) >> 31) & 0xff;
    if ((unsigned)rawg >= 256)
      rawg = ((~rawg) >> 31) & 0xff;
    if ((unsigned)rawb >= 256)
      rawb = ((~rawb) >> 31) & 0xff;
...
#ifdef _M_IX86
void __declspec(naked) __cdecl CubicFilterH_MMX(
  uint32 *      dst,
  const uint32 *src,
  uint32        count,
  uint32        accum,
  ...
  const int *   table)
{
  ...
  {
    int rawr =
      (c0 * (sint32)src08[2] + c1 * (sint32)src18[2] + c2 * (sint32)src28[2] + c3 * (sint32)src38[2] + 8192) >> 14;
    int rawg =
      (c0 * (sint32)src08[1] + c1 * (sint32)src18[1] + c2 * (sint32)src28[1] + c3 * (sint32)src38[1] + 8192) >> 14;
    int rawb =
      (c0 * (sint32)src08[0] + c1 * (sint32)src18[0] + c2 * (sint32)src28[0] + c3 * (sint32)src38[0] + 8192) >> 14;
    src08 += 4;
    src18 += 4;
    src28 += 4;
    src38 += 4;
...
#ifdef _M_IX86
void __declspec(naked)
  CubicFilterV_MMX(uint32 *dst, const uint32 *const *srcp, uint32 count, int phase, const uint32 *table)
{
  static const __declspec(align(8)) __int64 round = 0x0000200000002000;
  ...
  const void *         yf1)
{
  ...
  if ((uint32)rfinal >= 256)
    rfinal = (~rfinal >> 31) & 0xff;
  if ((uint32)gfinal >= 256)
    gfinal = (~gfinal >> 31) & 0xff;
  if ((uint32)bfinal >= 256)
    bfinal = (~bfinal >> 31) & 0xff;

  dst[0] = (rfinal << 16) + (gfinal << 8) + bfinal;
}

#ifdef _M_IX86
void __declspec(naked) SampleBicubic_MMX(
  uint32 *             dst,
  const uint32 *const *srcptrs,
  uint32               offset,
  const void *         xf0,
