...
#ifdef VD_CPU_X86
#pragma warning(disable : 4799) // warning C4799: function '`anonymous
                                // namespace'::ComputeScanLineImprovement_XRGB8888_MMX' has no EMMS instruction

void __declspec(naked) ComputeScanLineImprovement_X8R8G8B8_MMX(
  const void *src1,
  const void *src2,
  ptrdiff_t   pitch,
  uint32      w,
...
IVTCScore ComputeScanImprovement_X8R8G8B8_SSE2(
  const void *src1,
  const void *src2,
  ptrdiff_t   srcpitch,
  uint32      w,
...
void __declspec(naked) ComputeScanLineImprovement_L8_MMX(
  const void *src1,
  const void *src2,
  ptrdiff_t   pitch,
  uint32      w,
  ...
  int         weight100)
{
  ...
  do
  {
    ...
    do
    {
      ...
      const __m128i ad_lo = _mm_add_epi16(
        a_lo, _mm_srai_epi16(_mm_add_epi16(_mm_mullo_epi16(_mm_sub_epi16(d_lo, a_lo), mixfac), mixround), 7));
      const __m128i ad_hi = _mm_add_epi16(
        a_hi, _mm_srai_epi16(_mm_add_epi16(_mm_mullo_epi16(_mm_sub_epi16(d_hi, a_hi), mixfac), mixround), 7));
...
void DeblurFrames(
  const VDXPixmap &pxdst,
  const VDXPixmap &pxsrc0,
  const VDXPixmap &pxsrc1,
  const VDXPixmap &pxsrc2,
  const VDXPixmap &pxsrc3,
  int              weight)
{
...
struct VDVideoFilterIVTCConfig
{
  ...
  VDVideoFilterIVTCConfig() : mbReduceRate(false), mOffset(-1), mBlendWeight(50), mFieldMode(kFieldMode_Autoselect) {}

  bool operator==(const VDVideoFilterIVTCConfig &x) const
  {
    return mbReduceRate == x.mbReduceRate && mOffset == x.mOffset && mFieldMode == x.mFieldMode &&
...
public:
  VDVideoFilterIVTCDialog(VDVideoFilterIVTCConfig &config, IVDXFilterPreview2 *ifp2)
    : VDDialogFrameW32(IDD_FILTER_IVTC), mConfig(config), mifp2(ifp2)
  {}
...
void VDVideoFilterIVTC::Run()
{
  ...
  if (mConfig.mFieldMode == VDVideoFilterIVTCConfig::kFieldMode_Duplicated)
  {
    ...
    VDDEBUG(
      "scores: %10lld %10lld %10lld %10lld %10lld %10lld %10lld %10lld %10lld %10lld\n",
      scores[0].mVar[0],
      scores[1].mVar[0],
      scores[2].mVar[0],
      scores[3].mVar[0],
    ...
    for (int i = 0; i < 5; ++i)
    {
      ...
      VDDEBUG("Pscores[%d]: %10lld (%10ld)\n", i, pscores[i], pscores[i] > 0 ? 0 : (long)sqrt((double)pscores[i]));
    }
  ...
  else if (mConfig.mFieldMode == VDVideoFilterIVTCConfig::kFieldMode_Blurred)
  {
    ...
    for (int i = 0; i < 5; ++i)
    {
      pscores[i] = (scores[i + 2].mVar[0] + scores[i + 3].mVar[0] + scores[i + 4].mVar[0] + scores[i + 7].mVar[0] +
                    scores[i + 8].mVar[0] + scores[i + 9].mVar[0]) *
                     2 -
  ...
  else
  {
    ...
    for (int i = 0; i < 5; ++i)
    {
      pscores[i][0] = scores[i + 1].mVarShift[0] + 0 * scores[i + 2].mVarShift[0] - scores[i + 1].mVar[0] -
                      scores[i + 2].mVar[0] + scores[i + 6].mVarShift[0] + 0 * scores[i + 7].mVarShift[0] -
                      scores[i + 6].mVar[0] - scores[i + 7].mVar[0];
      pscores[i][1] = scores[i + 1].mVarShift[1] + 0 * scores[i + 2].mVarShift[1] - scores[i + 1].mVar[1] -
                      scores[i + 2].mVar[1] + scores[i + 6].mVarShift[1] + 0 * scores[i + 7].mVarShift[1] -
                      scores[i + 6].mVar[1] - scores[i + 7].mVar[1];

      VDDEBUG(
        "Pscores[%d]: %10lld | %-10lld (%10ld | %-10ld)\n",
        i,
        pscores[i][0],
        pscores[i][1],
    ...
    if (mConfig.mbReduceRate)
    {
      ...
      if (localOffset >= dupeOffset)
      {
        ...
        CopyPixmap(
          SlicePixmap(pxdst, !bestPolarity),
          SlicePixmap(*pxsrc[bestPhase == 0 || bestPhase == 4 ? 7 : 6], !bestPolarity));
        return;
      }
    }
    ...
    CopyPixmap(
      SlicePixmap(pxdst, !bestPolarity), SlicePixmap(*pxsrc[bestPhase == 4 || bestPhase == 3 ? 6 : 5], !bestPolarity));
  }
}
...
{
  static const char *const kModes[] = {"auto", "tff", "bff", "deblur", "frames"};
...
void VDVideoFilterIVTC::GetScriptString(char *buf, int maxlen)
{
  if (mConfig.mFieldMode == VDVideoFilterIVTCConfig::kFieldMode_Blurred)
    SafePrintf(
      ...
      maxlen,
      "Config(%d,%d,%d,%d)",
      mConfig.mbReduceRate,
      mConfig.mFieldMode,
      mConfig.mOffset,
      mConfig.mBlendWeight);
  else
...
extern const VDXFilterDefinition g_VDVFIVTC =
  VDXVideoFilterDefinition<VDVideoFilterIVTC>(NULL, "IVTC", "Removes 3:2 pulldown (telecine) from video.");
