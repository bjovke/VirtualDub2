...
struct VDVideoFilterInterlaceConfig
{
  ...
  VDVideoFilterInterlaceConfig() : mbLaceFields(false), mbOddFieldFirst(true) {}
};
...
VDVideoFilterInterlaceDialog::VDVideoFilterInterlaceDialog(VDVideoFilterInterlaceConfig &config)
  : VDDialogFrameW32(IDD_FILTER_INTERLACE), mConfig(config)
{}
...
{
  SafePrintf(
    buf, maxlen, " (%s, %s)", mConfig.mbLaceFields ? "fields" : "frames", mConfig.mbOddFieldFirst ? "BFF" : "TFF");
}
...
extern const VDXFilterDefinition g_VDVFInterlace = VDXVideoFilterDefinition<VDVideoFilterInterlace>(
  NULL,
  "interlace",
