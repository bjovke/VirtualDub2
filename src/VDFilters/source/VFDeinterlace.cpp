...
void nela_L8_SSE2(__m128i *dst, const __m128i *elabuf, int w16)
{
  ...
  do
  {
    ...
    // result = (scorel1 < scorec0) ? (scorel2 < scorel1 ? l2 : l1) : (scorer1 < scorec0) ? (scorer2 < scorer1 ? r2 :
    // r1) : c0
    ...
    __m128i pred =
      _mm_or_si128(_mm_or_si128(_mm_or_si128(result_l1, result_l2), _mm_or_si128(result_r1, result_r2)), result_c0);
...
void InterpPlane_NELA_X8R8G8B8(
  void *      dst,
  ptrdiff_t   dstpitch,
  const void *src,
  ptrdiff_t   srcpitch,
...
#if defined(VD_CPU_X86)
void __declspec(naked) __cdecl Average_MMX(
  void *      dst,
  ptrdiff_t   dstPitch,
  const void *src1,
  const void *src2,
...
void __declspec(naked) __cdecl Average_ISSE(
  void *      dst,
  ptrdiff_t   dstPitch,
  const void *src1,
  const void *src2,
...
{
  void (*blend_func)(
    void *dst, ptrdiff_t dstPitch, const void *src1, const void *src2, ptrdiff_t srcPitch, uint32 w16, uint32 h);
#if defined(VD_CPU_X86)
  if (SSE2_enabled)
...
#if defined(VD_CPU_X86)
void __declspec(naked) __stdcall Absdiff_MMX(
  void *      dst,
  ptrdiff_t   dstPitch,
  const void *src1,
  const void *src2,
...
#if defined(VD_CPU_X86)
void __declspec(naked) __stdcall AbsDiffBlended_MMX(
  void *      dst,
  ptrdiff_t   dstPitch,
  const void *src1,
  const void *src2,
...
void __declspec(naked) __stdcall AbsDiffBlended_ISSE(
  void *      dst,
  ptrdiff_t   dstPitch,
  const void *src1,
  const void *src2,
  ...
  uint32      w16)
{
  ...
  do
  {
    ...
    __m128i mx =
      _mm_min_epu8(_mm_subs_epu8(p2, _mm_max_epu8(p3, p1)), _mm_max_epu8(_mm_subs_epu8(p0, p1), _mm_subs_epu8(p4, p3)));
    __m128i mn =
      _mm_min_epu8(_mm_subs_epu8(_mm_min_epu8(p3, p1), p2), _mm_max_epu8(_mm_subs_epu8(p1, p0), _mm_subs_epu8(p3, p4)));
...
#if defined(VD_CPU_X86)
void __declspec(naked) __stdcall YadifTemporal_ISSE(
  void *      dst,
  const void *srct,
  const void *srcb,
  const void *srcAvgT0, // field 1/3 average, two scanlines up
...
void __declspec(naked) __stdcall YadifTemporal_MMX(
  void *      dst,
  const void *srct,
  const void *srcb,
  const void *srcAvgT0, // field 1/3 average, two scanlines up
...
struct VDVideoFilterDeinterlaceConfig
{
  ...
  VDVideoFilterDeinterlaceConfig() : mMode(kModeYadif), mTFF(true), mDoubleRate(false) {}
};
...
public:
  VDVideoFilterDeinterlaceDialog(VDVideoFilterDeinterlaceConfig &config)
    : VDDialogFrameW32(IDD_FILTER_DEINTERLACE), mConfig(config)
  {}
...
{
public:
  VDVideoFilterDeinterlace() : mVDXAFP_ELA1(0), mVDXAFP_ELA2(0), mVDXAFP_Final(0), mVDXART_ELA1(0), mVDXART_ELA2(0) {}
...
uint32 VDVideoFilterDeinterlace::GetParams()
{
  ...
  mbDoublingRate =
    (mConfig.mDoubleRate && mConfig.mMode != VDVideoFilterDeinterlaceConfig::kModeFold &&
     mConfig.mMode != VDVideoFilterDeinterlaceConfig::kModeUnfold);
  mBufferCount = 4;
  mBufferClock = 0;
...
void VDVideoFilterDeinterlace::Run_Yadif(bool keepField2)
{
  ...
  for (int plane = 0; plane < planeCount; ++plane)
  {
    ...
    const BufferDesc neededBuffers[4] = {
      {field0, kTypeAbsDiffBlended}, {field2, kTypeAbsDiffBlended}, {field1, kTypeAbsDiff}, {field1, kTypeAverage}};
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      switch (neededBuffers[i].mType)
      {
        case kTypeAverage:
          if (SSE2_enabled)
            Average_SSE2(
              planeBuffers[i].mStorage.data(),
              planePitch,
              srcPlanes[srcOffset].data,
              srcPlanes[srcOffset + 2].data,
              srcPitch,
              planePitch >> 4,
              h2);
#if defined(VD_CPU_X86)
...
void VDVideoFilterDeinterlace::Run_Duplicate(bool field2)
{
  ...
  {
    VDMemcpyRect(
      pxdst.data, pxdst.pitch * 2, (const char *)pxdst.data + pxdst.pitch, pxdst.pitch * 2, mLumaRowBytes, h2);

    if (mChromaRowBytes)
    {
      VDMemcpyRect(
        pxdst.data2, pxdst.pitch2 * 2, (const char *)pxdst.data2 + pxdst.pitch2, pxdst.pitch2 * 2, mChromaRowBytes, h2);
      VDMemcpyRect(
        pxdst.data3, pxdst.pitch3 * 2, (const char *)pxdst.data3 + pxdst.pitch3, pxdst.pitch3 * 2, mChromaRowBytes, h2);
    }
  }
  else
  {
    ...
    {
      VDMemcpyRect(
        (char *)pxdst.data2 + pxdst.pitch2, pxdst.pitch2 * 2, pxdst.data2, pxdst.pitch2 * 2, mChromaRowBytes, h2);
      VDMemcpyRect(
        (char *)pxdst.data3 + pxdst.pitch3, pxdst.pitch3 * 2, pxdst.data3, pxdst.pitch3 * 2, mChromaRowBytes, h2);
    }
  }
}
...
void VDVideoFilterDeinterlace::Run_Discard(bool field2)
{
  ...
  if (field2)
  {
    ...
    {
      VDMemcpyRect(
        pxdst.data2, pxdst.pitch2, (const char *)pxsrc.data2 + pxsrc.pitch2, pxdst.pitch2 * 2, mChromaRowBytes, h);
      VDMemcpyRect(
        pxdst.data3, pxdst.pitch3, (const char *)pxsrc.data3 + pxsrc.pitch3, pxdst.pitch3 * 2, mChromaRowBytes, h);
    }
  }
  else
  {
...
void VDVideoFilterDeinterlace::Run_Fold(bool field2)
{
  ...
  VDMemcpyRect(
    (char *)pxdst.data + pxdst.pitch,
    pxdst.pitch * 2,
    (const char *)pxsrc.data + mLumaRowBytes,
    pxsrc.pitch,
    mLumaRowBytes,
    h2);
  ...
  if (mChromaRowBytes)
  {
    ...
    VDMemcpyRect(
      (char *)pxdst.data2 + pxdst.pitch2,
      pxdst.pitch2 * 2,
      (const char *)pxsrc.data2 + mChromaRowBytes,
      pxsrc.pitch2,
      mChromaRowBytes,
      h2);
    ...
    VDMemcpyRect(
      (char *)pxdst.data3 + pxdst.pitch3,
      pxdst.pitch3 * 2,
      (const char *)pxsrc.data3 + mChromaRowBytes,
      pxsrc.pitch3,
      mChromaRowBytes,
      h2);
  }
...
void VDVideoFilterDeinterlace::Run_Unfold(bool field2)
{
  ...
  VDMemcpyRect(pxdst.data, pxdst.pitch, pxsrc.data, pxsrc.pitch * 2, mLumaRowBytes, h);
  VDMemcpyRect(
    (char *)pxdst.data + mLumaRowBytes,
    pxdst.pitch,
    (const char *)pxsrc.data + pxsrc.pitch,
    pxsrc.pitch * 2,
    mLumaRowBytes,
    h);
  ...
  if (mChromaRowBytes)
  {
    VDMemcpyRect(pxdst.data2, pxdst.pitch2, pxsrc.data2, pxsrc.pitch2 * 2, mChromaRowBytes, h);
    VDMemcpyRect(
      (char *)pxdst.data2 + mChromaRowBytes,
      pxdst.pitch2,
      (const char *)pxsrc.data2 + pxsrc.pitch2,
      pxsrc.pitch2 * 2,
      mChromaRowBytes,
      h);
    ...
    VDMemcpyRect(pxdst.data3, pxdst.pitch3, pxsrc.data3, pxsrc.pitch3 * 2, mChromaRowBytes, h);
    VDMemcpyRect(
      (char *)pxdst.data3 + mChromaRowBytes,
      pxdst.pitch3,
      (const char *)pxsrc.data3 + pxsrc.pitch3,
      pxsrc.pitch3 * 2,
      mChromaRowBytes,
      h);
  }
...
void VDVideoFilterDeinterlace::StartAccel(IVDXAContext *vdxa)
{
  ...
  mVDXAFP_ELA1 = vdxa->CreateFragmentProgram(
    kVDXAPF_D3D9ByteCodePS20, kVDFilterDeinterlaceFP_ELA1, sizeof kVDFilterDeinterlaceFP_ELA1);
  mVDXAFP_ELA2 = vdxa->CreateFragmentProgram(
    kVDXAPF_D3D9ByteCodePS20, kVDFilterDeinterlaceFP_ELA2, sizeof kVDFilterDeinterlaceFP_ELA2);
  mVDXAFP_Final = vdxa->CreateFragmentProgram(
    kVDXAPF_D3D9ByteCodePS20, kVDFilterDeinterlaceFP_Final, sizeof kVDFilterDeinterlaceFP_Final);
...
void VDVideoFilterDeinterlace::RunAccel(IVDXAContext *vdxa)
{
  ...
  const float mx[12] = {0, 0, 0, 0, (float)(fa->dst.h >> 1), 0, 0, 0, interpolatingBottomField ? 0.0f : 0.5f, 0, 0, 0};
...
{
  static const char *const kModes[] = {"yadif", "ELA", "bob", "blend", "duplicate", "discard", "unfold", "fold"};
...
extern const VDXFilterDefinition g_VDVFDeinterlace = VDXVideoFilterDefinition<VDVideoFilterDeinterlace>(
  NULL,
  "deinterlace",
  "Removes scanline artifacts from interlaced video. Parts based on the Yadif deinterlacing algorithm by Michael "
  "Niedermayer.");

#ifdef _MSC_VER
#pragma warning(disable : 4505) // warning C4505: 'VDXVideoFilter::[thunk]: __thiscall
                                // VDXVideoFilter::`vcall'{48,{flat}}' }'' : unreferenced local function has been
                                // removed
