...
#define SCALE2(i)                                                                                                 \
  SCALE((i) + 0), SCALE((i) + 1), SCALE((i) + 2), SCALE((i) + 3), SCALE((i) + 4), SCALE((i) + 5), SCALE((i) + 6), \
    SCALE((i) + 7), SCALE((i) + 8), SCALE((i) + 9)
...
#ifdef _M_IX86
static void __declspec(naked) asm_timesmooth_run_MMX(
  uint32 *dstbuf,
  uint32 *accumbuf,
  int     w,
  int     h,
...
static int timesmooth_run(const VDXFilterActivation *fa, const VDXFilterFunctions *ff)
{
  ...
  if (sfd->bIsFirstFrame)
  {
    ...
    do
    {
      ...
      do
      {
        accum[0] = accum[1] = accum[2] = accum[3] = accum[4] = accum[5] = accum[6] = *srcdst++ & 0xffffff;
...
static VDXScriptObject timesmooth_obj = {NULL, timesmooth_func_defs};
