...
struct VDVFBlendLayersConfig
{
  ...
  VDVFBlendLayersConfig() : mMode(kMode_Lerp), mAmount(1.0f) {}
};
...
VDVFBlendLayersDialog::VDVFBlendLayersDialog(VDVFBlendLayersConfig &config, IVDXFilterPreview2 *ifp2)
  : VDDialogFrameW32(IDD_FILTER_BLENDLAYERS), mConfig(config), mifp2(ifp2)
{
...
VDVFBlendLayers::VDVFBlendLayers() : mVDXAShader(0) {}
  ...
  switch (mConfig.mMode)
  {
    case VDVFBlendLayersConfig::kMode_Lerp:
      if (factor8 == 255)
        ...
        VDDSPProcessPlane3A(
          dstp, dstpitch, srcp1, src1pitch, srcp2, src2pitch, w16, h, VDDSPBlend8_LerpConst_SSE2, factor8);
      else
...
extern const VDXFilterDefinition g_VDVFBlendLayers =
  VDXVideoFilterDefinition<VDVFBlendLayers>(NULL, "blend layers", "Blends layers from two video streams.");
