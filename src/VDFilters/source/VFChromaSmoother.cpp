...
inline uint32 filterMPEG4(uint32 c1, int y1, uint32 c2, int y2, uint32 c3, int y3, uint32 c4, int y4, uint32 c5, int y5)
{
  ...
  unsigned r  = yd + ((((c1 >> 16) & 0xff) + ((c5 >> 16) & 0xff) +
                      2 * (((c2 >> 16) & 0xff) + ((c3 >> 16) & 0xff) + ((c4 >> 16) & 0xff)) + 4) >>
                     3);
  unsigned g  = yd + ((((c1 >> 8) & 0xff) + ((c5 >> 8) & 0xff) +
                      2 * (((c2 >> 8) & 0xff) + ((c3 >> 8) & 0xff) + ((c4 >> 8) & 0xff)) + 4) >>
                     3);
...
public:
  VDVFChromaSmootherDialog(int &mode, IVDXFilterPreview *pifp)
    : VDDialogFrameW32(IDD_FILTER_CHROMASMOOTHER), mMode(mode), mpPreview(pifp)
  {}
