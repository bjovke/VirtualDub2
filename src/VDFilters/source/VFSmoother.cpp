...
static const byte codes[] = {
  0x00, 0x20, 0x21, 0x41, 0x21, 0x41, 0x42, 0x62, 0x21, 0x41, 0x42, 0x62, 0x42, 0x62, 0x63, 0x83,
  0x20, 0x40, 0x41, 0x61, 0x41, 0x61, 0x62, 0x82, 0x41, 0x61, 0x62, 0x82, 0x62, 0x82, 0x83, 0xa3,
};
...
static int smoother_run(const VDXFilterActivation *fa, const VDXFilterFunctions *ff)
{
  ...
  filtrow_1(
    mfd->sum_row[4], (uint32 *)((char *)(srcf + 1) + srcf_pitch), pxdst.w - 2, srcf_pitch, g_thresh, mfd->square_table);
  ...
  do
  {
    if (h > 3)
      filtrow_1(
        mfd->sum_row[next_row],
        (uint32 *)((char *)(srcf + 1) + srcf_pitch * 2),
        pxdst.w - 2,
        srcf_pitch,
        g_thresh,
        mfd->square_table);
...
#endif
    {
      ...
      do
      {
        ...
        else
        {
          ...
          r = (p0 & 0xff0000) + (p1 & 0xff0000) * 2 + (p2 & 0xff0000) * 2 + (p3 & 0xff0000) * 2 + (p4 & 0xff0000);
          g = (p0 & 0xff00) + (p1 & 0xff00) * 2 + (p2 & 0xff00) * 2 + (p3 & 0xff00) * 2 + (p4 & 0xff00);
          b = (p0 & 0xff) + (p1 & 0xff) * 2 + (p2 & 0xff) * 2 + (p3 & 0xff) * 2 + (p4 & 0xff);
...
static VDXScriptObject smoother_obj = {NULL, smoother_func_defs};
