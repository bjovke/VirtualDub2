...
void VDVFSharpen::Run()
{
  ...
  *dst++ = do_conv(src++, m, C_BOTTOMOK | C_RIGHTOK, pitch);
  ...
  do
  {
    *dst++ = do_conv(src++, m, C_BOTTOMOK | C_LEFTOK | C_RIGHTOK, pitch);
  } while (--w);
...
#ifdef USE_ASM
  asm_sharpen_run(dst + 1, src + 1, fa->src.w - 2, fa->src.h - 2, fa->src.pitch, fa->dst.pitch, m[0], m[4]);
#endif
  ...
  *dst++ = do_conv(src++, m, C_TOPOK | C_RIGHTOK, pitch);
  ...
  do
  {
    *dst++ = do_conv(src++, m, C_TOPOK | C_LEFTOK | C_RIGHTOK, pitch);
  } while (--w);
