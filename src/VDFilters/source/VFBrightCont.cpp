...
#ifndef _M_IX86
static void brightcont_run_trans(void *dst, uint32 width, uint32 height, ptrdiff_t pitch, const uint8 tab[256])
{
  do
  {
    uint8 *p = (uint8 *)dst;
...
static void brightcont_run_trans8(void *dst, uint32 width, uint32 height, ptrdiff_t pitch, const uint8 tab[256])
{
  do
  {
    uint8 *p = (uint8 *)dst;
...
VDVFilterBrightCont::VDVFilterBrightCont() : mVDXAFP(0) {}
    ...
    case nsVDXPixmap::kPixFormat_XRGB8888:
      ...
      if (mConfig.bright >= 0)
        ...
        asm_brightcont1_run(
          ...
          (-mConfig.bright) * 0x00001000L);
#else
      brightcont_run_trans(pxdst.data, pxdst.w, pxdst.h, pxdst.pitch, mConfig.mLookup);
#endif
      break;
...
{
public:
  VDVFilterBrightContDialog(VDVFilterBrightContConfig &config, IVDXFilterPreview *pifp)
    : VDDialogFrameW32(IDD_FILTER_BRIGHTCONT), mConfig(config), mpPreview(pifp)
  {}
...
#ifdef _MSC_VER
#pragma warning(disable : 4505) // warning C4505: 'VDVFilterBrightCont::[thunk]: __thiscall
                                // VDVFilterBrightCont::`vcall'{24,{flat}}' }'' : unreferenced local function has been
                                // removed
#endif
