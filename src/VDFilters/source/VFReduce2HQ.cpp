...
namespace
{
void Average(void *dst, const void *src, uint32 width, uint32 height, ptrdiff_t srcpitch, ptrdiff_t dstpitch)
{
  do
  {
    ...
    for (uint32 x = 0; x < width; ++x)
    {
      ...
      const uint32 losum =
        ((p4 & 0xff030303) * 2 + (p1 & 0xff070707) + (p3 & 0xff070707) + (p5 & 0xff070707) + (p7 & 0xff070707)) * 2 +
        (p0 & 0xff0f0f0f) + (p2 & 0xff0f0f0f) + (p6 & 0xff0f0f0f) + (p8 & 0xff0f0f0f);
...
static int reduce2hq_run(const VDXFilterActivation *fa, const VDXFilterFunctions *ff)
{
  ...
  if (!(fa->src.h & 1))
  {
    ...
    {
      rb =
        ((READSRC(0, 1) & 0xff00ff) + (READSRC(0, 2) & 0xff00ff) + (READSRC(1, 1) & 0xff00ff) +
         (READSRC(1, 2) & 0xff00ff));
      g =
        ((READSRC(0, 1) & 0x00ff00) + (READSRC(0, 2) & 0x00ff00) + (READSRC(1, 1) & 0x00ff00) +
         (READSRC(1, 2) & 0x00ff00));
    ...
    {
      rb =
        ((READSRC(-1, 1) & 0xff00ff) + (READSRC(-1, 2) & 0xff00ff) + (READSRC(0, 1) & 0xff00ff) +
         (READSRC(0, 2) & 0xff00ff) + (READSRC(1, 1) & 0xff00ff) + (READSRC(1, 2) & 0xff00ff));
      g =
        ((READSRC(-1, 1) & 0x00ff00) + (READSRC(-1, 2) & 0x00ff00) + (READSRC(0, 1) & 0x00ff00) +
         (READSRC(0, 2) & 0x00ff00) + (READSRC(1, 1) & 0x00ff00) + (READSRC(1, 2) & 0x00ff00));
      *dst++ = ((rb / 9) & 0x00ff0000) | ((rb & 0x0000ffff) / 9) | ((g / 9) & 0x00ff00);
      src += 2;
    } while (--w);
  ...
  if (!(fa->src.w & 1))
  {
    ...
    {
      rb =
        (((READSRC(0, 0) & 0xff00ff) + (READSRC(0, 1) & 0xff00ff) + (READSRC(0, 2) & 0xff00ff)) * 2 +
         (READSRC(1, 0) & 0xff00ff) + (READSRC(1, 1) & 0xff00ff) + (READSRC(1, 2) & 0xff00ff));
      g =
        (((READSRC(0, 0) & 0x00ff00) + (READSRC(0, 1) & 0x00ff00) + (READSRC(0, 2) & 0x00ff00)) * 2 +
