...
VDXScriptObject box_obj = {NULL, box_func_defs};

extern const VDXFilterDefinition g_VDVFBoxBlur = {

  NULL,
  NULL,
  NULL,       // next, prev, module
  "box blur", // name
...
static void box_filter_produce_row(
  uint32 *dst,
  uint16 *tmp,
  uint32 *src_add,
  uint32 *src_sub,
...
xloop:
      ...
      do
    {
      ...
      *dst++ = ((r * mult) & 0xff0000) + (((g * mult) & 0xff0000) >> 8) + ((b * mult) >> 16);
...
static void box_do_vertical_pass(
  uint32 *  dst,
  ptrdiff_t dstpitch,
  uint32 *  src,
  ptrdiff_t srcpitch,
...
int boxConfigProc(VDXFilterActivation *fa, const VDXFilterFunctions *ff, VDXHWND hwnd)
{
  ...
  res = DialogBoxParam(g_hInst, MAKEINTRESOURCE(IDD_FILTER_BOX), (HWND)hwnd, boxConfigDlgProc, (LPARAM)mfd);
...
bool boxFssProc(VDXFilterActivation *fa, const VDXFilterFunctions *ff, char *buf, int buflen)
{
  ...
  _snprintf(buf, buflen, "Config(%d, %d)", mfd->filter_width, mfd->filter_power);
