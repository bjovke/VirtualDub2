...
static void iq_average_row5(uint32 *_src, IQPixel *_dst, int _count)
{
  ...
  uint32 c;
  long   r, g, b;
  long   i1, q1, i2 = 0, q2 = 0, i3 = 0, q3 = 0, i4 = 0, q4 = 0, i5 = 0, q5 = 0;
...
static void rb_average_row5(uint32 *_src, IQPixel *_dst, int _count)
{
  ...
  *dst =
    ((((src[-1] & 0x00ff00ff) * 3 + (src[0] & 0x00ff00ff) * 2 + (src[1] & 0x00ff00ff) * 2 + (src[2] & 0x00ff00ff)) >>
      3) &
     0x00ff00ff) |
    ((((src[-1] & 0x0000ff00) * 3 + (src[0] & 0x0000ff00) * 2 + (src[1] & 0x0000ff00) * 2 + (src[2] & 0x0000ff00)) >>
      3) &
     0x0000ff00);
  src++;
  dst++;
  ...
  do
  {
    *dst = ((((src[-2] & 0x00ff00ff) + (src[-1] & 0x00ff00ff) * 2 + (src[0] & 0x00ff00ff) * 2 +
              (src[1] & 0x00ff00ff) * 2 + (src[2] & 0x00ff00ff)) >>
             3) &
            0x00ff00ff) |
           ((((src[-2] & 0x0000ff00) + (src[-1] & 0x0000ff00) * 2 + (src[0] & 0x0000ff00) * 2 +
              (src[1] & 0x0000ff00) * 2 + (src[2] & 0x0000ff00)) >>
             3) &
            0x0000ff00);
    ++src;
  ...
  *dst =
    ((((src[-2] & 0x00ff00ff) + (src[-1] & 0x00ff00ff) * 2 + (src[0] & 0x00ff00ff) * 2 + (src[1] & 0x00ff00ff) * 3) >>
      3) &
     0x00ff00ff) |
    ((((src[-2] & 0x0000ff00) + (src[-1] & 0x0000ff00) * 2 + (src[0] & 0x0000ff00) * 2 + (src[1] & 0x0000ff00) * 3) >>
      3) &
     0x0000ff00);
  ++src;
  dst++;
...
static void ChromaShift(uint32 *dst0, uint32 *src0, ptrdiff_t dstpitch, ptrdiff_t srcpitch, sint32 w, sint32 h)
{
  ...
  do
  {
    ...
    do
    {
      ...
      ydiff = (54 * ((int)((cd >> 16) & 255) - r) + 183 * ((int)((cd & 0xff00) >> 8) - g) + 19 * ((int)(cd & 255) - b) +
               128) >>
              8;
...
static int yiq_run(const VDXFilterActivation *fa, const VDXFilterFunctions *ff)
{
  ...
  {
    IQPixel *row_p = mfd->rows, *row_c = mfd->rows + 1 * fa->dst.w, *row_n = mfd->rows + 2 * fa->dst.w, *row_t;
  ...
  {
    IQPixel *row_f = mfd->rows, *row_p = mfd->rows + 1 * fa->dst.w, *row_c = mfd->rows + 2 * fa->dst.w,
            *row_n = mfd->rows + 3 * fa->dst.w, *row_l = mfd->rows + 4 * fa->dst.w, *row_t;
    ...
    do
    {
      ...
      do
      {
        ...
        c = *dst;
        d = ((((r1[0] & 0x00ff00ff) + 2 * (r2[0] & 0x00ff00ff) + 2 * (r3[0] & 0x00ff00ff) + 2 * (r4[0] & 0x00ff00ff) +
               (r5[0] & 0x00ff00ff)) >>
              3) &
             0x00ff00ff) |
            ((((r1[0] & 0x0000ff00) + 2 * (r2[0] & 0x0000ff00) + 2 * (r3[0] & 0x0000ff00) + 2 * (r4[0] & 0x0000ff00) +
               (r5[0] & 0x0000ff00)) >>
              3) &
             0x0000ff00);
        ...
        y = (54 * ((long)((c >> 16) & 255) - (long)((d >> 16) & 255)) +
             183 * ((long)((c >> 8) & 255) - (long)((d >> 8) & 255)) + 19 * ((long)((c)&255) - (long)((d)&255))) >>
            8;
#endif
        r = ((d >> 16) & 255) + y;
        g = ((d >> 8) & 255) + y;
  ...
  {
    IQPixel *row_f = mfd->rows, *row_p = mfd->rows + 2 * fa->dst.w, *row_c = mfd->rows + 4 * fa->dst.w,
            *row_n = mfd->rows + 6 * fa->dst.w, *row_l = mfd->rows + 8 * fa->dst.w, *row_t;
  ...
  {
    ChromaShift(
      fa->src.Address32(0, 0), fa->src.Address32(0, 1), -fa->src.pitch, -fa->src.pitch, fa->src.w, fa->src.h - 1);
  }
  else if (mfd->mode == YIQMODE_CHROMADOWN)
  {
    ChromaShift(
      fa->src.Address32i(0, 0), fa->src.Address32i(0, 1), fa->src.pitch, fa->src.pitch, fa->src.w, fa->src.h - 1);
  }
  else
  {
    int cr, cg, cb, bias;
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam))
      {
        case IDOK: {
          ...
          if (IsDlgButtonChecked(hDlg, IDC_MODE_Y))
            mfd->mode = YIQMODE_Y;
          if (IsDlgButtonChecked(hDlg, IDC_MODE_I))
            mfd->mode = YIQMODE_I;
          if (IsDlgButtonChecked(hDlg, IDC_MODE_Q))
            mfd->mode = YIQMODE_Q;
          if (IsDlgButtonChecked(hDlg, IDC_MODE_IQAVERAGE3))
            mfd->mode = YIQMODE_IQAVERAGE3;
          if (IsDlgButtonChecked(hDlg, IDC_MODE_IQAVERAGE5))
            mfd->mode = YIQMODE_IQAVERAGE5;
          if (IsDlgButtonChecked(hDlg, IDC_MODE_IQAV5TMP))
            mfd->mode = YIQMODE_IQAV5TMP;
          if (IsDlgButtonChecked(hDlg, IDC_MODE_CHROMAUP))
            mfd->mode = YIQMODE_CHROMAUP;
          if (IsDlgButtonChecked(hDlg, IDC_MODE_CHROMADOWN))
            mfd->mode = YIQMODE_CHROMADOWN;
        }
          EndDialog(hDlg, 0);
          return TRUE;
    ...
    mfd ?
      (mfd->mode == YIQMODE_Y ? "Y channel" :
                                ...
                                mfd->mode == YIQMODE_Q ? "Q channel" :
                                                         ...
                                                         mfd->mode == YIQMODE_IQAV5TMP ?
                                                         "I/Q 5x5 + t/s" :
                                                         mfd->mode == YIQMODE_CHROMAUP ? "chroma up" : "chroma down") :
      "unconfigured");
}
...
static VDXScriptObject yiq_obj = {NULL, yiq_func_defs};
