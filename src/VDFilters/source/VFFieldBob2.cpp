...
{
public:
  VFBitmapRowIterator(const VDXBitmap &vb) : mpData((char *)vb.data + vb.pitch * (vb.h - 1)), mPitch(-vb.pitch) {}

  uint32 *operator*()
  {
    return (uint32 *)mpData;
  }
  const uint32 *operator*() const
  {
    return (const uint32 *)mpData;
  }

  uint32 *operator[](uint32 y)
  {
    return (uint32 *)(mpData + mPitch * y);
  }
  const uint32 *operator[](uint32 y) const
  {
    return (const uint32 *)(mpData + mPitch * y);
  }

  VFBitmapRowIterator &operator--()
  {
    mpData -= mPitch;
    return *this;
  }
...
template<class T> class VDPixmapRowIterator
{
public:
  VDPixmapRowIterator(const VDXPixmap &px) : mpData((char *)px.data), mPitch(px.pitch) {}

  VDPixmapRowIterator(const VDPixmap &px) : mpData((char *)px.data), mPitch(px.pitch) {}
  ...
  {
    return (const T *)mpData;
  }

  T *operator[](uint32 y)
  {
    return (T *)(mpData + mPitch * y);
  }
  const T *operator[](uint32 y) const
  {
    return (const T *)(mpData + mPitch * y);
  }

  VDPixmapRowIterator &operator--()
  {
    mpData -= mPitch;
    return *this;
  }
...
struct VDVideoFilterFieldBob2Config
{
  ...
  VDVideoFilterFieldBob2Config() : mMode(kModeBob), mbOddFieldFirst(true) {}
};
...
VDVideoFilterFieldBob2ConfigDialog::VDVideoFilterFieldBob2ConfigDialog(
  VDVideoFilterFieldBob2Config &config,
  IVDXFilterPreview2 *          ifp2)
  : VDDialogFrameW32(IDD_FILTER_BOBDOUBLER), mifp2(ifp2), mConfig(config), mConfigBackup(config)
{}
...
void VDVideoFilterFieldBob2::Run()
{
  ...
  else if (mConfig.mMode == VDVideoFilterFieldBob2Config::kModeAdaptiveELA)
  {
    ...
    uint32           w     = pxdst.w;
    uint32           h     = pxdst.h;
    mDifferencer.DifferencePixmaps(
      VDPixmapExtractField(mDifferenceBuffer, odd),
      VDPixmapExtractField(mPrevBuffers[0], odd),
      VDPixmapExtractField((const VDPixmap &)pxsrc, odd));

    VDPixmapRowIterator<uint32> srcIt(pxsrc);
    VDPixmapRowIterator<uint32> src2It(
      !second ? (const VDXPixmap &)static_cast<const VDPixmap &>(mPrevBuffers[0]) :
                static_cast<const VDXPixmap &>(pxsrc));
    ...
    for (uint32 y = 1; y < h - 1; ++y)
    {
      ...
      if (scanOdd == odd)
        ...
        elascan2(
          *dstIt++, srcIt[y - 1], src2It[y], srcIt[y + 1], diffIt[y - 1], diffIt[y], diffIt[y + 1], w, mTempRow.data());
    }
...
extern const VDXFilterDefinition g_VDVFFieldBob2 = VDXVideoFilterDefinition<VDVideoFilterFieldBob2>(
  NULL,
  "bob doubler",
