...
struct VDVFMergeLayersConfig
{
  ...
  VDVFMergeLayersConfig() : mMode(kMode_Lerp) {}
};
...
VDVFMergeLayersDialog::VDVFMergeLayersDialog(VDVFMergeLayersConfig &config, IVDXFilterPreview2 *ifp2)
  : VDDialogFrameW32(IDD_FILTER_MERGELAYERS), mConfig(config), mifp2(ifp2)
{
...
VDVFMergeLayers::VDVFMergeLayers() : mVDXAShader(0) {}
...
void VDVFMergeLayers::Run()
{
  ...
  switch (mConfig.mMode)
  {
    case VDVFMergeLayersConfig::kMode_Lerp:
      if (sse2)
        VDDSPProcessPlane4(
          dstp, dstpitch, srcp1, src1pitch, srcp2, src2pitch, srcp3, src3pitch, w16, h, VDDSPBlend8_Lerp_SSE2);
      else
        VDDSPProcessPlane4(
          dstp, dstpitch, srcp1, src1pitch, srcp2, src2pitch, srcp3, src3pitch, w16, h, VDDSPBlend8_Lerp);
      return;

    case VDVFMergeLayersConfig::kMode_Select:
      if (sse2)
        VDDSPProcessPlane4(
          dstp, dstpitch, srcp1, src1pitch, srcp2, src2pitch, srcp3, src3pitch, w16, h, VDDSPBlend8_Select_SSE2);
      else
        VDDSPProcessPlane4(
          dstp, dstpitch, srcp1, src1pitch, srcp2, src2pitch, srcp3, src3pitch, w16, h, VDDSPBlend8_Select);
      break;
  }
}
...
extern const VDXFilterDefinition g_VDVFMergeLayers =
  VDXVideoFilterDefinition<VDVFMergeLayers>(NULL, "merge layers", "Blends two layers together using a third layer.");
