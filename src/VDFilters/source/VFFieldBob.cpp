...
static const char *const g_szModes[] = {"none", "smooth", "up", "down"};
...
void fieldbob_string2(const VDXFilterActivation *fa, const VDXFilterFunctions *ff, char *buf, int maxlen)
{
  ...
  if (!(sfd = (fieldbobFilterData *)fa->filter_data))
    return;
...
BOOL APIENTRY fieldbobConfigDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message)
  {
    case WM_INITDIALOG: {
      ...
      {
        case 0:
          CheckDlgButton(hDlg, IDC_EVEN_NONE, BST_CHECKED);
          break;
        case 1:
          CheckDlgButton(hDlg, IDC_EVEN_SMOOTH, BST_CHECKED);
          break;
        case 2:
          CheckDlgButton(hDlg, IDC_EVEN_HALFUP, BST_CHECKED);
          break;
        case 3:
          CheckDlgButton(hDlg, IDC_EVEN_HALFDOWN, BST_CHECKED);
          break;
      }
      switch (sfd->oddmode)
      {
        case 0:
          CheckDlgButton(hDlg, IDC_ODD_NONE, BST_CHECKED);
          break;
        case 1:
          CheckDlgButton(hDlg, IDC_ODD_SMOOTH, BST_CHECKED);
          break;
        case 2:
          CheckDlgButton(hDlg, IDC_ODD_HALFUP, BST_CHECKED);
          break;
        case 3:
          CheckDlgButton(hDlg, IDC_ODD_HALFDOWN, BST_CHECKED);
          break;
      }
    }
      return (TRUE);
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam))
      {
        case IDOK: {
          ...
          if (IsDlgButtonChecked(hDlg, IDC_EVEN_NONE))
            sfd->evenmode = 0;
          if (IsDlgButtonChecked(hDlg, IDC_EVEN_SMOOTH))
            sfd->evenmode = 1;
          if (IsDlgButtonChecked(hDlg, IDC_EVEN_HALFUP))
            sfd->evenmode = 2;
          if (IsDlgButtonChecked(hDlg, IDC_EVEN_HALFDOWN))
            sfd->evenmode = 3;

          if (IsDlgButtonChecked(hDlg, IDC_ODD_NONE))
            sfd->oddmode = 0;
          if (IsDlgButtonChecked(hDlg, IDC_ODD_SMOOTH))
            sfd->oddmode = 1;
          if (IsDlgButtonChecked(hDlg, IDC_ODD_HALFUP))
            sfd->oddmode = 2;
          if (IsDlgButtonChecked(hDlg, IDC_ODD_HALFDOWN))
            sfd->oddmode = 3;
        }
          EndDialog(hDlg, TRUE);
          return TRUE;
...
int fieldbob_config(VDXFilterActivation *fa, const VDXFilterFunctions *ff, VDXHWND hWnd)
{
  return !DialogBoxParam(
    g_hInst, MAKEINTRESOURCE(IDD_FILTER_FIELDBOB), (HWND)hWnd, (DLGPROC)fieldbobConfigDlgProc, (LPARAM)fa->filter_data);
}
      ...
      if (--h > 0)
      {
        ...
        do
        {
          ...
          do
          {
            ...
            *dst++ = ((a >> 2) & 0x3f3f3f) + ((b & 0xfcfcfc) >> 2) * 3 +
                     ((((a & 0x030303) + (b & 0x030303) * 3) >> 2) & 0x030303);
          } while (--w2);
      ...
      if (--h > 0)
      {
        ...
        do
        {
          ...
          do
          {
            ...
            *dst++ = ((a >> 2) & 0x3f3f3f) * 3 + ((b & 0xfcfcfc) >> 2) +
                     ((((a & 0x030303) * 3 + (b & 0x030303)) >> 2) & 0x030303);
          } while (--w2);
...
static VDXScriptObject fieldbob_obj = {NULL, fieldbob_func_defs};
